...
namespace Mediaportal.TV.Server.TVLibrary.Interfaces.Analyzer
{
  ...
  internal interface IGrabberSiScte : IGrabberSiAtsc
  {
    ...
    /// <param name="pathSelect">An indication of the physical transmission path that the channel is associated with. Only applicable for cable channels. Refer to SCTE specifications for further detail.</param>
    ...
    /// <summary>
    /// Get the number of SCTE channels received by the grabber from the short-form virtual channel table.
    /// </summary>
    /// <returns>the number of SCTE channels received by the grabber from the SVCT</returns>
    [PreserveSig]
    new ushort GetSvctChannelCount();
    ...
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.I1)]
    new bool GetSvctChannel(ushort index,
