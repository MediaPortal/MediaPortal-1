...
namespace Mediaportal.TV.Server.TVLibrary.Implementations.Scte.Parser
{
  ...
  /// </summary>
  internal class ParserNit : ParserBase
  {
    ...
    private NitCarrierDefinitionDelegate _carrierDefinitionEventDelegate = null;
    private NitModulationModeDelegate _modulationModeEventDelegate = null;
    ...
    public event NitCarrierDefinitionDelegate OnCarrierDefinition
    {
      add
      {
        lock (_lock)
        {
          _carrierDefinitionEventDelegate += value;
        }
      }
      remove
      {
    ...
    public event NitModulationModeDelegate OnModulationMode
    {
      add
      {
        lock (_lock)
        {
          _modulationModeEventDelegate += value;
        }
      }
      remove
      {
    ...
    public void Decode(byte[] section)
    {
      lock (_lock)
      {
        ...
        {
          this.LogError("NIT: invalid section size {0}, expected at least 13 bytes", section.Length);
          return;
        }
        ...
        if (tableSubtype == TableSubtype.TransponderData)
        {
          ...
          {
            this.LogError("NIT: invalid section length at satellite ID, pointer = {0}, end of section = {1}", pointer, endOfSection);
            return;
          }
          satelliteId = section[pointer++];
        }
        this.LogDebug("NIT: section length = {0}, protocol version = {1}, first index = {2}, number of records = {3}, transmission medium = {4}, table subtype = {5}, satellite ID = {6}",
        ...
        for (byte i = 0; i < numberOfRecords; i++)
        {
          try
          {
            switch (tableSubtype)
            {
              ...
              case TableSubtype.TransponderData:
                ...
                break;
              default:
                this.LogError("NIT: unsupported table subtype {0}", tableSubtype);
                return;
            }
          }
          catch (Exception ex)
          ...
          if (pointer >= endOfSection)
          {
            this.LogError("NIT: invalid section length at table descriptor count, pointer = {0}, end of section = {1}, loop = {2}", pointer, endOfSection, i);
            return;
          }
          ...
          for (byte d = 0; d < descriptorCount; d++)
          {
            ...
            {
              this.LogError("NIT: detected table descriptor count {0} is invalid, pointer = {1}, end of section = {2}, loop = {3}, inner loop = {4}", descriptorCount, pointer, endOfSection, i, d);
              return;
            }
            byte tag = section[pointer++];
            byte length = section[pointer++];
            this.LogDebug("NIT: table descriptor, tag = 0x{0:x}, length = {1}", tag, length);
            if (pointer + length > endOfSection)
            {
              this.LogError("NIT: invalid table descriptor length {0}, pointer = {1}, end of section = {2}, loop = {3}, inner loop = {4}", length, pointer, endOfSection, i, d);
              return;
            }
            pointer += length;
          }
        ...
        while (pointer + 1 < endOfSection)
        {
          byte tag = section[pointer++];
          byte length = section[pointer++];
          this.LogDebug("NIT: descriptor, tag = 0x{0:x}, length = {1}", tag, length);
          if (pointer + length > endOfSection)
          {
            this.LogError("NIT: invalid descriptor length {0}, pointer = {1}, end of section = {2}", length, pointer, endOfSection);
            return;
          }
        ...
        if (pointer != endOfSection)
        {
          this.LogError("NIT: corruption detected at end of section, pointer = {0}, end of section = {1}", pointer, endOfSection);
          return;
        }
          ...
          _carrierDefinitionEventDelegate != null)
        {
          ...
          {
            _tableCompleteEventDelegate(MgtTableType.NitCds);
          ...
          _modulationModeEventDelegate != null)
        {
          ...
          {
            _tableCompleteEventDelegate(MgtTableType.NitMms);
            _tableCompleteEventDelegate = null;
          }
        }
      }
    ...
    private void DecodeCarrierDefinition(byte[] section, int endOfSection, ref int pointer, ref byte firstIndex, AtscTransmissionMedium transmissionMedium)
    {
      ...
      {
        throw new TvException("NIT: corruption detected at carrier definition, pointer = {0}, end of section = {1}", pointer, endOfSection);
      }
      ...
      if (_carrierDefinitionEventDelegate != null)
      {
        int carrierFrequency = firstCarrierFrequency;
        for (byte f = 0; f < numberOfCarriers; f++)
        {
          _carrierDefinitionEventDelegate(transmissionMedium, firstIndex++, carrierFrequency);
    ...
    private void DecodeModulationMode(byte[] section, int endOfSection, ref int pointer, ref byte firstIndex, AtscTransmissionMedium transmissionMedium)
    {
      ...
      if (_modulationModeEventDelegate != null)
      {
        _modulationModeEventDelegate(transmissionMedium, firstIndex++, transmissionSystem, innerCodingMode, isSplitBitstreamMode, modulationFormat, symbolRate);
      }
    }
