...
namespace Mediaportal.TV.Server.TVLibrary.Implementations.DVB.DisEqC
{
  ...
  public class GenPixBDA : IDisposable
  {
    ...
    public GenPixBDA(IBaseFilter tunerFilter)
    {
      ...
      Log.Debug("GenPix tuner filter name: {0}", tInfo.achName);
      //check the pin name
      IPin pin = DsFindPin.ByDirection(tunerFilter, PinDirection.Output, 0);
      PinInfo pInfo;
      pin.QueryPinInfo(out pInfo);
      Log.Debug("GenPix tuner filter pin name: {0}", pInfo.name);
      if (pin != null)
      {
        ...
        if (_propertySet != null)
        {
          ...
          {
            Log.Debug("GenPix BDA: DVB-S card found!");
            _isGenPix = true;
          ...
          {
            Log.Debug("GenPix BDA: DVB-S card NOT found!");
            _isGenPix = false;
          }
        }
      }
      else
        Log.Info("GenPix BDA: tuner pin not found!");
    }
    ...
    public void SendDiseqCommand(ScanParameters parameters, DVBSChannel channel)
    {
      if (_isGenPix == false)
        return;
      Log.Debug("SendDiseqc: {0},{1}", parameters.ToString(), channel.ToString());
      ...
      Log.Debug("GenPix: SendDiseqCommand: {0} with length {1}", txt, len);
      ...
      if (hr != 0)
      {
        Log.Info("GenPix: SendDiseqCommand returned: 0x{0:X} - {1}{2}", hr, HResult.GetDXErrorString(hr),
