...
namespace Mediaportal.TV.Server.TVLibrary.Implementations.DVB.ConditionalAccess.KNC
{
  ...
  public class KNCAPI : IDisposable, ICiMenuActions
  {
    ...
    public KNCAPI(IBaseFilter tunerFilter, uint DeviceIndex)
    {
      ...
      Log.Debug("KNC: card {0} detected: {1}", m_iDeviceIndex, info.achName);
    ...
    public void OpenCI()
    {
      ...
      {
        Log.Debug("KNC: card {0} HW Enable failed", m_iDeviceIndex);
      }
      ...
      unsafe
      {
        ...
        if (KNCBDA_CI_Enable(m_iDeviceIndex, m_tunerFilter, ptrCallback) != 0)
        {
          ...
          {
            Log.Debug("KNC: card {0} CI slot enabled successfully", m_iDeviceIndex);
            ...
            if (KNCBDA_CI_GetName(m_iDeviceIndex, nameBuffer, (uint)nameBuffer.MaxCapacity) != 0)
              Log.Debug("KNC: card {0} CAM Type: {1}", m_iDeviceIndex, nameBuffer);
            else
              Log.Debug("KNC: CI_GetName failed.");
          }
          else
          {
            Log.Debug("KNC: card {0} detected without CAM", m_iDeviceIndex);
          }
        }
      }
    }
    ...
    protected virtual void Dispose(bool disposing)
    {
      ...
      CloseCI();
      Log.Debug("KNC: Disposing CI handler");
    ...
    public bool IsCamReady()
    {
      ...
      Log.Info("KNC: IsCAMReady {0}", yesNo);
      return yesNo;
    }
    ...
    public bool SendPMT(byte[] pmt, int PMTlength)
    {
      ...
      Log.Info("KNC: SendPMT success = {0}", succeeded);
    ...
    public void SendDiseqCommand(ScanParameters parameters, DVBSChannel channel)
    {
      switch (channel.DisEqc)
      {
        case DisEqcType.Level1AA:
          Log.Info("KNC:  Level1AA - SendDiSEqCCommand(0x00)");
          SendDiSEqCCommand(0x00);
          break;
        case DisEqcType.Level1AB:
          Log.Info("KNC:  Level1AB - SendDiSEqCCommand(0x01)");
          SendDiSEqCCommand(0x01);
          break;
        case DisEqcType.Level1BA:
          Log.Info("KNC:  Level1BA - SendDiSEqCCommand(0x0100)");
          SendDiSEqCCommand(0x0100);
          break;
        case DisEqcType.Level1BB:
          Log.Info("KNC:  Level1BB - SendDiSEqCCommand(0x0101)");
          SendDiSEqCCommand(0x0101);
          break;
        case DisEqcType.SimpleA:
          Log.Info("KNC:  SimpleA - SendDiSEqCCommand(0x00)");
          SendDiSEqCCommand(0x00);
          break;
        case DisEqcType.SimpleB:
          Log.Info("KNC:  SimpleB - SendDiSEqCCommand(0x01)");
          SendDiSEqCCommand(0x01);
          break;
        default:
          return;
    ...
    {
      Log.Info("KNC:  SendDiSEqC Command {0}", ulRange);
          ...
          if (ControlNode != null)
          {
            ...
            if (hr == 0)
            {
              if (FrequencyFilter != null)
              {
                ...
                Log.Info("KNC:  put_Range:{0} success:{1}", ulRange, hr);
                if (hr == 0)
                {
                  ...
                  if (hr == 0)
                  {
                    ...
                    if (hr == 0)
                    {
                      Log.Info("KNC:  CommitChanges() Succeeded");
                      return true;
                    }
                    // reset configuration
                    Log.Info("KNC:  CommitChanges() Failed!");
                    DecviceControl.StartChanges();
                    DecviceControl.CommitChanges();
                    return false;
                  }
                  Log.Info("KNC:  CheckChanges() Failed!");
                  return false;
                }
                Log.Info("KNC:  put_Range Failed!");
                return false;
              }
            }
          }
      ...
      Log.Info("KNC:  GetControlNode Failed!");
      return false;
    }
    ...
    public bool IsCamPresent()
    {
      ...
      Log.Info("KNC: IsCIAvailable {0}", yesNo);
      return yesNo;
    }
    ...
    public void OnCiState(byte slot, KNCCiSlotStatus State, String lpszMessage, IntPtr pParam)
    {
      lock (this)
      {
        ...
        Log.Debug("KNC: card {0} CI State: {1} {2}", m_iDeviceIndex, lpszMessage, State);
      }
    }
    ...
    public void OnCiOpenDisplay(byte slot, IntPtr pParam)
    {
      lock (this)
      {
        Log.Debug("OnKncCiOpenDisplay slot: {0}", slot);
      }
    }
                         ...
                         IntPtr pParam)
    {
      try
      {
        lock (this)
        {
          Log.Debug("OnKncCiMenu slot:       {0}", slot);
          Log.Debug("OnKncCiMenu title:      {0}", lpszTitle);
          Log.Debug("OnKncCiMenu subtitle:   {0}", lpszSubTitle);
          Log.Debug("OnKncCiMenu bottom:     {0}", lpszSubTitle);
          Log.Debug("OnKncCiMenu lpszBottom: {0}", lpszBottom);
          Log.Debug("OnKncCiMenu nNumChoices:{0}", nNumChoices);
      ...
      {
        Log.Debug("OnKncCiMenu exception: {0}", ex.ToString());
      }
    }
    ...
    public void OnCiMenuChoice(byte slot, uint nChoice, String lpszText, IntPtr pParam)
    {
      try
      {
        lock (this)
        {
          Log.Debug("OnKncCiMenuChoice slot:{0} choice:{1} text:{2}", slot, nChoice, lpszText);
          if (m_ciMenuCallback != null)
          {
      ...
      {
        Log.Debug("OnKncCiMenuChoice exception: {0}", ex.ToString());
      }
    }
    ...
    public void OnCiRequest(byte slot, bool bBlind, uint nAnswerLength, String lpszText, IntPtr pParam)
    {
      try
      {
        lock (this)
        {
          Log.Debug("OnKncCiRequest slot:{0} bBlind:{1} nAnswerLength:{2} text:{3}", slot, bBlind, nAnswerLength,
                        lpszText);
          if (m_ciMenuCallback != null)
          {
      ...
      {
        Log.Debug("OnKncCiRequest exception: {0}", ex.ToString());
      }
    }
    ...
    public void OnCiCloseDisplay(byte slot, uint nDelay, IntPtr pParam)
    {
      try
      {
        lock (this)
        {
          Log.Debug("OnKncCiCloseDisplay slot:{0} nDelay:{1}", slot, nDelay);
          if (m_ciMenuCallback != null)
          {
      ...
      {
        Log.Debug("KncCiCloseDisplay exception: {0}", ex.ToString());
      }
    }
    ...
    public bool SelectMenu(byte choice)
    {
      if (!m_bCAM_present)
        return false;
      Log.Debug("KNC: Select CI Menu entry {0}", choice);
    ...
    public bool SendMenuAnswer(bool Cancel, String Answer)
    {
      ...
      Log.Debug("KNC: Send Menu Answer: {0}, Cancel: {1}", Answer, Cancel);
