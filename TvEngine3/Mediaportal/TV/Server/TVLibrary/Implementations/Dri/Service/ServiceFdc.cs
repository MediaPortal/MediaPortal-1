...
{
  internal class ServiceFdc : ServiceBase
  {
    ...
    public ServiceFdc(CpDevice device)
                            ...
                            out bool currentSpectrumInversion, out IList<ushort> currentPidList)
    {
      ...
      currentPidList = outParams[4].ToString().Split(',').Select(x => Convert.ToUInt16(x, 16)).ToList<ushort>();
    }
    ...
    public void AddPid(IList<ushort> addPidList, out byte remainingPidFilter)
    {
    ...
    public void RemovidPid(IList<ushort> removePidList)
    {
