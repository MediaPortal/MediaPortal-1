...
namespace Mediaportal.TV.Server.TVLibrary.Implementations.Dri.Service
{
  internal class ServiceTuner : ServiceBase
  {
    ...
    public ServiceTuner(CpDevice device)
                            ...
                            out uint currentFrequency, out TunerModulation currentModulation, out bool pcrLockStatus)
    {
      ...
      currentModulation = (TunerModulation)(string)outParams[1];
                                  ...
                                  out int currentSignalLevel, out uint currentSnr)
    {
      ...
      currentModulation = (TunerModulation)(string)outParams[2];
    ...
    public bool SeekSignal(uint startFrequency, uint stopFrequency, List<TunerModulation> newModulationList,
                            uint increment, bool seekUp, ushort timeToBlock)
    {
      if (_seekSignalAction == null)
      {
        this.LogWarn("DRI: device {0} does not implement a Tuner SeekSignal action", _device.UDN);
        return false;
      }
    ...
    public bool SeekCancel()
    {
      ...
      {
        this.LogWarn("DRI: device {0} does not implement a Tuner SeekCancel action", _device.UDN);
        return false;
      }
