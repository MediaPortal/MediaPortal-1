...
namespace Mediaportal.TV.Server.TVLibrary.Implementations.Dvb
{
  ...
  internal class ChannelScannerDvb : IChannelScannerInternal, ICallBackGrabber
  {
    ...
    public void ReloadConfiguration()
    {
      ...
      _preferFreesatChannelDetails = SettingsManagement.GetValue("scanPreferFreesatChannelDetails", false);
      this.LogDebug("  prefer Freesat details? = {0}", _preferFreesatChannelDetails);
      ...
      this.LogDebug("    sources...");
      string[] sourceIdStrings = SettingsManagement.GetValue("scanLogicalChannelNumberSources", string.Empty).Split(',');
    ...
    public virtual void Scan(IChannel channel, bool isFastNetworkScan, out IList<ScannedChannel> channels, out IDictionary<ChannelGroupType, IDictionary<ulong, string>> groupNames)
    {
      ...
      try
      {
        ...
        do
        {
          ...
          {
            this.LogInfo("scan DVB: scan completed, tables seen = [{0}], tables complete = [{1}]", _seenTables, _completeTables);
            break;
          }
    ...
    public virtual IList<ScannedTransmitter> ScanNetworkInformation(IChannel channel)
    {
      ...
      try
      {
        ...
        do
        {
          ...
          {
            this.LogInfo("scan DVB: NIT scan completed, tables seen = [{0}], tables complete = [{1}]", _seenTables, _completeTables);
            break;
          }
    ...
    private LogicalChannelNumber? LoadLogicalChannelNumberSourceConfiguration(LogicalChannelNumberSource source)
    {
      ...
      if (settingName != null)
      {
        string[] configParts = SettingsManagement.GetValue(settingName, string.Empty).Split(',');
        if (configParts.Length >= 2)
        {
          lcn.TableId = 0x4a;
          if (
          ...
          {
            this.LogDebug("      source              = {0}", source);
            this.LogDebug("        bouquet ID        = {0}", lcn.TableIdExtension);
            this.LogDebug("        region ID         = {0}", lcn.TableIdExtension);
            return lcn;
          }
        }
        return null;
      ...
      {
        ushort bouquetId = (ushort)SettingsManagement.GetValue("scanProviderFreeviewSatellite", 0);
        if (bouquetId > 0)
        {
          lcn.TableId = 0x4a;
          lcn.TableIdExtension = bouquetId;
          this.LogDebug("      source              = {0}", source);
          this.LogDebug("        bouquet ID        = {0}", bouquetId);
          return lcn;
        }
        return null;
      }
    ...
    private void CollectServices(IGrabberSiDvb grabber, IChannel tuningChannel, ushort currentTransportStreamId, IDictionary<uint, ProgramInfo> programs, bool isSdtActualReceived, bool collectSdtOtherServices, out ushort currentTransportStreamOriginalNetworkId, out IDictionary<uint, ScannedChannel> channels, out IDictionary<ChannelGroupType, IDictionary<ulong, string>> groupNames)
    {
      ...
      for (ushort i = 0; i < serviceCount; i++)
      {
        ...
        this.LogInfo("    LCN count = {0, -3}, distinct values = [{1}], Dish sub-channel number = {2}, selected LCN = {3}", logicalChannelNumberCount, string.Join(", ", distinctLcns), dishSubChannelNumber, preferredLcnString);
        if (distinctLcns.Count > 1)
        {
          for (ushort n = 0; n < logicalChannelNumberCount; n++)
          {
        ...
        {
          this.LogWarn("scan DVB: service tuning details are not available, ONID = {0}, TSID = {1}, service ID = {2}", originalNetworkId, transportStreamId, serviceId);
          continue;
        }
        ...
        newChannel.LogicalChannelNumber = preferredLcnString;
