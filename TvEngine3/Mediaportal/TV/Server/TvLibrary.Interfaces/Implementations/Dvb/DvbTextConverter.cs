    ...
    public static string Convert(byte[] stringBytes, int encodedByteCount, int offset, out int decodedByteCount)
    {
        ...
        if (b1 < 0x20)
        {
          ...
          switch (b1)
          {
            ...
            case 0x10:
              ...
              {
                Log.Warn("DVB text: unsupported three byte encoding byte 0x{0:x}, byte count = {1}, encoded byte count = {2}, offset = {3}", b1, stringBytes.Length, encodedByteCount, offset);
                Dump.DumpBinary(stringBytes);
                return string.Empty;
              }
              break;
            ...
            default:
              if (b1 > 0xb)
              {
                Log.Warn("DVB text: unsupported one byte encoding byte 0x{0:x}, byte count = {1}, encoded byte count = {2}, offset = {3}", b1, stringBytes.Length, encodedByteCount, offset);
                Dump.DumpBinary(stringBytes);
                return string.Empty;
              }
      ...
      switch (codePage)
      {
        ...
        case 28602:   // ISO/IEC 8859-12
          Log.Warn("DVB text: selected ISO/IEC 8859-12 encoding does not exist, byte count = {0}, encoded byte count = {1}, offset = {2}", stringBytes.Length, encodedByteCount, offset);
          Dump.DumpBinary(stringBytes);
          return string.Empty;
        default:
