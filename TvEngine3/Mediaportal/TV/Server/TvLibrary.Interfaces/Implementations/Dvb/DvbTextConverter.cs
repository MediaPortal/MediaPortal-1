...
namespace Mediaportal.TV.Server.TVLibrary.Interfaces.Implementations.Dvb
{
  ...
  public class DvbTextConverter
  {
    ...
    public static string Convert(byte[] buffer, int encodedByteCount, int offset, out int decodedByteCount)
    {
      ...
      {
        Log.Warn("DVB text: unexpected parameter values, buffer length = {0}, encoded byte count = {1}, offset = {2}", buffer.Length, encodedByteCount, offset);
      ...
      {
        Log.Warn("DVB text: {0}, buffer length = {1}, encoded byte count = {2}, offset = {3}", error, buffer.Length, encodedByteCount, offset);
        Dump.DumpBinary(buffer);
        return string.Empty;
      }
      ...
      try
      {
        switch (codePage)
        {
          ...
          case 28602:   // ISO/IEC 8859-12
            Log.Warn("DVB text: selected ISO/IEC 8859-12 encoding does not exist, buffer length = {0}, encoded byte count = {1}, offset = {2}", buffer.Length, encodedByteCount, offset);
            Dump.DumpBinary(buffer);
            return string.Empty;
          default:
      ...
      finally
      {
        ...
        {
          Log.Warn("DVB text: unexpected byte sequence seen, buffer length = {0}, encoded byte count = {1}, offset = {2}", buffer.Length, encodedByteCount, offset);
          Dump.DumpBinary(buffer);
        }
      }
    }
    ...
    private static void ProcessOneBytePerCharEncoding(byte[] buffer, int encodedByteCount, int offset, out int decodedByteCount, out bool isUnexpectedByteSequenceSeen, out byte[] content, out int contentByteCount, out byte[] emphasizedContent, out int emphasizedContentByteCount)
    {
      ...
      while (offset < stop)
      {
        ...
        {
          Log.Warn("DVB text: unexpected byte/character in 1-byte-per-character string, c = 0x{0:x2}, offset = {1}", b, offset - 1 - initialOffset);
        ...
        else if (b >= 0x80 && b <= 0x9f)
        {
          ...
          {
            Log.Warn("DVB text: unexpected control character in 1-byte-per-character string, c = 0x{0:x2}, offset = {1}", b, offset - 1 - initialOffset);
            isUnexpectedByteSequenceSeen = true;
          }
          continue;
        }
    ...
    private static void ProcessTwoBytePerCharEncoding(byte[] buffer, int encodedByteCount, int offset, out int decodedByteCount, out bool isUnexpectedByteSequenceSeen, out byte[] content, out int contentByteCount, out byte[] emphasizedContent, out int emphasizedContentByteCount)
    {
      ...
      while (offset + 1 < stop)
      {
        ...
        if (b1 == 0)
        {
          ...
          {
            Log.Warn("DVB text: unexpected byte/character in 2-byte-per-character string, c = 0x{0:x2}{1:x2}, offset = {2}", b1, b2, offset - 2 - initialOffset);
            isUnexpectedByteSequenceSeen = true;
            continue;
          }
        }
        ...
        if (b1 == 0xe0 && b2 >= 0x80 && b2 <= 0x9f)
        {
          ...
          {
            Log.Warn("DVB text: unexpected control character in 2-byte-per-character string, c = 0x{0:x2}{1:x2}, offset = {2}", b1, b2, offset - 2 - initialOffset);
            isUnexpectedByteSequenceSeen = true;
          }
          continue;
        }
    ...
    private static void ProcessVariableBytePerCharEncoding(byte[] buffer, int encodedByteCount, int offset, out int decodedByteCount, out bool isUnexpectedByteSequenceSeen, out byte[] content, out int contentByteCount, out byte[] emphasizedContent, out int emphasizedContentByteCount)
    {
      ...
      while (offset < stop)
      {
        ...
        {
          Log.Warn("DVB text: unexpected byte/character in variable-byte-per-character string, c = 0x{0:x}, offset = {1}", c, offset - charByteCount - initialOffset);
          isUnexpectedByteSequenceSeen = true;
          continue;
        }
        else if (c >= 0xee8280 && c <= 0xee829f)
        {
          ...
          {
            Log.Warn("DVB text: unexpected control character in variable-byte-per-character string, c = 0x{0:x}, offset = {1}", c, offset - charByteCount - initialOffset);
            isUnexpectedByteSequenceSeen = true;
          }
          continue;
        }
      ...
      foreach (byte b in bytes)
      {
        ...
        switch (b)
        {
          ...
          default:
            ...
            {
              Log.Warn("DVB text: unexpected byte/character in ISO/IEC 8859-10 string, c = 0x{0:x}, offset = {1}", b, offset);
              isUnexpectedByteSequenceSeen = true;
            }
            else
            {
      ...
      foreach (byte b in bytes)
      {
        ...
        switch (b)
        {
          ...
          default:
            if (b < 0x20 || (0x7f <= b && b <= 0x9f))
            {
              Log.Warn("DVB text: unexpected byte/character in ISO/IEC 8859-14 string, c = 0x{0:x}, offset = {1}", b, offset);
              isUnexpectedByteSequenceSeen = true;
            }
            else
            {
      ...
      while (true)
      {
        ...
        switch (b)
        {
          ...
          default:
                ...
                case 0xc1:
                  ...
                  switch (b)
                  {
                    ...
                    case 0x75:
                      ...
                      break;
                    default:
                      Log.Warn("DVB text: unexpected byte/character in ISO/IEC 6937-1 string, c = 0x{0:x2}{1:x2}, offset = {2}", b1, b, offset);
                      isUnexpectedByteSequenceSeen = true;
                      break;
                  }
                  #endregion
            ...
            {
              Log.Warn("DVB text: unexpected byte/character in ISO/IEC 6937-1 string, c = 0x{0:x}, offset = {1}", b, offset);
              isUnexpectedByteSequenceSeen = true;
            }
            else
            {
