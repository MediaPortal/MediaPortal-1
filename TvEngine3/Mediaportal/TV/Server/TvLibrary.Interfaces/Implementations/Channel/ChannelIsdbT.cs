...
{
  /// <summary>
  /// An implementation of <see cref="T:IChannel"/> for ISDB-T channels.
  /// </summary>  
  [DataContract]
  public class ChannelIsdbT : ChannelDvbBase, IChannelIsdb, IChannelOfdm
  {
    #region variables
    ...
    {
      ChannelIsdbT isdbtChannel = channel as ChannelIsdbT;
      if (
        isdbtChannel == null ||
        Frequency != isdbtChannel.Frequency ||
        Bandwidth != isdbtChannel.Bandwidth
      )
      {
        return true;
      }
    ...
    {
      ChannelIsdbT channel = obj as ChannelIsdbT;
      if (
        channel == null ||
        !base.Equals(obj) ||
    ...
    {
      return string.Format("ISDB-T, {0}, frequency = {1} kHz, bandwidth = {2} kHz",
