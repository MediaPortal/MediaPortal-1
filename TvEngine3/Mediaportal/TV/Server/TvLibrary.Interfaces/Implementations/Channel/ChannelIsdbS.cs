...
{
  /// <summary>
  /// An implementation of <see cref="T:IChannel"/> for ISDB-S channels.
  /// </summary>
  [DataContract]
  public class ChannelIsdbS : ChannelDvbBase, IChannelIsdb, IChannelSatellite
  {
    #region variables
    ...
    {
      ChannelIsdbS isdbsChannel = channel as ChannelIsdbS;
      if (
        isdbsChannel == null ||
        Longitude != isdbsChannel.Longitude ||
        Frequency != isdbsChannel.Frequency ||
        Polarisation != isdbsChannel.Polarisation ||
        ModulationScheme != isdbsChannel.ModulationScheme ||
        SymbolRate != isdbsChannel.SymbolRate ||
        FecCodeRate != isdbsChannel.FecCodeRate
      )
      {
        return true;
      }
    ...
    {
      ChannelIsdbS channel = obj as ChannelIsdbS;
      if (
        channel == null ||
        !base.Equals(obj) ||
    ...
    {
      return string.Format("ISDB-S, {0}, satellite ID = {1}, frequency = {2} kHz, polarisation = {3}, modulation scheme = {4}, symbol rate = {5} ks/s, FEC code rate = {6}",
