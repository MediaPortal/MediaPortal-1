...
{
  /// <summary>
  /// An implementation of <see cref="T:IChannel"/> for AM radio channels.
  /// </summary>
  [DataContract]
  public class ChannelAmRadio : ChannelBase, IChannelPhysical
  {
    #region variables
    ...
    public override bool IsDifferentTransmitter(IChannel channel)
    {
      ...
      if (amRadioChannel == null || Frequency != amRadioChannel.Frequency)
      {
        return true;
      }
      return false;
    ...
    {
      ChannelAmRadio channel = obj as ChannelAmRadio;
      if (
        channel == null ||
        !base.Equals(obj) ||
    ...
    {
      return string.Format("AM radio, {0}, frequency = {1} kHz", base.ToString(), Frequency);
    }
