...
{
  public partial class ScanCable : SectionSettings
  {
    public const BroadcastStandard SUPPORTED_BROADCAST_STANDARDS = BroadcastStandard.DvbC | BroadcastStandard.IsdbC;
    ...
    public ScanCable(string name, int tunerId)
      : base(name)
    {
      _tunerId = tunerId;
      InitializeComponent();
    ...
    {
      this.LogDebug("scan cable: activating, tuner ID = {0}", _tunerId);
      ...
      {
        string country = ServiceAgents.Instance.SettingServiceAgent.GetValue("scanCable" + _tunerId + "Country", System.Globalization.RegionInfo.CurrentRegion.EnglishName);
        string region = ServiceAgents.Instance.SettingServiceAgent.GetValue("scanCable" + _tunerId + "Region", string.Empty);
        string transmitter = ServiceAgents.Instance.SettingServiceAgent.GetValue("scanCable" + _tunerId + "Transmitter", TuningDetailFilter.ALL_TUNING_DETAIL_ITEM);
        _tuningDetailFilter = new TuningDetailFilter(_tunerId, TuningDetailGroup.Cable, comboBoxCountry, country, comboBoxRegionProvider, region, comboBoxTransmitter, transmitter);
        ...
        comboBoxBroadcastStandard.SelectedItem = ((BroadcastStandard)ServiceAgents.Instance.SettingServiceAgent.GetValue("scanCable" + _tunerId + "BroadcastStandard", (int)BroadcastStandard.DvbC)).GetDescription();
      ...
      numericTextBoxFrequency.Value = ServiceAgents.Instance.SettingServiceAgent.GetValue("scanCable" + _tunerId + "Frequency", 163000);
      comboBoxModulation.SelectedItem = ((ModulationSchemeQam)ServiceAgents.Instance.SettingServiceAgent.GetValue("scanCable" + _tunerId + "Modulation", (int)ModulationSchemeQam.Qam256)).GetDescription();
      if (comboBoxModulation.SelectedItem == null)
      {
        comboBoxModulation.SelectedIndex = 0;
      }
      numericTextBoxSymbolRate.Value = ServiceAgents.Instance.SettingServiceAgent.GetValue("scanCable" + _tunerId + "SymbolRate", 6875);
    ...
    {
      this.LogDebug("scan cable: deactivating, tuner ID = {0}", _tunerId);

      DebugSettings();
      ServiceAgents.Instance.SettingServiceAgent.SaveValue("scanCable" + _tunerId + "Country", comboBoxCountry.SelectedItem.ToString());
      ServiceAgents.Instance.SettingServiceAgent.SaveValue("scanCable" + _tunerId + "Region", comboBoxRegionProvider.SelectedItem.ToString());
      ServiceAgents.Instance.SettingServiceAgent.SaveValue("scanCable" + _tunerId + "Transmitter", comboBoxTransmitter.SelectedItem.ToString());
      ServiceAgents.Instance.SettingServiceAgent.SaveValue("scanCable" + _tunerId + "BroadcastStandard", Convert.ToInt32(typeof(BroadcastStandard).GetEnumFromDescription((string)comboBoxBroadcastStandard.SelectedItem)));
      ServiceAgents.Instance.SettingServiceAgent.SaveValue("scanCable" + _tunerId + "Frequency", numericTextBoxFrequency.Value);
      ServiceAgents.Instance.SettingServiceAgent.SaveValue("scanCable" + _tunerId + "Modulation", Convert.ToInt32(typeof(ModulationSchemeQam).GetEnumFromDescription((string)comboBoxModulation.SelectedItem)));
      ServiceAgents.Instance.SettingServiceAgent.SaveValue("scanCable" + _tunerId + "SymbolRate", numericTextBoxSymbolRate.Value);
    ...
    private void buttonScan_Click(object sender, EventArgs e)
    {
      ...
      {
        this.LogInfo("scan cable: start scanning, country = {0}, region = {1}", comboBoxCountry.SelectedItem, comboBoxRegionProvider.SelectedItem);
