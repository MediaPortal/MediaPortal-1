...
namespace Mediaportal.TV.Server.Plugins.TunerExtension.Genpix
{
  ...
  public class Genpix : BaseCustomDevice, ICustomTuner, IDiseqcDevice
  {
    ...
    public override bool Initialise(string tunerExternalId, CardType tunerType, object context)
    {
      ...
      {
        this.LogDebug("Genpix: property set not supported, hr = 0x{0:x}", hr);
        return false;
      }
    ...
    public bool Tune(IChannel channel)
    {
      ...
      this.LogError("Genpix: failed to tune, hr = 0x{0:x}", hr);
      return false;
    }
    ...
    public bool SetToneState(ToneBurst toneBurstState, Tone22k tone22kState)
    {
      ...
      this.LogError("Genpix: failed to set tone state, hr = 0x{0:x}", hr);
      return false;
    }
