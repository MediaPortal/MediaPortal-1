...
namespace Mediaportal.TV.Server.Plugins.TunerExtension.HauppaugeEcp
{
  ...
  /// </summary>
  public class HauppaugeEncoder : BaseCustomDevice, IEncoder
  {
    #region enums
    ...
    public override void OnBeforeTune(ITVCard tuner, IChannel currentChannel, ref IChannel channel, out TunerAction action)
    {
      ...
      {
        this.LogError("Hauppauge encoder: result = failure, hr = 0x{0:x} ({1}), byte count = {2}", hr, HResult.GetDXErrorString(hr), returnedByteCount);
        return;
      }
