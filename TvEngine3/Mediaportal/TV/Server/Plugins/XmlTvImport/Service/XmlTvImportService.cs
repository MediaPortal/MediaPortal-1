...
namespace Mediaportal.TV.Server.Plugins.XmlTvImport.Service
{
  public class XmlTvImportService : IXmlTvImportService
  {
    private static XmlTvImport _importer = null;

    public static XmlTvImport Importer
    {
      set
      {
        _importer = value;
    ...
    public IDictionary<string, IDictionary<string, IList<string>>> GetGuideChannelDetails()
    {
    ...
    public void GetScheduledActionsStatus(out DateTime dateTime, out string status)
    {
      _importer.ReadScheduledActionStatus(out dateTime, out status);
    }
  }
}