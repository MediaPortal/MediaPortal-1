    ...
    private void ProcessActiveHandler_OnPowerSchedulerEvent(PowerSchedulerEventArgs args)
    {
      switch (args.EventType)
      {
        ...
        case PowerSchedulerEventType.Elapsed:
          ...
          string processes = SettingsManagement.GetValue("PowerSchedulerProcesses", string.Empty);
          ...
          if (!IsEqual(processList, setting.Get<List<string>>()))
          {
            ...
            this.LogDebug("ProcessActiveHandler: Preventing standby for processes: {0}", processes);
          }

          // Check for MP client running?
          setting = ps.Settings.GetSetting("CheckForMPClientRunning");
          _checkForMPClientRunning = SettingsManagement.GetValue("PowerSchedulerCheckForMPClientRunning", false);
          if (setting.Get<bool>() != _checkForMPClientRunning)
          {
            setting.Set<bool>(_checkForMPClientRunning);
            this.LogDebug("ProcessActiveHandler: Prevent standby when MP client is not running : {0}", _checkForMPClientRunning);
          }

          // Check if away mode should be used
          setting = ps.Settings.GetSetting("ProcessesAwayMode");
          _useAwayMode = SettingsManagement.GetValue("PowerSchedulerProcessesAwayMode", false);
          if (setting.Get<bool>() != _useAwayMode)
          {
            setting.Set<bool>(_useAwayMode);
            this.LogDebug("ProcessActiveHandler: Use away mode: {0}", _useAwayMode);
          }
