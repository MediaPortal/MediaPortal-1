...
namespace Mediaportal.TV.Server.Plugins.PowerScheduler.Handlers
{
  ...
  public class ActiveNetworkStandbyHandler : IStandbyHandler, IStandbyHandlerEx
  {
    ...
    private void NetworkMonitorHandler_OnPowerSchedulerEvent(PowerSchedulerEventArgs args)
    {
      ...
      switch (args.EventType)
      {
        case PowerSchedulerEventType.Elapsed:
          ...
          bool enabled = SettingsManagement.GetValue("PowerSchedulerNetworkMonitorEnabled", false);
          ...
          if (setting.Get<bool>() != enabled) // Setting changed
          {
            ...
            if (enabled) // Start
            {
              this.LogDebug("NetworkMonitorHandler: Network monitor started");
              StartNetworkMonitor();
            }
            else // Stop
            {
              this.LogDebug("NetworkMonitorHandler: Network monitor stopped");
              StopNetworkMonitor();
            }
          }
          ...
          if (enabled) 
          {
            ...
            _idleLimit = SettingsManagement.GetValue("PowerSchedulerNetworkMonitorIdleLimit", 2);
            if (setting.Get<Int32>() != _idleLimit)
            {
              setting.Set<Int32>(_idleLimit);
              this.LogDebug("NetworkMonitorHandler: Idle limit in KB/s: {0}", _idleLimit);
            }

            // Check if away mode should be used
            setting = ps.Settings.GetSetting("NetworkMonitorAwayMode");
            _useAwayMode = SettingsManagement.GetValue("PowerSchedulerNetworkMonitorAwayMode", false);
            if (setting.Get<bool>() != _useAwayMode)
            {
              setting.Set<bool>(_useAwayMode);
              this.LogDebug("NetworkMonitorHandler: Use away mode: {0}", _useAwayMode);
            }
          }
    ...
    private void StartNetworkMonitor()
    {
      ...
      {
        this.LogError(ex, "NetworkMonitorHandler: Exception in StartNetworkMonitor");
      }
    }
        ...
        foreach (NetworkAdapter adapter in _monitoredAdapters)
        {
          try
          {
            ...
            if ((adapter.DlSpeed >= _idleLimit) || (adapter.UlSpeed >= _idleLimit))
            {
              ...
              // this.LogDebug("NetworkMonitorHandler: dlSpeed: {0}", adapter.DlSpeed);
              // this.LogDebug("NetworkMonitorHandler: ulSpeed: {0}", adapter.UlSpeed);
              _preventers++;
            }
          }
          catch (Exception ex)
          {
            this.LogError(ex, "NetworkMonitorHandler: Exception in updating adapter {0}", adapter.Name);
          }
        }
