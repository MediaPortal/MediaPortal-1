  ...
  public class ActiveSharesStandbyHandler : IStandbyHandler, IStandbyHandlerEx
  {
    ...
    internal class ShareMonitor
    {
      ...
      internal ShareMonitor(string shareName, string clientName, string userName)
      {
        ...
        this.LogDebug("ActiveSharesHandler: Monitor connections to share '{0}' from client '{1}' by user '{2}' (Type '{3}')", _share, _client, _user, MonitoringType);
      }
    ...
    private void ProcessActiveHandler_OnPowerSchedulerEvent(PowerSchedulerEventArgs args)
    {
      switch (args.EventType)
      {
        ...
        case PowerSchedulerEventType.Elapsed:
          ...
          PowerSetting setting = ps.Settings.GetSetting("ActiveSharesEnabled");
          _enabled = SettingsManagement.GetValue("PowerSchedulerActiveSharesEnabled", false);
          if (setting.Get<bool>() != _enabled)
          {
            setting.Set<bool>(_enabled);
            this.LogDebug("ActiveSharesHandler: Monitoring active shares {0}", _enabled ? "enabled" : "disabled");
          }

          if (_enabled)
          {
            setting = ps.Settings.GetSetting("ActiveShares");
            string _connections = SettingsManagement.GetValue("PowerSchedulerActiveShares", string.Empty);
            if (setting.Get<string>() != _connections)
            {
              ...
              if (_sharesToMonitor.Count == 0)
                _sharesToMonitor.Add(new ShareMonitor(string.Empty, string.Empty, string.Empty));
            }
          }

          // Check if away mode should be used
          setting = ps.Settings.GetSetting("ActiveSharesAwayMode");
          _useAwayMode = SettingsManagement.GetValue("PowerSchedulerActiveSharesAwayMode", false);
          if (setting.Get<bool>() != _useAwayMode)
          {
            setting.Set<bool>(_useAwayMode);
            this.LogDebug("ActiveSharesHandler: Use away mode: {0}", _useAwayMode);
          }
            ...
            ("SELECT ShareName, UserName, ComputerName, NumberOfFiles  FROM Win32_ServerConnection WHERE NumberOfFiles > 0"))
          {
            ...
            foreach (ServerConnection connection in connections)
            {
              foreach (ShareMonitor shareBeingMonitored in _sharesToMonitor)
              {
                ...
                {
                  this.LogDebug("{0}: Standby is not allowed due to connection to share '{1}' from client '{2}' by user '{3}'", HandlerName, connection.ShareName, connection.ComputerName, connection.UserName);
