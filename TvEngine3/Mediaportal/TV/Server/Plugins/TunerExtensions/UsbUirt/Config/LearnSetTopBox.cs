...
namespace Mediaportal.TV.Server.Plugins.TunerExtension.UsbUirt.Config
{
  ...
  {
    private uint _usbUirtIndex = 0;

    public LearnSetTopBox(uint usbUirtIndex)
    {
      _usbUirtIndex = usbUirtIndex;
      InitializeComponent();
    }
    ...
    private void buttonLearn_Click(object sender, EventArgs e)
    {
      ...
      this.LogInfo("USB-UIRT learn STB: learn command, USB-UIRT index = {0}, command name = {1}", _usbUirtIndex, item.SubItems[0].Text);

      NotifyForm dlg = new NotifyForm(SectionSettings.MESSAGE_CAPTION, "Hold the remote control approximately 5 centimeters from the USB-UIRT, then press and hold the target button.");
      dlg.Show(this);
      dlg.WaitForDisplay();
      ...
      ThreadPool.QueueUserWorkItem(
        ...
        {
          string command;
          LearnResult result = ServiceAgents.Instance.PluginService<IUsbUirtConfigService>().Learn(_usbUirtIndex, new TimeSpan(0, 0, 10), out command);
          this.LogInfo("USB-UIRT learn STB: learn result = {0}, command = {1}", result, command);
          this.Invoke((MethodInvoker)delegate
          {
            ...
            {
              MessageBox.Show("Learning failed. Ensure the remote control batteries are okay, and try again with the remote control slightly closer to or further from the USB-UIRT.", SectionSettings.MESSAGE_CAPTION, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else if (result == LearnResult.NotOpen)
            {
              MessageBox.Show("The selected USB-UIRT's interface is currently closed.", SectionSettings.MESSAGE_CAPTION, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            ...
            {
              MessageBox.Show("The selected USB-UIRT seems to have been disconnected.", SectionSettings.MESSAGE_CAPTION, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
          });
        }
      );
    ...
    private void buttonForget_Click(object sender, EventArgs e)
    {
      foreach (ListViewItem item in listViewCommands.SelectedItems)
      {
        this.LogDebug("USB-UIRT learn STB: forget command, name = {0}", item.SubItems[0].Text);
    ...
    {
      this.LogInfo("USB-UIRT learn STB: save STB profile");
      ...
      if (!ServiceAgents.Instance.PluginService<IUsbUirtConfigService>().SaveSetTopBoxProfile(profile))
      {
        this.LogError("USB-UIRT learn STB: failed to save STB profile, name = {0}", profile.Name);
