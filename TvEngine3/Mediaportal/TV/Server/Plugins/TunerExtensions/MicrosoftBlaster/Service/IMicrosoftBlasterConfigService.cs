...
namespace Mediaportal.TV.Server.Plugins.TunerExtension.MicrosoftBlaster.Service
{
  ...
  internal interface IMicrosoftBlasterConfigService
  {
    ...
    [OperationContract]
    LearnResult Learn(string transceiverDevicePath, TimeSpan timeLimit, out string command);

    [OperationContract]
    TransmitResult Transmit(string transceiverDevicePath, TransmitPort port, string setTopBoxProfileName, string channelNumber);
  }
}