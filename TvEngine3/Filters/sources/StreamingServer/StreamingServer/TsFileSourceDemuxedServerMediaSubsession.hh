...
#ifndef _TSFILESOURCE_DEMUXED_SERVER_MEDIA_SUBSESSION_HH
#define _TSFILESOURCE_DEMUXED_SERVER_MEDIA_SUBSESSION_HH
...
class TsFileSourceDemuxedServerMediaSubsession: public OnDemandServerMediaSubsession{
public:
  static TsFileSourceDemuxedServerMediaSubsession*
  createNew(TsFileSinkDemux& demux, u_int8_t streamIdTag,
	    Boolean reuseFirstSource,
	    Boolean iFramesOnly = False, double vshPeriod = 5.0);
  // The last two parameters are relevant for video streams only

private:
  TsFileSourceDemuxedServerMediaSubsession(TsFileSinkDemux& demux,
				       u_int8_t streamIdTag, Boolean reuseFirstSource,
				       Boolean iFramesOnly, double vshPeriod);
      // called only by createNew();
  virtual ~TsFileSourceDemuxedServerMediaSubsession();
