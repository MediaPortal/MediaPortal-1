...
int CSectionDecoder::AddToSection(byte* tsPacket, int index, int sectionLen)
{
	int newstart=-1;
  int len=-1;
  if (index+sectionLen < 184)
  ...
  else
  {
	...
	//LogDebug("newstart: %d len: %d",newstart,len);
	if (len<1)
		return 188;
...
void CSectionDecoder::OnTsPacket(CTsHeader& header,byte* tsPacket)
{
			...
			if (m_section.BufferPos != 0)
      {
		...
		while (start < 188)
		{
			...
			if (m_section.BufferPos==0)
			{
				...
				start=AddToSection(tsPacket,start,m_section.SectionLength-m_section.BufferPos);
			if (m_section.SectionComplete())
			{
				OnNewSection(m_section);
	      if (m_pCallback!=NULL)
  ...
  //LogDebug("got packet with payload. BufferPos: %d SectionLength: %d",m_section.BufferPos,m_section.SectionLength);
}
/*	if (m_bLog)
