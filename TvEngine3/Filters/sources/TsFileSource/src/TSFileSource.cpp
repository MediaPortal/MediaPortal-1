...
STDMETHODIMP CTSFileSourceFilter::GetBasePCRPosition(REFERENCE_TIME *pos)
{
	if(!pos)
		return E_INVALIDARG;

	CAutoLock lock(&m_Lock);
	*pos = m_pPin->getBasePCRPosition();

	return NOERROR;
}

STDMETHODIMP CTSFileSourceFilter::GetStartPCRPosition(REFERENCE_TIME *pos)
{
	if(!pos)
		return E_INVALIDARG;

	CAutoLock lock(&m_Lock);
	*pos = m_pPin->getStartPCRPosition();
