...
namespace TvLibrary.Implementations.DVB
{
  ...
  public class ProfRed : IDiSEqCController
  {
    ...
    public void SendDiseqCommand(ScanParameters parameters, DVBSChannel channel)
    {
      ...
      Marshal.WriteInt32(_ptrDiseqc, 149, 4);//send_message_length
      //Marshal.WriteInt32(_ptrDiseqc, 162, 0);//receive_message_length
      //Marshal.WriteInt32(_ptrDiseqc, 168, 3);//amplitude_attenuation
      if (antennaNr == 1)//for simple diseqc switches (i.e. 22KHz tone burst)
      {
          Marshal.WriteByte(_ptrDiseqc, 162, (int)BurstModulationType.TONE_BURST_UNMODULATED);//
      }
      else
      {
          Marshal.WriteByte(_ptrDiseqc, 162, (int)BurstModulationType.TONE_BURST_MODULATED);//default to tone_burst_modulated
      }
      Marshal.WriteByte(_ptrDiseqc, 166, 3);//default
      //Marshal.WriteByte(_ptrDiseqc, 176, (int)DisEqcVersion.DISEQC_VER_1X);//default
      //Marshal.WriteByte(_ptrDiseqc, 166, RxMode.RXMODE_NOREPLY);//default
      Marshal.WriteByte(_ptrDiseqc, 178, 1);//last_message TRUE */
      
      string txt = "";
      for (int i = 0; i < 4; ++i)
          txt += String.Format("0x{0:X} ", Marshal.ReadByte(_ptrDiseqc, i));
      for (int i = 149; i < 188; i = (i + 4))
