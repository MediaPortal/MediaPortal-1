...
namespace TvThumbnails
{
  public class Thumbnailer
  {
    ...
    private const string _extractApp = "ffmpeg.exe";

    private const int _previewColumns = 2;

    private const int _previewRows = 2;

    private const bool _leaveShareThumb = false;
    ...
    public Thumbnailer()
    {
      ...
      _extractorPath = currentPathInfo.DirectoryName + "\\" + _extractApp;
    }

    [MethodImpl(MethodImplOptions.Synchronized)]
    public bool CreateVideoThumb(string aVideoPath, string aThumbPath, bool aOmitCredits)
    {
    ...
    private bool CreateThumbnail(Image aDrawingImage, string aThumbTargetPath, int aThumbWidth, int aThumbHeight,
    ...
    private bool SaveThumbnail(string aThumbTargetPath, Image myImage)
    {
      try
      {
    ...
    /// <param name="aFastMode">Use low quality resizing without interpolation suitable for small thumbnails</param>
    /// <returns>Whether the thumb has been successfully created</returns>
    private bool CreateThumbnail(string aInputFilename, string aThumbTargetPath, int iMaxWidth, int iMaxHeight,
                                       int iRotate)
    {
    ...
    private void KillProcess(string aProcessName)
    {
      try
      {
    ...
    private ProcessFailedConditions GetMtnConditions()
    {
    ...
    [MethodImpl(MethodImplOptions.Synchronized)]
    private bool StartProcess(string aAppName, string aArguments, string aWorkingDir, int aExpectedTimeoutMs,
