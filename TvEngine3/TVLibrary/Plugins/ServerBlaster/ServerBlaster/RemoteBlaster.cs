...
namespace TVEngine.Devices
{
	...
	sealed class Blaster : Device
	{
        ...
        public static void Send(int blasterPort, byte[] packet, int deviceType, int deviceSpeed)
        {
            //Log.Write("Blaster.Send: Checking if  Singleton is null.");
            if (_deviceSingleton == null) return; 
            //Log.Write("Blaster.Send: Checking if Device Singleton Stream is null."); 
            if (_deviceSingleton._deviceStream == null) return;
            //Log.Write("Blaster.Send: Done Initial Checking. Will check arguments."); 
            if (blasterPort < 0 || blasterPort > 2) throw new ArgumentException("blasterPort must be 1, 2 or 0 (both)");
            //Log.Write("Blaster.Send: BlasterPort Done."); 
            if (deviceType < 0 || deviceType > 1) throw new ArgumentException("blasterType must be 1, or 0 (0 - MS, 1- SMK)");
            //Log.Write("Blaster.Send: BlasterType Done."); 
            ...
            //Log.Write("Blaster.Send: BlasterSpeed Done."); 
            ...
            //Log.Write("Blaster.Send: Type {0}, Speed {1}", deviceType, s);
            //Log.Write("Blaster.Send: Port {0}", blasterPort == 0 ? "1 & 2" : blasterPort.ToString());
            //Log.Write("Seding packets");
            _deviceSingleton._deviceStream.Write(packetSpeed[s], 0, packetSpeed[s].Length);
            //Log.Write("Blaster.Send: Wrote speed packets");
            _deviceSingleton._deviceStream.Write(packetPorts[p], 0, packetPorts[p].Length);
            //Log.Write("Blaster.Send: Wrote port packets");
            _deviceSingleton._deviceStream.Write(packet, 0, packet.Length);
            //Log.Write("Blaster.Send: Wrote channel data  packets");
            _deviceSingleton._deviceStream.Flush();
            //Log.Write("Blaster.Send: Flushed");
        }
