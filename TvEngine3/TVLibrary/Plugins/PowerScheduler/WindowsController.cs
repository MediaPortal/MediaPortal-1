...
namespace TvEngine.PowerScheduler
{
  ...
  public class WindowsController
  {
    ...
    {
      Log.Debug(LogType.PS, "WindowsController: Setting ExitWindows to {0}.{1}", handler.Target, handler.Method);
    ...
    protected static void ExitWindowsDefaultThread(object _data)
    {
      ...
      Log.Debug(LogType.PS, "WindowsController: Performing ExitWindows {0}, force: {1}", how, force);
      bool res;
      switch (how)
      {
      ...
      Log.Debug(LogType.PS, "WindowsController: ExitWindows performed, result: {0}", res);
      if (env.after != null)
    ...
    {
      Log.Info(LogType.PS, "--Exit Windows - ", how.ToString() + ", " + force.ToString());
