...
namespace MyWeather
{
    ...
    public class WeatherDataModel
    {
        ...
        bool ParseCurrentCondition(XmlNode xmlElement)
        {
            ...
            CurCondition.Icon = String.Format(@"{0}.png", GetInteger(xmlElement, "icon"));
        ...
        bool ParseDayForeCast(XmlNode xmlElement)
        {
            ...
            for (int i = 0; i < NUM_DAYS; i++)
            {
                if (element != null)
                {
                    ...
                    if (dayElement != null)
                    {
                        DaysForeCast[i].iconImageNameLow = String.Format("{0}.png", GetInteger(dayElement, "icon"));
                        DaysForeCast[i].iconImageNameHigh = String.Format("{0}.png", GetInteger(dayElement, "icon"));
