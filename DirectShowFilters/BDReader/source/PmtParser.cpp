  ...
  try
    ...
    while (len1 > 0)
    {
      ...
      if(stream_type==SERVICE_TYPE_VIDEO_MPEG1 
        ...
        || stream_type==SERVICE_TYPE_VIDEO_VC1 )
      {
        VideoPid pid;
      ...
      if(stream_type==SERVICE_TYPE_AUDIO_MPEG1 || 
        ...
        //stream_type==SERVICE_TYPE_AUDIO_LPCM ||  // no sound?
        stream_type==SERVICE_TYPE_AUDIO_DTS ||
        stream_type==SERVICE_TYPE_AUDIO_MLP ||  // not supported yet
        stream_type==SERVICE_TYPE_AUDIO_DD_PLUS ||
        stream_type==SERVICE_TYPE_AUDIO_DTS_HD ||
        stream_type==SERVICE_TYPE_AUDIO_TS_HD_XLL )
      {				  
        AudioPid pid;
        pid.Pid=elementary_PID;
      ...
      if (stream_type==SERVICE_TYPE_PGS_SUBTITLES)
      {
        SubtitlePid pid;
        pid.Pid=elementary_PID;
        pid.SubtitleServiceType=SERVICE_TYPE_PGS_SUBTITLES;
