...
#ifndef __CONTINUOUS_STREAM_RANGE_COLLECTION_DEFINED
#define __CONTINUOUS_STREAM_RANGE_COLLECTION_DEFINED
...
class CContinuousStreamRangeCollection : public CCollection<CContinuousStreamRange>
{
public:
  CContinuousStreamRangeCollection(HRESULT *result);
  virtual ~CContinuousStreamRangeCollection(void);
  ...
  bool AddContinuousStreamRange(int64_t filterStartPosition, int64_t protocolStartPosition, int64_t streamLength, uint32_t flags);
...
protected:
  ...
  // adds continuous stream range to collection
  // do not call this method directly
  // @param item : the reference to continuous stream range to add
  // @return : true if successful, false otherwise
  virtual bool Add(CContinuousStreamRange *item);

  // clones specified item
  // @param item : the item to clone
  // @return : deep clone of item or NULL if not implemented
  virtual CContinuousStreamRange *Clone(CContinuousStreamRange *item);
};
