...
CMovieFragmentBox::CMovieFragmentBox(HRESULT *result)
  : CBox(result)
{
  this->type = NULL;
...
bool CMovieFragmentBox::ParseInternal(const unsigned char *buffer, uint32_t length, bool processAdditionalBoxes)
{
  if (__super::ParseInternal(buffer, length, false))
  {
    ...
    this->flags |= (wcscmp(this->type, MOVIE_FRAGMENT_BOX_TYPE) == 0) ? BOX_FLAG_PARSED : BOX_FLAG_NONE;

    if (this->IsSetFlags(BOX_FLAG_PARSED))
    {
      // box is movie fragment box, parse all values
