...
CMediaInformationBox::CMediaInformationBox(HRESULT *result, uint32_t handlerType)
  : CBox(result)
{
  this->type = NULL;
...
bool CMediaInformationBox::ParseInternal(const unsigned char *buffer, uint32_t length, bool processAdditionalBoxes)
{
  if (__super::ParseInternal(buffer, length, false))
  {
    ...
    this->flags |= (wcscmp(this->type, MEDIA_INFORMATION_BOX_TYPE) == 0) ? BOX_FLAG_PARSED : BOX_FLAG_NONE;

    if (this->IsSetFlags(BOX_FLAG_PARSED))
    {
      // box is media information box, parse all values
