...
CNullMediaHeaderBox::CNullMediaHeaderBox(HRESULT *result)
  : CFullBox(result)
{
  this->type = NULL;
...
bool CNullMediaHeaderBox::ParseInternal(const unsigned char *buffer, uint32_t length, bool processAdditionalBoxes)
{
  if (__super::ParseInternal(buffer, length, false))
  {
    ...
    this->flags |= (wcscmp(this->type, NULL_MEDIA_HEADER_BOX_TYPE) == 0) ? BOX_FLAG_PARSED : BOX_FLAG_NONE;

    if (this->IsSetFlags(BOX_FLAG_PARSED))
    {
      // box is null media header box, parse all values
