...
CTag *CTagFactory::CreateTag(HRESULT *result, unsigned int version, CGeneralTag *generalTag)
{
  ...
  if ((result != NULL) && (SUCCEEDED(*result)))
  {
    ...
    if (SUCCEEDED(*result))
    {
      ...
      CHECK_CONDITION_EXECUTE(SUCCEEDED(*result), *result = temp->ParseGeneralTag(generalTag, version));
      ...
      if (SUCCEEDED(*result))
      {
        ...
        CREATE_SPECIFIC_TAG(temp, TAG_START, CStartTag, (*result), tag, version);
