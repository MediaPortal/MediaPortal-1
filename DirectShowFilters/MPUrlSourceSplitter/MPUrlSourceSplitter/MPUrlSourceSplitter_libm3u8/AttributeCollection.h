...
public:
  ...
  virtual ~CAttributeCollection(void);

  /* get methods */

  // get the attribute from collection with specified index
  // @param index : the index of attribute to find
  // @return : the reference to attribute or NULL if not found
  virtual CAttribute *GetAttribute(unsigned int index);

  // get the attribute from collection with specified name
  // @param name : the name of attribute to find
  // @param invariant : specifies if attribute name shoud be find with invariant casing
  // @return : the reference to attribute or NULL if not found
  virtual CAttribute *GetAttribute(const wchar_t *name, bool invariant);
...
protected:
  ...
  virtual const wchar_t *GetKey(CAttribute *item);

  // clones specified item
  // @param item : the item to clone
  // @return : deep clone of item or NULL if not implemented
  virtual CAttribute *Clone(CAttribute *item);
};
