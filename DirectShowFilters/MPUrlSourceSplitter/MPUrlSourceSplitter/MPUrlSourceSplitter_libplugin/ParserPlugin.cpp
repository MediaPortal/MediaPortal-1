...
HRESULT CParserPlugin::SetConnectionParameters(const CParameterCollection *parameters)
{
  ...
  if (SUCCEEDED(result))
  {
    this->flags &= ~(PARSER_PLUGIN_FLAG_DUMP_INPUT_DATA | PARSER_PLUGIN_FLAG_DUMP_OUTPUT_DATA | PLUGIN_FLAG_SPLITTER | PLUGIN_FLAG_IPTV | PLUGIN_FLAG_DOWNLOADING);
    ...
    this->flags |= this->connectionParameters->GetValue(PARAMETER_NAME_DOWNLOAD_FILE_NAME, true, NULL) ? PLUGIN_FLAG_DOWNLOADING : PROTOCOL_PLUGIN_FLAG_NONE;
