...
CParserPlugin::ParserResult CMPUrlSourceSplitter_Parser_F4M::GetParserResult(void)
{
  if (this->parserResult == Pending)
  {
    HRESULT result = S_OK;
    
    if (this->protocolHoster->GetStreamCount() == 1)
    {
      ...
      if (SUCCEEDED(result))
      {
        ...
        while (SUCCEEDED(result) && (this->parserResult == Pending) && (!receivedSameLength))
        {
          package->Clear();
          ...
          CHECK_HRESULT_EXECUTE(result, this->protocolHoster->ProcessStreamPackage(package));
            ...
            if (response != NULL)
            {
              ...
              if (!receivedSameLength)
              {
                ...
                if (SUCCEEDED(result))
                {
                  ...
                  CF4MManifest *manifest = new CF4MManifest(&result);
...
unsigned int CMPUrlSourceSplitter_Parser_F4M::GetReceiveDataTimeout(void)
{
