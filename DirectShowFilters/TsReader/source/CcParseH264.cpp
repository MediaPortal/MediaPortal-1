...
CCparse::CCparse()
{
  //cc_data = new unsigned char[1024];
...
int CCparse::EBSPtoRBSP(unsigned char *streamBuffer, int end_bytepos, int begin_bytepos)
{
  int i, j, count;
  count = 0;
...
unsigned char * CCparse::remove_03emu(unsigned char *from, unsigned char *to)
{
	int num=to-from;
...
void CCparse::sei_rbsp (unsigned char *seibuf, int Length)
{
  unsigned char *tbuf = seibuf;
  unsigned char *seiend = seibuf + Length;
	//LogDebug ("CCparse: sei_rbsp() - Start1, seibuf: %d, seiend: %d, Length: %d, endbyte: %x", seibuf, seiend, Length, *(seiend-1));
	seiend = remove_03emu(seibuf, seiend); // Remove 'emulation_prevention_three_byte' added bytes
	//LogDebug ("CCparse: sei_rbsp() - Start2, seibuf: %d, seiend: %d, endbyte: %x", seibuf, seiend, *(seiend-1));
  ...
  {
      if(*tbuf != 0x80)
          LogDebug("CCparse: sei_rbsp() - Strange rbsp_trailing_bits value: %02X",*tbuf);
  }
  else
	{
		// TODO: This really really looks bad
		LogDebug ("CCparse: sei_rbsp() - WARNING: Unexpected SEI unit length...trying to continue. seibuf: %d, seiend: %d, Length: %d", seibuf, seiend, Length);
		//LogDebug ("CCparse: sei_rbsp() - Failed block");
...
unsigned char *CCparse::sei_message (unsigned char *seibuf, unsigned char *seiend)
{
    int payloadType = 0;
	while (*seibuf==0xff)
	{
    ...
    LogDebug ("CCparse: sei_message() - Payload type: %d size: %d - seibuf: %d, seiend: %d ", payloadType, payloadSize, seibuf, seiend);
	if(seibuf > seiend )
	{
		...
		if (payloadType==4)
		{
			LogDebug ("CCparse: sei_message() - Warning: Subtitles payload seems incorrect (too long), continuing but it doesn't look good..");
		}
		else
		{
			LogDebug ("CCparse: sei_message() - Warning: Non-subtitles payload seems incorrect (too long), continuing but it doesn't look good..");
		}
	}
...
void CCparse::copy_ccdata_to_buffer (char *source, int new_cc_count)
{
	ccblocks_in_avc_total++;
	if (cc_buffer_saved==0)
	{
		LogDebug ("CCparse: copy_ccdata_to_buffer() - Warning: Probably loss of CC data, unsaved buffer being rewritten");
		ccblocks_in_avc_lost++;
	}
	memcpy(cc_data+cc_count*3, source, new_cc_count*3+1);
	cc_count+=new_cc_count;byte
	cc_buffer_saved=0;
}


void CCparse::user_data_registered_itu_t_t35 (unsigned char *userbuf, unsigned char *userend)
{
    ...
    {
        LogDebug ("CCparse: udr_itu_t_t35() - Not a supported user data SEI, itu_t_35_country_code: %02x", itu_t_t35_country_code);
        return;
    }
	...
	switch (itu_t_35_provider_code)
	{
		case 0x0031: // ANSI/SCTE 128
			LogDebug ("CCparse: udr_itu_t_t35() - Caption block in ANSI/SCTE 128...");
			if (*tbuf==0x47 && *(tbuf+1)==0x41 && *(tbuf+2)==0x39 && *(tbuf+3)==0x34) // ATSC1_data() - GA94
			{
				LogDebug ("CCparse: udr_itu_t_t35() - ATSC1_data()...");
				tbuf+=4;
				...
				{
					case 0x03:
						LogDebug ("CCparse: udr_itu_t_t35() - cc_data (finally)!");
...
//						{
//							LogDebug ("CCparse: udr_itu_t_t35() - process_cc_data_flag == 0, skipping this caption block.");
//							break;
//						}
...
//						{
//							LogDebug ("CCparse: udr_itu_t_t35() - Fixed bit should be 1, but it's 0 - skipping this caption block.");
//							break;
//						}
//						if (*tbuf & 0x20) // Third bit must be 0
//						{
//							LogDebug ("CCparse: udr_itu_t_t35() - Fixed bit should be 0, but it's 1 - skipping this caption block.");
//							break;
//						}
...
//						if (*tbuf!=0xFF)
//						{
//							LogDebug ("CCparse: udr_itu_t_t35() - Fixed value should be 0xFF, but it's %02X - skipping this caption block.", *tbuf);
//						}
...
//							"Syntax problem: user_data_len != cc_count*3+3.");
						...
						{
							LogDebug ("CCparse: udr_itu_t_t35() - FATAL - Syntax problem: Too many caption blocks.");
							return;
						}
						if (cc_tmpdata[local_cc_count*3]!=0xFF)
						{
							LogDebug ("CCparse: udr_itu_t_t35() - FATAL - Syntax problem: Final 0xFF marker missing.");
							return;
						}
						...
						if (local_cc_count*3+1 > cc_databufsize)
						{
							...
							if (!cc_data)
							{
								LogDebug ("CCparse: udr_itu_t_t35() - FATAL - Out of memory");
								return;
						  }
							cc_databufsize = (long) cc_count*6+1;
						}
						// Copy new cc data into cc_data
			      LogDebug ("CCparse: udr_itu_t_t35() - copy new 'code 49' CC data into cc_data, length: %d", local_cc_count);
						copy_ccdata_to_buffer ((char *) cc_tmpdata, local_cc_count);
						break;
					case 0x06:
						LogDebug ("CCparse: udr_itu_t_t35() - bar_data (unsupported for now)");
						break;
					default:
						LogDebug ("CCparse: udr_itu_t_t35() - SCTE/ATSC reserved.");
				}
			...
			break;
		case 0x002F:
            LogDebug ("CCparse: udr_itu_t_t35() - ATSC1_data() - provider_code = 0x002F");

			user_data_type_code = *((byte*)tbuf);
			if(user_data_type_code != 0x03)
			{
				LogDebug ("CCparse: udr_itu_t_t35() - Not supported  user_data_type_code: %02x\n",
				   user_data_type_code);
				return;
			}
			tbuf++;
			...
			{
				LogDebug ("CCparse: udr_itu_t_t35() - FATAL - Syntax problem: user_data_len != cc_count*3+3.");
				return;
			}
      ...
      // Copy new cc data into cc_data - replace command below.
			LogDebug ("CCparse: udr_itu_t_t35() - copy new 'code 47' CC data into cc_data, length: %d", local_cc_count);
      copy_ccdata_to_buffer ((char *) cc_tmpdata, local_cc_count);

			//dump(tbuf,user_data_len-1,0);
			break;
		default:
			LogDebug ("CCparse: udr_itu_t_t35() - Not a supported user data SEI, itu_t_35_provider_code: %04x\n", itu_t_35_provider_code);
			break;
	}
}
