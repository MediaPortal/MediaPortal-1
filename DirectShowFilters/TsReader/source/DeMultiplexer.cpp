  ...
  , m_filter(filter)
{
  ...
  m_reader = nullptr;
  pTeletextEventCallback = nullptr;
  pSubUpdateCallback = nullptr;
  pTeletextPacketCallback = nullptr;
  pTeletextServiceInfoCallback = nullptr;
  m_iAudioReadCount = 0;
...
bool CDeMultiplexer::GetAudioStreamType(int stream, CMediaType& pmt, int iPosition)
{
  ...
  if (stream < 0 || stream >= (int)m_audioStreams.size() || m_mpegPesParser == nullptr)
  {
    pmt.InitMediaType();
...
{
  if (m_pids.videoPids.size() != 0 && m_mpegPesParser != nullptr)
  {
...
{
  if (m_bFlushDelgNow || m_bFlushRunning || m_bStarting) return nullptr; //Flush pending or Start() active
  if (IsAudioChanging()) return nullptr; //Waiting for MP player to do something....
  ...
  if (m_currentSubtitlePid == 0) return nullptr;
  if (m_bEndOfFile) return nullptr;
...
int CDeMultiplexer::ReadFromFile(ULONG lDataLength)
{
  ...
  if (m_filter.IsSeeking() || m_bFlushDelgNow || m_bFlushRunning || (m_reader == nullptr))
  {
    return -1;
  }
              ...
              if ((*(INT16 *)ps & 0xE0FF) == 0xE056 && len > 6) //Syncword bits==0x2B7 (first 11 bits)
              {
                ...
                if (*(INT16 *)(ps + 3) == 0x0020) //Preamble to AudioSpecificConfig() data
                {
                  ...
                  if (hFreq>2 && hFreq<11 && hChannels<7 && hChannels>0 && (hObjectType==2 || hObjectType==5 || hObjectType==29)) //Sanity checks...
                  {
                    ...
                    {
                      CAutoLock plock(&m_mpegPesParser->m_sectionAudioPmt);
                      m_mpegPesParser->OnAudioPacket(nullptr, 0, m_AudioStreamType, m_iAudioStream, true); //Generate default info
...
void CDeMultiplexer::FillTeletext(CTsHeader& header, byte* tsPacket)
{
  ...
  if (pTeletextEventCallback != nullptr)
  {
  ...
  if (pTeletextPacketCallback != nullptr)
  {
...
void CDeMultiplexer::OnNewChannel(CChannelInfo& info)
{
  ...
  if (pSubUpdateCallback != nullptr)
  {
    int bitmap_index = -1;
    (*pSubUpdateCallback)((int)m_subtitleStreams.size(), ((int)m_subtitleStreams.size() > 0 ? &m_subtitleStreams[0] : nullptr), &bitmap_index);
    if (bitmap_index >= 0)
    {
