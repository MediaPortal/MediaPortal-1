...
HRESULT CFilterOutPin::CompleteConnect(IPin *pReceivePin)
{
	LogDebug("pin:CompleteConnect()");
		HRESULT hr;
	PIN_INFO inf;
	hr=pReceivePin->QueryPinInfo(&inf);
	CLSID id;
	hr=inf.pFilter->GetClassID(&id);
...
HRESULT CFilterOutPin::ChangeStart()
{
	...
	{
		//IPin *demuxIn;
		//HRESULT hr=m_pMPTSFilter->m_pPin->ConnectedTo(&demuxIn);
		//if(SUCCEEDED(hr))
		//{
		//	hr=demuxIn->BeginFlush();
		//	SeekIFrame();
		//	hr=demuxIn->EndFlush();
		//	hr=demuxIn->NewSegment(m_rtStart,m_rtStop,m_dRateSeeking);
		//	hr=demuxIn->Release();
		//}
