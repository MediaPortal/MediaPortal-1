...
class TsClientTrickPlayState {
public:
  TsClientTrickPlayState(MPEG2TransportStreamIndexFile* indexFile);
...
TsFileServerMediaSubsession*
TsFileServerMediaSubsession::createNew(UsageEnvironment& env,
						   char const* fileName,
  ...
  return new TsFileServerMediaSubsession(env, fileName, indexFile,
						     reuseFirstSource);
}
...
//    fDuration = fIndexFile->getPlayingDuration();
  ...
  LogDebug("TsFileServerMediaSubsession:ctor");
}

TsFileServerMediaSubsession
::~TsFileServerMediaSubsession() 
{
  LogDebug("TsFileServerMediaSubsession:dtor");
  if (fIndexFile != NULL) { // we support 'trick play'
    Medium::close(fIndexFile);

    // Clean out the client session hash table:
    while (1) {
      TsClientTrickPlayState* client
	= (TsClientTrickPlayState*)(fClientSessionHashTable->RemoveNext());
...
void TsFileServerMediaSubsession
::startStream(unsigned clientSessionId, void* streamToken, TaskFunc* rtcpRRHandler,
	      void* rtcpRRHandlerClientData, unsigned short& rtpSeqNum,
	      unsigned& rtpTimestamp) {
  if (fIndexFile != NULL) { // we support 'trick play'
    TsClientTrickPlayState* client = lookupClient(clientSessionId);
...
FramedSource* TsFileServerMediaSubsession
::createNewStreamSource(unsigned clientSessionId, unsigned& estBitrate) {
  ...
  if (fIndexFile != NULL) { // we support 'trick play'
    ...
    TsClientTrickPlayState* client = lookupClient(clientSessionId);
    if (client == NULL) {
      client = new TsClientTrickPlayState(fIndexFile);
...
RTPSink* TsFileServerMediaSubsession
...
void TsFileServerMediaSubsession::testScaleFactor(float& scale) {
...
float TsFileServerMediaSubsession::duration() const
{
...
TsClientTrickPlayState* TsFileServerMediaSubsession
::lookupClient(unsigned clientSessionId) {
  return (TsClientTrickPlayState*)(fClientSessionHashTable->Lookup((char const*)clientSessionId));
}


////////// ClientTrickPlayState implementation //////////

TsClientTrickPlayState::TsClientTrickPlayState(MPEG2TransportStreamIndexFile* indexFile)
...
void TsClientTrickPlayState::updateStateFromNPT(double npt) {
  fNPT = (float) npt;
...
void TsClientTrickPlayState::updateStateOnScaleChange() {
  fScale = fNextScale;
...
void TsClientTrickPlayState::updateStateOnPlayChange(Boolean reverseToPreviousVSH) {
  updateTSRecordNum();
  if (fTrickPlaySource == NULL) {
...
void TsClientTrickPlayState::setSource(MPEG2TransportStreamFramer* framer) {
  fFramer = framer;
  fOriginalTransportStreamSource = (ByteStreamFileSource*)(framer->inputSource());
}

void TsClientTrickPlayState::updateTSRecordNum(){
  if (fFramer != NULL) fTSRecordNum += fFramer->tsPacketCount();
}

void TsClientTrickPlayState::reseekOriginalTransportStreamSource() {
...
void TsFileServerMediaSubsession::seekStreamSource(FramedSource* inputSource, double seekNPT)
{
  ...
  if (seekNPT>(fileDuration-0.5)) seekNPT=(fileDuration-0.5);
  if (seekNPT <0) seekNPT=0;
