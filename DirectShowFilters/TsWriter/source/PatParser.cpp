...
void CPatParser::AnalyzePidInfo(vector<PidInfo2> pidInfo,int &hasVideo, int &hasAudio)
{
  ...
  while (it!=pidInfo.end())
  {
    ...
    //end of workaround
    if (info.logicalStreamType==SERVICE_TYPE_VIDEO_MPEG1 || info.logicalStreamType==SERVICE_TYPE_VIDEO_MPEG2 || info.logicalStreamType==SERVICE_TYPE_DCII_VIDEO_MPEG2 || info.logicalStreamType==SERVICE_TYPE_VIDEO_MPEG4 || info.logicalStreamType==SERVICE_TYPE_VIDEO_H264)
      hasVideo=1;
