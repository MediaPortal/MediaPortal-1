...
void CPatParser::AnalyzePidInfo(vector<PidInfo2> pidInfo,int &hasVideo, int &hasAudio)
{
    ...
    if (info.streamType==SERVICE_TYPE_DVB_SUBTITLES2 && info.logicalStreamType==0xffffffff && info.elementaryPid==0xd49)
    {
      ...
      info.logicalStreamType=SERVICE_TYPE_VIDEO_HEVC;
      LogDebug("AnalyzePidInfo: set ITV HD video stream to HEVC");
    }
    //end of workaround
    if (info.logicalStreamType == SERVICE_TYPE_VIDEO_MPEG1 || info.logicalStreamType == SERVICE_TYPE_VIDEO_MPEG2 || info.logicalStreamType == SERVICE_TYPE_VIDEO_MPEG4 || info.logicalStreamType == SERVICE_TYPE_VIDEO_H264 || info.logicalStreamType == SERVICE_TYPE_VIDEO_HEVC || info.logicalStreamType == SERVICE_TYPE_VIDEO_MPEG2_DCII)
			hasVideo=1;
