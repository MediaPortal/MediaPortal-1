...
void CSdtParser::DVB_GetService(BYTE *b,CChannelInfo& info)
{
	...
	pointer = 4;
	DvbTextToString(b+pointer,service_provider_name_length,info.ProviderName,sizeof(info.ProviderName));
	pointer += service_provider_name_length;
	service_name_length = b[pointer];
	pointer += 1;
	DvbTextToString(b+pointer, service_name_length,info.ServiceName,sizeof(info.ServiceName));
}
