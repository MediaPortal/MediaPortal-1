...
namespace MsSqlLibrary
{
    public class MLItemDataSource : IMLItemDataSource
    {
        ...
        {
            IMeedioItem Values = ItemDAL.GetGroupByItems(sqlProvider.CreateCommand(true),Tags, "", Convert.ToString(Tags[TagName]), false);
            if (Tags != null && Tags.Count > 0)
                return (string[])(Tags.Keys as ArrayList).ToArray(typeof(string));
            return null;
        }
        ...
        private int CreateNewTags()
        {
            ...
            if(max >= 0)
            {
                ...
                {	//Add  free tag to tags table
                    ...
                    //Add index for new colums
                    ItemDAL.CreateIndex(sqlProvider.CreateCommand(), CurrentTag);
                }
                if (EndTransact)
                    EndUpdate();
            }
        ...
        {
            return TagDAL.RenameTag(sqlProvider.CreateCommand(), OldTagName, NewTagName);
        }

        public bool DeleteTag(string TagName)
        {
                //free the tag so it is unused
            return (TagDAL.FreeTag(sqlProvider.CreateCommand(), TagName))
        ...
        {
            return ItemDAL.AddOrUpdateItem(sqlProvider.CreateCommand(UpdateItemSql(Item)), Item);
        }

        public bool DeleteItem(IMLItem Item)
        {
            return ItemDAL.DeleteItem(sqlProvider.CreateCommand(), Item.ID);
        }

        public bool DeleteAllItems()
        {
            return ItemDAL.DeleteAllItems(sqlProvider.CreateCommand());
        }
        ...
        {
            return ItemDAL.GetItemCount(sqlProvider.CreateCommand());
        }
        ...
        public IMLItemList CustomSearch(string Filter, string GroupBy, string GroupFunc, string OrderBy, string OrderType, bool Asc)
        {
            ...
            if (UseListTags)
            {
                IMeedioItem ListTags = ItemDAL.GetGroupByItems(sqlProvider.CreateCommand(true),Tags, Filter, GroupByCol, GroupFunc == "index");
        ...
        public bool UpdateDataSet(DataSet ds)
        {
            ...
            //commit transaction
            bool Transact = BeginUpdate();
            rtn = ItemDAL.UpdateDataSet(da,CommandBld,ds,Tags);
            if (Transact)
                EndUpdate();
            return rtn;
        }
        ...
        public bool BeginUpdate()
        {
            ...
            //try
            //{
            //    if (RWConnection.State == ConnectionState.Closed)
            //        RWConnection.Open();
            //    Transaction = (SQLiteTransaction)RWConnection.BeginTransaction();
            //    InProgress = true;
            //}
            //catch (Exception e)
            //{
        ...
        //public void CustomUpdate(string Filter, IMeedioItem Item)
        //{
        //    string SetAll = "";
        ...
        public void Refresh()
        {
            ...
            //Tags = (new MeedioItem()) as IMeedioItem;
            //LoadTags();
            //CopyToMem();
        }

        //private bool GetTableList(ref IMeedioItemList TableList)
        //{
        //    TableList = QueryTable(SQLStatements.SelectTables);
        //    return (TableList != null);
        //}

        //private bool GetIndexList(ref IMeedioItemList IndexList)
        //{
        ...
        private void CopyToMem()
        {
            ...
            //IMeedioItemList TableList = null;
            //IMeedioItemList IndexList = null;
