...
* Afrow UK            Jan 18 2011  Unicode build
...
NSISFUNC(exec)
{
  ...
  ptp->dosExec = (TCHAR*)GlobalAlloc(GPTR, string_size);
  ptp->pipeWrite = (TCHAR*)GlobalAlloc(GPTR, string_size);
  ptp->logFile = (TCHAR*)GlobalAlloc(GPTR, string_size);
