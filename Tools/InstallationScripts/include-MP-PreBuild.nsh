...
!system '"$%ProgramFiles(x86)%\MSBuild\14.0\Bin\MSBUILD.exe" /m "${git_ROOT}\Build\RestorePackages.targets"' = 0

!ifdef BUILD_MediaPortal
!system 'ant -f ${LibblurayJAR} -Dsrc_awt=:java-j2se' = 0
!insertmacro PrepareBuildReport DirectShowFilters
!ifdef x64Environment
!system '"C:\Program Files (x86)\MSBuild\14.0\Bin\MSBUILD.exe" ${logger} /m /target:rebuild /property:Configuration=Release "${git_DirectShowFilters}\Filters.sln"' = 0
!else
!system '"C:\Program Files\MSBuild\14.0\Bin\MSBUILD.exe" ${logger} /m /target:rebuild /property:Configuration=Release "${git_DirectShowFilters}\Filters.sln"' = 0
!endif
!insertmacro FinalizeBuildReport
!insertmacro PrepareBuildReport MediaPortal
!system '"$%ProgramFiles(x86)%\MSBuild\14.0\Bin\MSBUILD.exe" ${logger} /m /target:Rebuild /property:Configuration=Release;Platform=x86 "${git_MP}\MediaPortal.sln"' = 0
!insertmacro FinalizeBuildReport
!endif

!ifdef BUILD_TVServer
!insertmacro PrepareBuildReport TvLibrary
!system '"$%ProgramFiles(x86)%\MSBuild\14.0\Bin\MSBUILD.exe" ${logger} /m /target:Rebuild /property:Configuration=Release;Platform=x86 "${git_TVServer}\TvLibrary.sln"' = 0
!insertmacro FinalizeBuildReport
!insertmacro PrepareBuildReport TvPlugin
!system '"$%ProgramFiles(x86)%\MSBuild\14.0\Bin\MSBUILD.exe" ${logger} /m /target:Rebuild /property:Configuration=Release;Platform=x86 "${git_TVServer}\TvPlugin\TvPlugin.sln"' = 0
!insertmacro FinalizeBuildReport
!endif
...
!system '"$%ProgramFiles(x86)%\MSBuild\14.0\Bin\MSBUILD.exe" ${logger} /m /p:ALToolPath="${ALToolPath}" /target:Rebuild /property:Configuration=Release;Platform=x86 "${git_DeployTool}\MediaPortal.DeployTool.sln"' = 0
!insertmacro FinalizeBuildReport
!endif
