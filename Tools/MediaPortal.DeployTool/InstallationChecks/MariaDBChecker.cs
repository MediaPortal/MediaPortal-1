...
namespace MediaPortal.DeployTool.InstallationChecks
{
  internal class MariaDBChecker : IInstallationPackage
  {
    ...
    private static string strMySQData = "";
    ...
    private bool IsMySQL51Installed()
    {
      ...
      if (key != null)
      {
        strMySQ = key.GetValue("Location").ToString();
        if (Utils.CheckTargetDir(strMySQ) && strMySQ.Contains("MySQL Server 5.1"))
        {
          strMySQData = key.GetValue("DataLocation").ToString();
          key.Close();
          return true;
        }
      }
    ...
    {
      if (!string.IsNullOrEmpty(strMySQ) && !string.IsNullOrEmpty(strMySQData))
      {
        ...
        strMySqlDump = "\"" + strMySQ + "bin\\mysqldump.exe" + "\"";
