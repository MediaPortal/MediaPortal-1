...
{
  public partial class InstallDlg_SVN : DeployDialog, IDeployDialog
  {
    public InstallDlg_SVN()
    {
      InitializeComponent();
      type=DialogType.Installation_SVN;
      PopulateListView();
      UpdateUI();
    }
    ...
    private void CheckAddPackage(IInstallationPackage package)
    {
      CheckResult result = package.CheckStatus();
      if (result.state != CheckState.NOT_INSTALLED)
      {
        AddPackageToListView(package);
      }
    }
    private void AddPackageToListView(IInstallationPackage package)
    {
      ...
      switch (result.state)
      {
        ...
        case CheckState.VERSION_MISMATCH:
          ...
          item.SubItems.Add(Localizer.Instance.GetString("Install_actionDownloadUpdate"));
          item.ForeColor = System.Drawing.Color.Purple;
          break;
        case CheckState.VERSION_LOOKUP_FAILED:
          item.SubItems.Add(Localizer.Instance.GetString("InstallDlg_VersionLookupFailed"));
          item.SubItems.Add(Localizer.Instance.GetString("Install_actionNothing"));
          item.ForeColor = System.Drawing.Color.Red;
          break;
      }
    ...
    private void PopulateListView()
    {
      ...
      CheckAddPackage(new MediaPortalChecker_SVN());
      CheckAddPackage(new TvServerChecker_SVN());
      CheckAddPackage(new TvPluginChecker_SVN());
      ...
      if (result.state != CheckState.INSTALLED)
      {
        switch (result.state)
        {
          case CheckState.NOT_INSTALLED:
            ...
            break;
          case CheckState.VERSION_LOOKUP_FAILED:
            item.SubItems[1].Text = Localizer.Instance.GetString("InstallDlg_VersionLookupFailed");
            break;
          case CheckState.VERSION_MISMATCH:
