...
namespace MediaPortal.GUI.TV
{
  ...
  public class GUITvGuideBase : GUIWindow
  {
    ...
    private List<TVChannel> _channelList = new List<TVChannel>();
    private List<TVRecording> _recordingList = new List<TVRecording>();
    ...
    private bool _autoTurnOnTv = false;
    private bool _disableXMLTVImportOption = false;
    ...
    private List<TVNotify> _notifyList = new List<TVNotify>();
    ...
    private static bool _workerThreadRunning = false;
        ...
        case GUIMessage.MessageType.GUI_MSG_WINDOW_INIT:
          {
            ...
            if (_autoTurnOnTv)
            {
              ...
              GUIMessage msg = new GUIMessage(GUIMessage.MessageType.GUI_MSG_RESUME_TV, (int) Window.WINDOW_TV, GetID, 0,
                                              0, 0, null);
    ...
    private void RenderSingleChannel(TVChannel channel)
    {
      int chan = _channelOffset;
      ...
      for (int iChannel = 0; iChannel < _channelCount; iChannel++)
      {
        if (chan < _channelList.Count)
        {
          ...
          strLogo = Util.Utils.GetCoverArt(Thumbs.TVChannel, tvChan.Name);
          if (File.Exists(strLogo))
          {
      ...
      strLogo = Util.Utils.GetCoverArt(Thumbs.TVChannel, channel.Name);
    ...
    private void setGuideHeadngVisibility(bool visible)
    {
    ...
    private void RenderChannel(int iChannel, TVChannel channel, long iStart, long iEnd, bool selectCurrentShow)
    {
    ...
    private void Import()
    {
      ...
      import.ShowProgress += new XMLTVImport.ShowProgressHandler(import_ShowProgress);
      ...
      if (pDlgOK != null)
      {
        ...
        pDlgOK.SetLine(2,
                       import.ImportStats.StartTime.ToShortDateString() + " - " +
                       import.ImportStats.EndTime.ToShortDateString());
        if (!bSucceeded)
        {
      ...
      if (dlg != null)
      {
        ...
        switch (dlg.SelectedId)
        {
          case 652: //Recorded TV
            GUIWindowManager.ActivateWindow((int) Window.WINDOW_RECORDEDTV);
            break;

          case 603: //Scheduled TV
            GUIWindowManager.ActivateWindow((int) Window.WINDOW_SCHEDULER);
            break;

          case 724: //TVGuide search
            GUIWindowManager.ActivateWindow((int) Window.WINDOW_SEARCHTV);
            break;
    ...
    private void import_ShowProgress(XMLTVImport.Stats stats)
    {
      GUIDialogProgress dlgProgress =
        (GUIDialogProgress) GUIWindowManager.GetWindow((int) Window.WINDOW_DIALOG_PROGRESS);
      if (dlgProgress != null)
