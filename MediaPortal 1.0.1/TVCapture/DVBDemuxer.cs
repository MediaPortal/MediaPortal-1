...
namespace MediaPortal.TV.Recording
{
  public class DVBDemuxer : ISampleGrabberCB
  {
    ...
    private static readonly int[,,] AudioBitrates = new int[,,]
                                                      {
                                                        {
                                                          {
    ...
    private static readonly double[] AudioTimes = new double[] {0.0, 103680000.0, 103680000.0, 34560000.0};

    private UInt32[] CRC32Data = new UInt32[]
                                   {
    ...
    private int _pidTeletext = 0;
    private int _pidSubtitle = 0;
    private int _pidVideo = 0;
    private int _pidMp2Audio = 0;
    private int _pidPmt = 0;
    private int _pidAc3;
    private bool _ac3Present = false;
    private DateTime _ac3Timer;
    private string _channelName = "";
    private int _restBufferLen = 0;
    private byte[] _restBuffer = new byte[200];
    private IntPtr _ptrRestBuffer = Marshal.AllocCoTaskMem(200);
#if GRABPPMT
  // pmt
...
int m_bufferPositionPMT=0;
    ...
    private DateTime _packetTimer = DateTime.MinValue;
    private bool _isReceivingPackets = false;
    private bool _isScrambled = false;
    private ulong _numberOfPacketsReceived = 0;
    // card
    private static int _currentDVBCard = 0;
    private static NetworkType _currentNetworkType;
    ...
    private TSHelperTools.TSHeader _packetHeader = new TSHelperTools.TSHeader();
    private TSHelperTools m_tsHelper = new TSHelperTools();

    private DVBEPG _epgClass = new DVBEPG();
  ...
  // pmt handling
    ...
    public void SetChannelData(int audio, int video, int ac3, int teletext, int subtitle, string channelName, int pmtPid,
                               int programnumber)
    {
    ...
    private UInt32 GetCRC32(byte[] data)
    {
    ...
    private UInt32 GetCRC32(byte[] data, UInt32 skip, UInt32 len)
    {
      UInt32 crc = 0xffffffff;
    ...
    private UInt32 GetSectionCRCValue(byte[] data, int ptr)
    {
    ...
    private bool ParseAudioHeader(byte[] data, ref AudioHeader header)
    {
    ...
    private void SaveData(int pid, int tableID, byte[] data)
    {
      lock (data.SyncRoot)
      {
        if (pid == 0xd3)
    ...
    private byte[] GetAudioHeader(byte[] data)
    {
      int pos = 0;
      bool found = false;
      ...
      if ((data[pos + 3] & 0xE0) == 0xC0)
      {
        if ((data[pos + 3] & 0xE0) == 0xC0)
        {
          Array.Copy(data, pos, data, 0, data.Length - pos);
          return GetPES(data);
        }
      }
      return new byte[184];
    ...
    public int BufferCB(double SampleTime, IntPtr pBuffer, int BufferLen)
    {
      ...
      if (_restBufferLen > 0)
      {
        ...
        if (len > 0 && len < BufferLen)
        {
          if (_restBufferLen >= 0 && _restBufferLen + len < 200)
          {
            ...
            //set offset ...
            if (Marshal.ReadByte(pBuffer, len) == 0x47 && Marshal.ReadByte(pBuffer, len + 188) == 0x47 &&
                Marshal.ReadByte(pBuffer, len + 2*188) == 0x47)
            {
              off = len;
            }
          }
      ...
      if (off == -1)
      {
        ...
        {
          if (Marshal.ReadByte(pBuffer, i) == 0x47 && Marshal.ReadByte(pBuffer, i + 188) == 0x47 &&
              Marshal.ReadByte(pBuffer, i + 2*188) == 0x47)
          {
