...
namespace MediaPortal.Visualization
{
    public class WMPInterop
    {
        ...
        {
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 2 * MaxSamples)]
        ...
        internal static extern bool InitWMPEngine([MarshalAs(UnmanagedType.LPWStr)] string strVizCLSID, int presetIndex, VisualizationBase.OutputContextType outputContextType, IntPtr callBack, IntPtr hOutput, ref VisualizationBase.RECT rect);
    ...
    public class WMPViz : VisualizationBase
    {
        ...
        public override bool Initialize()
        {
            ...
            try
            {
                ...
                result = WMPInterop.InitWMPEngine(VizPluginInfo.CLSID, VizPluginInfo.PresetIndex, outputType, IntPtr.Zero, VisualizationWindow.CompatibleDC, ref rect);
                _Initialized = result;
            ...
            catch (Exception ex)
            {
                ...
                Log.Error("  Visualization Manager: WMP visualization engine initialization failed with the following exception {0}", ex);
                return false;
            }
        ...
        private bool GetFftData(ref byte[] audioData)
        {
            ...
            if(!_IsPreviewVisualization)
            {
                ...
                fft = new float[1024];
                Un4seen.Bass.Bass.BASS_ChannelGetData(stream, ref fft[0], (int)Un4seen.Bass.BASSData.BASS_DATA_FFT2048);
            }
