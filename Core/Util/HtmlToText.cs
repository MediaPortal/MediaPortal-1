...
namespace MediaPortal.Util
{
  ...
  public class HtmlToText
  {
    ...
    private char ch;
    private int pos;
    private bool convert_characters = true;

    private int LEFT = 1;
    private int CENTER = 2;
    private int RIGHT = 3;

    private int spreads = 50;
    private int hr_spreads = 50;
    private int paragraph = 0;

    private int tab = 4; // tabulator
    private int spaces = 0; // spaces at beginning of line
    private bool nooutput = false; // for SCRIPT, etc: no output
    private int orderedlist = 0; // OL

    private int div_test = 0;

    private string word = "";

    private string line = "";
    private int line_len = 0; // apparent length of the line
    private int line_len_old = 0;
    private int line_pos = 0; // true length of line
    private int word_len = 0; // apparent length of the word
    private int word_pos = 0; // true length of word
    private int nrBlankLines = 0; // how many line were blank
    private int noleadingblanks = 0; // remove blanks lines at the start of the ouput

    private bool convert_tags = false;
    private int errorlevel = 1;
    private int shrink_lines = 0;
    private bool option_links = false;
    private bool option_title = false;
    private bool remove_empty_alt = true; /* dont show [] for <IMG ALT="">  */
    private bool option_no_image = true; /* don't show [Image] */
    private bool option_no_alt = true; /* don't show [alt text] for <IMG ALT="alt text" */
    private bool stripMultipleBlankLines = true; // Strip multiple blank lines
    private bool removeLinesWithOnlyLinks = true;

    // Dynamic align added by autophile@starband.net 29 Mar 2002
    private Stack align = new Stack();
    ...
    private char get_attr(out string attr_name, out string attr_ctnt)
    {
      string temp;
    ...
    private void readTag()
    {
      ...
      if (!nooutput)
      {
        ...
        else if (CMP("!DOCTYPE", str))
        {
          while ((ch = (char)nextChar()) != '>') ;
        }
        ...
        else if (CMP("A", str))
        {
          href();
          word_plus_ch('[');
        } //if ( !option_links) start_nooutput();}
        else if (CMP("/A", str))
        {
        ...
        else if (CMP("INPUT", str))
        {
          image("Input", 0);
        }

          // table
        else if (CMP("TABLE", str))
        {
          push_align(LEFT);
          neuer_paragraph();
        }
    ...
    private void parse_entities(ref string s)
    {
      int i = 0;
      string entity = "";
      int len = s.Length;
    ...
    private bool html_entity(string str, out string conv)
    {
      if (CMP("&quot;", str))
      {
        return (set_char(str, out conv, '"'));
      }
      else if (CMP("&;", str))
      {
        return (set_char(str, out conv, '&'));
      } // for those brain damaged ones
      else if (CMP("&amp;", str))
      {
        return (set_char(str, out conv, '&'));
      }
    ...
    private bool latin1(string str, out string conv)
    {
      if (CMP("&nbsp;", str))
      {
        return (set_char(str, out conv, 160));
      } // no-break space
      else if (CMP("&iexcl;", str))
      {
        return (set_char(str, out conv, 161));
      } // inverted exclamation mark
      else if (CMP("&cent;", str))
      {
        return (set_char(str, out conv, 162));
      } // cent sign
      else if (CMP("&pound;", str))
      {
        return (set_char(str, out conv, 163));
      } // pound sterling sign
      else if (CMP("&curren;", str))
      {
        return (set_char(str, out conv, 164));
      } // general currency sign
      else if (CMP("&yen;", str))
      {
        return (set_char(str, out conv, 165));
      } // yen sign
      else if (CMP("&brvbar;", str))
      {
        return (set_char(str, out conv, 166));
      } // broken (vertical) bar
      else if (CMP("&sect;", str))
      {
        return (set_char(str, out conv, 167));
      } // section sign
      else if (CMP("&uml;", str))
      {
        return (set_char(str, out conv, 168));
      } // umlaut (dieresis)
      else if (CMP("&copy;", str))
      {
        return (set_char(str, out conv, 169));
      } // copyright sign
      else if (CMP("&ordf;", str))
      {
        return (set_char(str, out conv, 170));
      } // ordinal indicator, feminine
      else if (CMP("&laquo;", str))
      {
        return (set_char(str, out conv, 171));
      } // angle quotation mark, left
      else if (CMP("&not;", str))
      {
        return (set_char(str, out conv, 172));
      } // not sign
      else if (CMP("&shy;", str))
      {
        return (set_char(str, out conv, '\0'));
      } // soft hyphen, just swallow it
      else if (CMP("&reg;", str))
      {
        return (set_char(str, out conv, 174));
      } // registered sign
      else if (CMP("&macr;", str))
      {
        return (set_char(str, out conv, 175));
      } // macron
      else if (CMP("&deg;", str))
      {
        return (set_char(str, out conv, 176));
      } // degree sign
      else if (CMP("&plusmn;", str))
      {
        return (set_char(str, out conv, 177));
      } // plus-or-minus sign
      else if (CMP("&sup2;", str))
      {
        return (set_char(str, out conv, 178));
      } // superscript two
      else if (CMP("&sup3;", str))
      {
        return (set_char(str, out conv, 179));
      } // superscript three
      else if (CMP("&acute;", str))
      {
        return (set_char(str, out conv, 180));
      } // acute accent
      else if (CMP("&micro;", str))
      {
        return (set_char(str, out conv, 181));
      } // micro sign
      else if (CMP("&para;", str))
      {
        return (set_char(str, out conv, 182));
      } // pilcrow (paragraph sign)
      else if (CMP("&middot;", str))
      {
        return (set_char(str, out conv, 183));
      } // middle dot
      else if (CMP("&cedil;", str))
      {
        return (set_char(str, out conv, 184));
      } // cedilla
      else if (CMP("&sup1;", str))
      {
        return (set_char(str, out conv, 185));
      } // superscript one
      else if (CMP("&ordm;", str))
      {
        return (set_char(str, out conv, 186));
      } // ordinal indicator, masculine
      else if (CMP("&raquo;", str))
      {
        return (set_char(str, out conv, 187));
      } // angle quotation mark, right
      else if (CMP("&frac14;", str))
      {
        return (set_char(str, out conv, 188));
      } // fraction one-quarter
      else if (CMP("&frac12;", str))
      {
        return (set_char(str, out conv, 189));
      } // fraction one-half
      else if (CMP("&frac34;", str))
      {
        return (set_char(str, out conv, 190));
      } // fraction three-quarters
      else if (CMP("&iquest;", str))
      {
        return (set_char(str, out conv, 191));
      } // inverted question mark
      else if (CMP("&Agrave;", str))
      {
        return (set_char(str, out conv, 192));
      } // capital A, grave accent
      else if (CMP("&Aacute;", str))
      {
        return (set_char(str, out conv, 193));
      } // capital A, acute accent
      else if (CMP("&Acirc;", str))
      {
        return (set_char(str, out conv, 194));
      } // capital A, circumflex accent
      else if (CMP("&Atilde;", str))
      {
        return (set_char(str, out conv, 195));
      } // capital A, tilde
      else if (CMP("&Auml;", str))
      {
        return (set_char(str, out conv, 196));
      } // capital A, dieresis or umlaut mark
      else if (CMP("&Aring;", str))
      {
        return (set_char(str, out conv, 197));
      } // capital A, ring
      else if (CMP("&AElig;", str))
      {
        return (set_char(str, out conv, 198));
      } // capital AE diphthong (ligature)
      else if (CMP("&Ccedil;", str))
      {
        return (set_char(str, out conv, 199));
      } // capital C, cedilla
      else if (CMP("&Egrave;", str))
      {
        return (set_char(str, out conv, 200));
      } // capital E, grave accent
      else if (CMP("&Eacute;", str))
      {
        return (set_char(str, out conv, 201));
      } // capital E, acute accent
      else if (CMP("&Ecirc;", str))
      {
        return (set_char(str, out conv, 202));
      } // capital E, circumflex accent
      else if (CMP("&Euml;", str))
      {
        return (set_char(str, out conv, 203));
      } // capital E, dieresis or umlaut mark
      else if (CMP("&Igrave;", str))
      {
        return (set_char(str, out conv, 204));
      } // capital I, grave accent
      else if (CMP("&Iacute;", str))
      {
        return (set_char(str, out conv, 205));
      } // capital I, acute accent
      else if (CMP("&Icirc;", str))
      {
        return (set_char(str, out conv, 206));
      } // capital I, circumflex accent
      else if (CMP("&Iuml;", str))
      {
        return (set_char(str, out conv, 207));
      } // capital I, dieresis or umlaut mark
      else if (CMP("&ETH;", str))
      {
        return (set_char(str, out conv, 208));
      } // capital Eth, Icelandic
      else if (CMP("&Ntilde;", str))
      {
        return (set_char(str, out conv, 209));
      } // capital N, tilde
      else if (CMP("&Ograve;", str))
      {
        return (set_char(str, out conv, 210));
      } // capital O, grave accent
      else if (CMP("&Oacute;", str))
      {
        return (set_char(str, out conv, 211));
      } // capital O, acute accent
      else if (CMP("&Ocirc;", str))
      {
        return (set_char(str, out conv, 212));
      } // capital O, circumflex accent
      else if (CMP("&Otilde;", str))
      {
        return (set_char(str, out conv, 213));
      } // capital O, tilde
      else if (CMP("&Ouml;", str))
      {
        return (set_char(str, out conv, 214));
      } // capital O, dieresis or umlaut mark
      else if (CMP("&times;", str))
      {
        return (set_char(str, out conv, 215));
      } // multiply sign
      else if (CMP("&Oslash;", str))
      {
        return (set_char(str, out conv, 216));
      } // capital O, slash
      else if (CMP("&Ugrave;", str))
      {
        return (set_char(str, out conv, 217));
      } // capital U, grave accent
      else if (CMP("&Uacute;", str))
      {
        return (set_char(str, out conv, 218));
      } // capital U, acute accent
      else if (CMP("&Ucirc;", str))
      {
        return (set_char(str, out conv, 219));
      } // capital U, circumflex accent
      else if (CMP("&Uuml;", str))
      {
        return (set_char(str, out conv, 220));
      } // capital U, dieresis or umlaut mark
      else if (CMP("&Yacute;", str))
      {
        return (set_char(str, out conv, 221));
      } // capital Y, acute accent
      else if (CMP("&THORN;", str))
      {
        return (set_char(str, out conv, 222));
      } // capital THORN, Icelandic
      else if (CMP("&szlig;", str))
      {
        return (set_char(str, out conv, 223));
      } // small sharp s, German (sz ligature)
      else if (CMP("&agrave;", str))
      {
        return (set_char(str, out conv, 224));
      } // small a, grave accent
      else if (CMP("&aacute;", str))
      {
        return (set_char(str, out conv, 225));
      } // small a, acute accent
      else if (CMP("&acirc;", str))
      {
        return (set_char(str, out conv, 226));
      } // small a, circumflex accent
      else if (CMP("&atilde;", str))
      {
        return (set_char(str, out conv, 227));
      } // small a, tilde
      else if (CMP("&auml;", str))
      {
        return (set_char(str, out conv, 228));
      } // small a, dieresis or umlaut mark
      else if (CMP("&aring;", str))
      {
        return (set_char(str, out conv, 229));
      } // small a, ring
      else if (CMP("&aelig;", str))
      {
        return (set_char(str, out conv, 230));
      } // small ae diphthong (ligature)
      else if (CMP("&ccedil;", str))
      {
        return (set_char(str, out conv, 231));
      } // small c, cedilla
      else if (CMP("&egrave;", str))
      {
        return (set_char(str, out conv, 232));
      } // small e, grave accent
      else if (CMP("&eacute;", str))
      {
        return (set_char(str, out conv, 233));
      } // small e, acute accent
      else if (CMP("&ecirc;", str))
      {
        return (set_char(str, out conv, 234));
      } // small e, circumflex accent
      else if (CMP("&euml;", str))
      {
        return (set_char(str, out conv, 235));
      } // small e, dieresis or umlaut mark
      else if (CMP("&igrave;", str))
      {
        return (set_char(str, out conv, 236));
      } // small i, grave accent
      else if (CMP("&iacute;", str))
      {
        return (set_char(str, out conv, 237));
      } // small i, acute accent
      else if (CMP("&icirc;", str))
      {
        return (set_char(str, out conv, 238));
      } // small i, circumflex accent
      else if (CMP("&iuml;", str))
      {
        return (set_char(str, out conv, 239));
      } // small i, dieresis or umlaut mark
      else if (CMP("&eth;", str))
      {
        return (set_char(str, out conv, 240));
      } // small eth, Icelandic
      else if (CMP("&ntilde;", str))
      {
        return (set_char(str, out conv, 241));
      } // small n, tilde
      else if (CMP("&ograve;", str))
      {
        return (set_char(str, out conv, 242));
      } // small o, grave accent
      else if (CMP("&oacute;", str))
      {
        return (set_char(str, out conv, 243));
      } // small o, acute accent
      else if (CMP("&ocirc;", str))
      {
        return (set_char(str, out conv, 244));
      } // small o, circumflex accent
      else if (CMP("&otilde;", str))
      {
        return (set_char(str, out conv, 245));
      } // small o, tilde
      else if (CMP("&ouml;", str))
      {
        return (set_char(str, out conv, 246));
      } // small o, dieresis or umlaut mark
      else if (CMP("&divide;", str))
      {
        return (set_char(str, out conv, 247));
      } // divide sign
      else if (CMP("&oslash;", str))
      {
        return (set_char(str, out conv, 248));
      } // small o, slash
      else if (CMP("&ugrave;", str))
      {
        return (set_char(str, out conv, 249));
      } // small u, grave accent
      else if (CMP("&uacute;", str))
      {
        return (set_char(str, out conv, 250));
      } // small u, acute accent
      else if (CMP("&ucirc;", str))
      {
        return (set_char(str, out conv, 251));
      } // small u, circumflex accent
      else if (CMP("&uuml;", str))
      {
        return (set_char(str, out conv, 252));
      } // small u, dieresis or umlaut mark
      else if (CMP("&yacute;", str))
      {
        return (set_char(str, out conv, 253));
      } // small y, acute accent
      else if (CMP("&thorn;", str))
      {
        return (set_char(str, out conv, 254));
      } // small thorn, Icelandic
      else if (CMP("&yuml;", str))
      {
        return (set_char(str, out conv, 255));
    ...
    private bool microsoft_entities(string str, out string conv)
    {
      ...
      {
        set_char(str, out conv, ',');
      }
      else if (CMP("&sbquo;", str))
      {
        set_char(str, out conv, ',');
      }
      else if (number == 8218)
      ...
      {
        set_char(str, out conv, 'f');
      } // Latin Small Letter F With Hook
      else if (CMP("&fnof;", str))
      {
        set_char(str, out conv, 'f');
      } // Latin Small Letter F With Hook
      else if (number == 402)
      ...
      {
        set_char(str, out conv, 'S');
      }
      else if (CMP("&Scaron;", str))
      {
        set_char(str, out conv, 'S');
      }
      else if (number == 352)
    ...
    private void microsoft_character(int c)
    {
      switch (c)
      {
          /* Microsoft... */
        case 0x80: /* MICROSOFT EURO */
          word_plus_string("EUR");
          break;
    ...
    private void push_align(int a)
    {
      align.Push(a);
    }

    private void pop_align()
    {
      align.Pop();
    }
    ...
    private bool only_spaces(string z)
    {
      int len = z.Length;
      int i;
      bool ret = true;
    ...
    private void print_line()
    {
      ...
      if (noleadingblanks == 0)
      {
        noleadingblanks = (only_spaces(line) ? 0 : 1);
      }
    ...
    private bool doesLineContainOnlyLinks(string line)
    {
      bool inLink = false;
      bool linksOnly = true;
      int open = 0;
    ...
    private void word_plus_string(string s)
    {
      int len = s.Length,
          i = word_pos,
          j = 0;
    ...
    private void word_plus_ch(int c)
    {
      word += (char)c;
      word_pos++;
      word_len++;
    ...
    private void hr()
    {
      ...
      while (ch != '>')
      {
        ...
        if (CMP("ALIGN", attr_name))
        {
          ...
          else if (CMP("CENTER", attr_ctnt))
          {
            hr_align = CENTER;
          }
          else if (CMP("RIGHT", attr_ctnt))
          {
            hr_align = RIGHT;
          }
          else if (CMP("JUSTIFY", attr_ctnt))
          {
            hr_align = LEFT;
          }
          else
          {
            if (errorlevel >= 2)
            {
              print_error("Error ", "No LEFT|CENTER|RIGHT found!\n");
            }
          }
        }
        else if (CMP("WIDTH", attr_name))
        {
          ...
          if (attr_ctnt[i - 1] == '%')
          {
            ...
            if (hr_width == 100)
            {
              hr_width = hr_spreads - 4;
            }
            else
            {
              hr_width = hr_spreads * hr_width / 100;
            }
          }
          else
          {
            hr_width = int.Parse(attr_ctnt) / 8;
            if (hr_width > hr_spreads - 4)
            {
              hr_width = hr_spreads - 4;
            }
          }
    ...
    private void output_string(string str)
    {
    ...
    */
    ...
    private void end_dt() {} /* end_dt */

    /* Definition Description */

    private void start_dd()
    {
      end_dd();
    ...
    private void image(string alt_text, int show_alt)
    {
      int found_alt = 0;
      string attr_name;
      string attr_ctnt;
    ...
    private void print_error(string error, string text)
    {
    ...
    private void print_footnote_number(ref string temp, int number)
    {
      temp = String.Format("[{0}]", number);
    }

    private void construct_footnote(ref string temp, int number, string link)
    {
