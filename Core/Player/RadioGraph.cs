...
namespace MediaPortal.Player
{
  ...
  public class RadioGraph
  {
    ...
    private State m_graphState = State.None;
    private IGraphBuilder m_graphBuilder = null;
    private ICaptureGraphBuilder2 m_captureGraphBuilder = null;
    private IBaseFilter m_filterCaptureVideo = null;
    private IAMTVTuner m_TVTuner = null;
    private DsROTEntry _rotEntry = null; // Cookie into the Running Object Table
    private VideoCaptureDevice m_videoCaptureDevice = null;
    private IMediaControl m_mediaControl = null;
    private string m_strVideoCaptureFilter = "";
    private int m_iCurrentChannel = 28;
    private int m_iCountryCode = 31;
    private int m_iTuningSpace = 0;
    private bool m_bUseCable = false;
    private IBaseFilter m_audioFilter = null;
    private string m_strAudioDevice = string.Empty;
    private string m_strAudioInputPin = string.Empty;
    private IBaseFilter m_filterCaptureAudio = null;
...
*/
    ...
    public void Tune(int channel)
    {
      ...
      if (m_graphState == State.Created)
      {
        ...
        if (m_strAudioDevice.Length > 0)
        {
          CrossBar.Route(m_graphBuilder, m_captureGraphBuilder, m_filterCaptureVideo, true, false, false, false, false,
                         true);
        }
        ...
        if (hr != 0)
        {
          Log.Info("RadioGraph:start listening returned :0x{0:X}", hr);
        }
      }
      else
      {
    ...
    public void DeleteGraph()
    {
      ...
      if (m_TVTuner != null)
      {
        DirectShowUtil.ReleaseComObject(m_TVTuner);
      }
      m_TVTuner = null;

      if (m_audioFilter != null)
      {
        DirectShowUtil.ReleaseComObject(m_audioFilter);
      }
      m_audioFilter = null;
      ...
      if (m_filterCaptureVideo != null)
      {
        DirectShowUtil.ReleaseComObject(m_filterCaptureVideo);
      }
      m_filterCaptureVideo = null;

      if (m_filterCaptureAudio != null)
      {
        DirectShowUtil.ReleaseComObject(m_filterCaptureAudio);
      }
      m_filterCaptureAudio = null;
      ...
      {
        DirectShowUtil.ReleaseComObject(m_captureGraphBuilder);
      }
      m_captureGraphBuilder = null;

      if (m_graphBuilder != null)
      {
        DirectShowUtil.ReleaseComObject(m_graphBuilder);
      }
      m_graphBuilder = null;
    ...
    public int AudioFrequency
    {
      get
      {
        if (m_graphState != State.Listening)
        {
          return 0;
        }
        if (m_TVTuner == null)
