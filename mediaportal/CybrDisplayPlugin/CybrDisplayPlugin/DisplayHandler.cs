namespace CybrDisplayPlugin
{
    ...
    public class DisplayHandler
    {
        ...
        private List<CybrDisplayPlugin.Setting.Image> images;
        ...
        internal void DisplayLines()
        {
            ...
            {
                Log.Info("CybrDisplayPlugin.DisplayHandler.DisplayLines(): Sending lines to display.", new object[0]);
            }
            try
            {
            ...
            {
                Log.Error("CybrDisplayPlugin.DisplayHandler.DisplayLines(): CAUGHT EXCEPTION {0}\n\n{1}\n\n" + exception.Message, new object[] { exception.StackTrace });
            }
        }
        ...
        private void DrawImages(Graphics graphics)
        {
            ...
            {
                Log.Info("CybrDisplayPlugin.DisplayHandler.DrawImages(): No images to process", new object[0]);
            }
            foreach (CybrDisplayPlugin.Setting.Image image in this.Images)
            {
                if (Settings.Instance.ExtensiveLogging)
                {
                    Log.Info("CybrDisplayPlugin.DisplayHandler.DrawImages(): Drawing image to buffer", new object[0]);
                }
        ...
        protected string Process(int _line)
        {
            if (Settings.Instance.ExtensiveLogging)
            {
                Log.Info("CybrDisplayPlugin.DisplayHandler.Process(): Processing line #{0}", new object[] { _line });
            }
            Line line = this.lines[_line];
            string str = line.Process();
            if (Settings.Instance.ExtensiveLogging)
            {
                Log.Info("CybrDisplayPlugin.DisplayHandler.Process(): translated line #{0} to \"{2}\"", new object[] { _line, str });
            }
            ...
            {
                Log.Error("CybrDisplayPlugin.DisplayHandler.Process(): CAUGHT EXCEPTION - {0}\n\n{1}\n\n" + exception.Message, new object[] { exception.StackTrace });
            }
            if (str.Length <= this.widthInChars)
            {
                if (Settings.Instance.ExtensiveLogging)
                {
                    Log.Info("CybrDisplayPlugin.DisplayHandler.Process(): final processing result: \"{0}\"", new object[] { str });
                }
                switch (line.Alignment)
                {
                    case CybrDisplayPlugin.Setting.Alignment.Centered:
                    {
                        int count = (this.widthInChars - str.Length) / 2;
                        return (new string(' ', count) + str + new string(' ', (this.widthInChars - str.Length) - count));
                    }
                    case CybrDisplayPlugin.Setting.Alignment.Right:
        ...
        private void ProcessG(Graphics _graphics, int _line)
        {
            if (font.SizeInPoints != Settings.Instance.FontSize)
            {
                if (Settings.Instance.ExtensiveLogging)
                {
                    Log.Debug("CybrDisplayPlugin.DisplayHandler.ProcessG: Adjusting graphics Font size to {0}.", new object[] { Settings.Instance.FontSize });
                }
                font.Dispose();
                font = new Font(Settings.Instance.Font, (float) Settings.Instance.FontSize);
            }
            ...
            if (!string.IsNullOrEmpty(str))
            {
                ...
                {
                    Log.Error("CybrDisplayPlugin.DisplayHandler.ProcessG(): error - {0}" + exception.Message, new object[0]);
                }
        ...
        private void SendGraphics()
        {
            using (Bitmap bitmap = this.GetEmptyBitmap())
            {
                using (Graphics graphics = Graphics.FromImage(bitmap))
                {
                    if (Settings.Instance.ExtensiveLogging)
                    {
                        Log.Info("CybrDisplayPlugin.DisplayHandler.SendGraphics(): Processing graphics display.", new object[0]);
                    }
        ...
        internal void Start()
        {
            Log.Info("CybrDisplayPlugin.DisplayHandler.Start(): Called", new object[0]);
            Log.Info("CybrDisplayPlugin.DisplayHandler.Start(): Calling driver Setup() function", new object[0]);
            ...
            if (font.SizeInPoints != Settings.Instance.FontSize)
            {
                font.Dispose();
                font = new Font(Settings.Instance.Font, (float) Settings.Instance.FontSize);
                Log.Info("CybrDisplayPlugin.DisplayHandler.Start(): Forcing font size to {0}", new object[] { Settings.Instance.FontSize });
            }
            ...
            Log.Info("CybrDisplayPlugin.DisplayHandler.Start(): Calling driver Initialize() function", new object[0]);
            this.display.Initialize();
            this.display.SetCustomCharacters(Settings.Instance.CustomCharacters);
            Log.Info("CybrDisplayPlugin.DisplayHandler.Start(): Completed", new object[0]);
        }

        internal void Stop()
        {
            Log.Info("CybrDisplay.DisplayHandler.Stop(): Called", new object[0]);
            Log.Info("CybrDisplay.DisplayHandler.Stop(): Calling driver CleanUp() function", new object[0]);
            this.display.CleanUp();
            Log.Info("CybrDisplay.DisplayHandler.Stop(): completed", new object[0]);
        }

        public List<CybrDisplayPlugin.Setting.Image> Images
        {
            get
            {
                return this.images;
