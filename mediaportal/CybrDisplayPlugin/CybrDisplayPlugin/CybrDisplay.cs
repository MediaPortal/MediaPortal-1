namespace CybrDisplayPlugin
{
    ...
    public class CybrDisplay : IPlugin, ISetupForm
    {
        ...
        public CybrDisplay()
        {
            Log.Info("CybrDisplay - constructor: called", new object[0]);
        }

        public string Author()
        {
            Log.Info("CybrDisplay.Author(): called", new object[0]);
            return "CybrMage";
        }
        ...
        {
            Log.Info("CybrDisplay.CanEnable(): called", new object[0]);
            return true;
        }

        public bool DefaultEnabled()
        {
            Log.Info("CybrDisplay.DefaultEnabled(): called", new object[0]);
            return false;
        }

        public string Description()
        {
            Log.Info("CybrDisplay.Description(): called", new object[0]);
        ...
        {
            Log.Info("CybrDisplay.DoStart(): called", new object[0]);
        ...
        {
            string str;
            Log.Debug("CybrDisplay.GetHome(): Called.", new object[0]);
        ...
        private void GetTVSource()
        {
            ...
            if ((File.Exists(Config.GetFolder(Config.Dir.Base) + @"\TvControl.dll") && File.Exists(Config.GetFolder(Config.Dir.Base) + @"\TvLibrary.Interfaces.dll")) && File.Exists(Config.GetFolder(Config.Dir.Plugins) + @"\Windows\TvPlugin.dll"))
            {
                using (MediaPortal.Profile.Settings settings = new MediaPortal.Profile.Settings(Config.GetFile(Config.Dir.Config, "MediaPortal.xml")))
                {
                    ...
                    {
                        Log.Info("CybrDisplay.GetTVSource(): Found configured TVServer instalation", new object[0]);
                        UseTVServer = true;
                    }
                    else
                    {
                        Log.Info("CybrDisplay.GetTVSource(): Found TVServer instalation", new object[0]);
                    }
                }
            }
        }
        ...
        {
            Log.Info("CybrDisplay.GetWindowId(): called", new object[0]);
            return 0x4da6;
        }
        ...
        {
            Log.Info("CybrDisplay.HasSetup(): called", new object[0]);
            return true;
        }
        ...
        {
            Log.Info("CybrDisplay.PluginName(): called", new object[0]);
            return "CybrDisplay Plugin";
        }
