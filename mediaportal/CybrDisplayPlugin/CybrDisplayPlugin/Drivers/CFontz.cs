namespace CybrDisplayPlugin.Drivers
{
    ...
    public class CFontz : BaseDisplay, IDisplay, IDisposable
    {
        ...
        private void LoadAdvancedSettings()
        {
            ...
            FileInfo info = new FileInfo(Config.GetFile(Config.Dir.Config, "CybrDisplay_CFontz.xml"));
        ...
        private void UpdateAdvancedSettings()
        {
            if (DateTime.Now.Ticks >= this.LastSettingsCheck.AddMinutes(1.0).Ticks)
            {
                ...
                if (File.Exists(Config.GetFile(Config.Dir.Config, "CybrDisplay_CFontz.xml")))
                {
                    FileInfo info = new FileInfo(Config.GetFile(Config.Dir.Config, "CybrDisplay_CFontz.xml"));
        ...
        public class AdvancedSettings
        {
            ...
            public static CFontz.AdvancedSettings Load()
            {
                ...
                if (File.Exists(Config.GetFile(Config.Dir.Config, "CybrDisplay_CFontz.xml")))
                {
                    ...
                    XmlTextReader xmlReader = new XmlTextReader(Config.GetFile(Config.Dir.Config, "CybrDisplay_CFontz.xml"));
            ...
            public static void Save(CFontz.AdvancedSettings ToSave)
            {
                ...
                XmlSerializer serializer = new XmlSerializer(typeof(CFontz.AdvancedSettings));
                XmlTextWriter writer = new XmlTextWriter(Config.GetFile(Config.Dir.Config, "CybrDisplay_CFontz.xml"), Encoding.UTF8);
                writer.Formatting = Formatting.Indented;
        ...
        private class CFDisplay
        {
            ...
            public void DrawVerticalBarGraph(ref CybrDisplay.EQControl EQSettings, ref byte[] EqDataArray)
            {
                if (!((!this._isOpen | !this.commPort.IsOpen) | this._IsDisplayOff))
                {
