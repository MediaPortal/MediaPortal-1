...
namespace MediaPortal.Configuration.Sections
{
  public class TV : SectionSettings
  {
    ...
    public override void LoadSettings()
    {
      ...
      FillLists(mpListViewAvailAudioLang, mpListViewPreferredAudioLang, preferredAudioLanguages);
      FillLists(mpListViewAvailSubLang, mpListViewPreferredSubLang, preferredSubLanguages);
      _SingleSeat = Network.IsSingleSeat();
    }

    private void FillLists(MPListView availList, MPListView preferredList, string preferredLanguages)
    {
      ...
      for (int i = 0; i < preferredLanguageKeys.Length; i++)
      {
        string key = preferredLanguageKeys[i];
        if (!string.IsNullOrEmpty(key) && !preferredList.Items.ContainsKey(key))
        {
          Iso639Language lang = Iso639LanguageCollection.Instance.GetLanguageFromThreeLetterCode(key);
          ListViewItem item = null;
          if (lang == null)
          {
            item = new ListViewItem(new string[] { "Custom", key }) { Name = key };
          }
          else
          {
            // Is the next key the alternative code for this same language?
            if (i < preferredLanguageKeys.Length - 1)
            {
              string nextKey = preferredLanguageKeys[i + 1];
              if (!key.Equals(nextKey) && (lang.TerminologicCode.Equals(nextKey) || lang.BibliographicCode.Equals(nextKey)))
              {
                key += ";" + nextKey;
                i++;
              }
            }
            item = new ListViewItem(new string[] { lang.Name, key }) { Name = key };
          }
      ...
      HashSet<string> preferredLanguageKeyHash = new HashSet<string>(preferredLanguageKeys);
      foreach (Iso639Language lang in Iso639LanguageCollection.Instance.Languages)
      {
        bool add = !preferredLanguageKeyHash.Remove(lang.TerminologicCode);
        string key = lang.TerminologicCode;
        if (!lang.TerminologicCode.Equals(lang.BibliographicCode))
        {
          add &= !preferredLanguageKeyHash.Remove(lang.BibliographicCode);
          key += ";" + lang.BibliographicCode;
        }
        if (add)
        {
          ListViewItem item = new ListViewItem(new string[] { lang.Name, key }) { Name = key };
