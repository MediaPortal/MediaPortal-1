	...
	public class PowerScheduler : IPluginReceiver, IWakeable, ISetupForm
  {
    ...
    #region Private Methods
		...
		private void LoadSettings()
		{
			...
			{
				_shutDownInterval = xmlreader.GetValueAsInt("powerscheduler", "shutdowninterval", 2);
    ...
    /// <param name="windowId">id of the window which is about to be activated</param>
		...
		{
			if (_shutDownTime == DateTime.MaxValue) return; 
    ...
    protected bool WakeAbleDisallowShutdown()
    {
		...
		void CheckNextRecoring()
		{
			...
			if (_shutDownTime < DateTime.MaxValue) ealiestStartTime = _shutDownTime;
			...
			if (TVDatabase.GetRecordings(ref recList))
			{
				...
				{
					if ((rec.Canceled > 0) || (rec.IsDone())) continue;
