...
namespace ProcessPlugins.ExternalDisplay.Drivers
{
  ...
  /// <author>CybrMage</author>
    ...
    public class MatrixGX : BaseDisplay, IDisplay
    {
        ...
        MOGXDisplay MOD = new MOGXDisplay();
        bool _IsOpen = false;
        bool _Contrast = false;
        bool _BackLightControl = false;
        private int _Gcols;
        private int _Grows;
        ...
        public bool IsDisabled
        {
            get {
                ...
                _isDisabled = false;
                int fMissing = 0;
                if (!File.Exists("LibDriverCoreClient.dll")) fMissing += 1;
                if (!File.Exists("log4net.dll")) fMissing += 2;
                if (!File.Exists("fastbitmap.dll")) fMissing += 4;
                if (fMissing > 0)
                {
                    if ((fMissing & 1) > 0)
                    {
        ...
        public void SetLine(int line, string message)
        {
            ...
            _lines[line] = message;
            if (_lines[0].Trim() == "MediaPortal") _mpIsIdle = true; else _mpIsIdle = false;
            if (line == 1)
            {
        ...
        {
            Form AdvSettings = new MatrixGX.AdvancedSetupForm();
                          ...
                          bool _backLight, int _contrast)
        {
            ...
            try
            {
                ...
                Log.Debug("IDisplay: MatrixGX.Setup() - Advanced Options - Vol: {0}, Prog: {1}, Invert: {2}, Backlight: R{3} - G{4} - B{5}, Dicon: {6}, DFAM: {7}", _useVolumeDisplay.ToString(), _useProgressDisplay.ToString(), _useInvertedDisplay.ToString(), _BacklightRED.ToString(), _BacklightGREEN.ToString(), _BacklightBLUE.ToString(), _useIcons, _useDiskIconForAllMedia);
                // GX series display
                _Grows = _linesG;
                _Gcols = _colsG;
                if (_Gcols > 240)
                ...
                _IsOpen = MOD.OpenDisplay(_port, _delay, _contrast, _BacklightRED, _BacklightGREEN, _BacklightBLUE, _useInvertedDisplay);
                if (_IsOpen)
                {
...
//            MOD.CloseDisplay(_BackLightControl);
        ...
        private class MOGXDisplay
        {
            ...
            public bool OpenDisplay(string _port, int _delay, int _contrast, int _backlightR, int _backlightG, int _backlightB, bool _InvertDisplay)
            {
                ...
                try
                {
                    ...
                    if (GX_Devices.Count() > 0)
                    {
                        ...
                        _displayThread = new Thread(new ThreadStart(this.DisplayUpdate));
            ...
            public void CloseDisplay(bool _backlight)
            {
                ...
                try
                {
                    ...
                    lock (DWriteMutex)
                    {
                        ...
                        while (_displayThread.IsAlive) Thread.Sleep(100);
                        _isOpen = false;
                    }
...
//                    if ((_Bitmap.Height == gx_bounds.Height) & (_Bitmap.Width == gx_bounds.Width))
                    ...
                    else
                    {
                        ...
                        GX_Surface = _Bitmap.Clone(new Rectangle(0, 0, bWidth, bHeight), System.Drawing.Imaging.PixelFormat.Format1bppIndexed);
            ...
            public void BacklightOff()
            {
                ...
                lock (DWriteMutex)
                {
                    GX_Session.SetRGBBacklight(0, 0, 0);
                }
            }
            ...
            private RectangleF GetTextBounds()      // must be called with DWriteMutex LOCKED
            {
                ...
                if (_useProgressDisplay) gx_bounds.Height -= 8;
                if (_useIcons) gx_bounds.Width -= 32;
                return gx_bounds;
            }
...
//                GX_Session.SendAsyncFrame(GX_Surface);
            ...
            {
                if (!_useIcons || _isClosing) return;
                Log.Debug("MatrixGX.MOGXDisplay.ClearIconArea() - called");
                RectangleF gx_bounds = GetTextBounds();
                GX_Graphics.FillRectangle(Brushes.White, gx_bounds.Width, 0, GX_Surface.Width - gx_bounds.Width, GX_Surface.Height);
            ...
            private void DrawProgressBars()  // must be called with DWriteMutex LOCKED
            {
                ...
                if ((!_useProgressDisplay & !_useVolumeDisplay) || _isClosing) return;
                // simulate icons 
                int progLevel = 0;
                int volLevel = 0;
                float factor = 0;
                ...
                if (g_Player.Playing & _useVolumeDisplay)
                {
                    ...
                    {
                        factor = (float)(65535 / barWidth - 6);
                        volLevel = (int)(((float)DShowNET.AudioMixer.AudioMixerHelper.GetVolume() / (float)factor) - 0.01) + 1;
//                        volLevel = (DShowNET.AudioMixer.AudioMixerHelper.GetVolume() / 324);
                ...
                {
                    progLevel = (int)((((float)g_Player.CurrentPosition / (float)g_Player.Duration) - 0.01) * (barWidth - 6)) + 1;
            ...
            private enum LargeIconType : int
            {
                IDLE = 0,
                TV = 1,
                MOVIE = 2,
            ...
            private void DisplayUpdate()
            {
                ...
                while (true)
                {
                    ...
                    Current_Status = ICON_STATUS;
                    flashStatus = !flashStatus;
                    LastCurrentLargeIcon = CurrentLargeIcon;
                    LastVolLevel = volLevel;
                    LastProgLevel = progLevel;
                    NewCurrentLargeIcon = 0;
                    ICON_STATUS = 0x0;
                    Disk.Off();
                    Disk.Animate();
                    ...
                    if (g_Player.Playing)
                    {
                        ...
                        if (g_Player.IsMusic)
                        {
                            ...
                            if (!_useDiskIconForAllMedia) Disk.Off();
                        ...
                        if (g_Player.IsVideo & !g_Player.IsDVD)
                        {
                            ...
                            // determine the media type
                            currentItem = GUIPropertyManager.GetProperty("#Play.Current.File");
                            Log.Debug("current file: {0}", currentItem.ToString());
                            if (currentItem.Length > 0)
                            {
                                ICON_STATUS |= (uint)IconType.ICON_Music;
                                // there is a currently playing file - set appropriate icons
                    ...
                    if (NewCurrentLargeIcon != LastCurrentLargeIcon) CurrentLargeIcon = NewCurrentLargeIcon;
                    ...
                    lock (DWriteMutex)
                    {
                        ...
                        if (_useProgressDisplay || _useVolumeDisplay || _useIcons) ClearIconArea();
                        if (_useProgressDisplay || _useVolumeDisplay) DrawProgressBars();
                        if (_useIcons)
                        {
                            if (!g_Player.Playing || !_useDiskIconForAllMedia)
                            {
        ...
        public class AdvancedSettings
        {
            ...
            public static void Save()
            {
                ...
                XmlSerializer ser = new XmlSerializer(typeof(AdvancedSettings));
                XmlTextWriter w = new XmlTextWriter(Config.GetFile(Config.Dir.Config, "ExternalDisplay_MatrixGX.xml"), Encoding.UTF8);
        ...
        public class AdvancedSetupForm : Form
        {
            ...
            private IDisplay lcd = Settings.Instance.LCDType;
            ...
            {
                Log.Debug("IDisplay MatrixGX.AdvancedSetupForm.btnTestDisplay() clicked");
            }
