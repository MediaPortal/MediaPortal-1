        ...
        switch (msg.WParam.ToInt32())
        {
          ...
          case PowerManager.PBT_APMQUERYSUSPEND:
            ...
            if (!_singleSeat && _denyQuerySuspend)
            {
              ...
              ((IPowerScheduler)this).SuspendSystemWithOptions("System", (int)RestartOptions.Hibernate, false);
      ...
      while (_parentThread.IsAlive)
      {
        ...
        if (_singleSeat)
        {
          ...
          if (_lastUserActivitySignaled < _lastUserTime)
          {
            ...
            if (RemotePowerControl.Instance.IsConnected())
        ...
        else
        {
          ...
          try
          {
            if (RemotePowerControl.Instance.IsConnected())
            {
              ...
              ServiceAgents.Instance.PluginService<IPowerController>().UserActivityDetected(DateTime.MinValue);
            }
          }
          catch (Exception ex)
          {
    ...
    private void LoadSettings()
    {
      ...
      if (_settings == null)
      {
        using (Settings reader = new MPSettings())
        {
          ...
          // Detect single-seat
          // TODO this is fragile, needs improvement
      ...
      if (_singleSeat)
      {
        ...
        {
          // Check if PowerScheduler should actively put the system into standby
          IPowerSettings settings = ServiceAgents.Instance.PluginService<IPowerController>().PowerSettings();
          if (_settings.ShutdownEnabled != settings.ShutdownEnabled)
          {
            _settings.ShutdownEnabled = settings.ShutdownEnabled;
            Log.Debug("PS: Server plugin setting - PowerScheduler forces system to go to standby when idle: {0}", settings.ShutdownEnabled);
            changed = true;
          }
          ...
          if (_settings.ShutdownEnabled)
          {
            ...
            if (_settings.ShutdownMode != settings.ShutdownMode)
            {
              _settings.ShutdownMode = settings.ShutdownMode;
          ...
          if (_settings.IdleTimeout != settings.IdleTimeout)
          {
            _settings.IdleTimeout = settings.IdleTimeout;
            Log.Debug("PS: Server plugin setting - {0}: {1} minutes", (_settings.ShutdownEnabled ? "Standby after" : "System idle timeout"), settings.IdleTimeout);
            changed = true;
          }

          // Get configured pre-wakeup time from local tvservice
          if (_settings.PreWakeupTime != settings.PreWakeupTime)
          {
            _settings.PreWakeupTime = settings.PreWakeupTime;
            Log.Debug("PS: Pre-wakeup time: {0} seconds", settings.PreWakeupTime);
            changed = true;
          }

          // Check if PowerScheduler should wakeup the system automatically
          if (_settings.WakeupEnabled != settings.WakeupEnabled)
          {
            _settings.WakeupEnabled = settings.WakeupEnabled;
            Log.Debug("PS: Server plugin setting - Wakeup system for various events: {0}", settings.WakeupEnabled);
            changed = true;
          }
        }
        else
