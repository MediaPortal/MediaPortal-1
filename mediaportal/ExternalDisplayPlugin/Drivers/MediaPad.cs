...
namespace ProcessPlugins.ExternalDisplay.Drivers
{
  ...
  public class MediaPad : BaseDisplay, IDisplay
  {
    ...
    public void SetLine(int line, string message)
    {
      ...
      {
        Log.Warn("ExternalDisplay: Could not send text to Mediapad LCD ({0})", res.ToString());
      }
    }
    ...
    public void Setup(string port, int lines, int cols, int delay, int linesG, int colsG, int timeG, bool backLight,
                      int contrast)
    {
      Log.Info("ExternalDisplay: Found Logitech diNovo Mediapad with ID {0} and LCD ID {1}", mediaPadId, mediaPadLcdId);
    ...
    private void Clear()
    {
      ...
      if (res != Status.Success)
      {
        Log.Warn("ExternalDisplay: Could not clear Mediapad LCD ({0}", res.ToString());
      }
    }
    ...
    private void SetDisplayMode(ScreenMode mode)
    {
      ...
      if (res != Status.Success)
      {
        Log.Warn("ExternalDisplay: Could not switch Mediapad to time mode ({0})", res.ToString());
      }
    }
    ...
    private void Beep()
    {
      ...
      if (res != Status.Success)
      {
        Log.Warn("ExternalDisplay: Could not make Mediapad beep ({0}", res.ToString());
      }
      return;
    }
