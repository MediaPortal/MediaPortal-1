...
namespace MediaPortal.GUI.Radio
{
  ...
  public class GUIRadioGuideBase : GUIWindow
  {
    ...
    private string _currentRadioStation = string.Empty;
    ...
    private void Update(bool selectCurrentShow)
    {
      lock (this)
      {
        ...
        else
        {
          ...
          if (null != img)
          {
            _currentRadioStation = img.Label1;
          }
        }
        UpdateVerticalScrollbar();
      }
    ...
    private void SetProperties()
    {
      ...
      if (_cursorY == 0 || _currentProgram == null)
      {
        ...
        string strLogo = Util.Utils.GetCoverArt(Thumbs.Radio, strChannel);
      ...
      {
        string strLogo = Util.Utils.GetCoverArt(Thumbs.Radio, _currentProgram.Channel);
        ...
        {
          GUIPropertyManager.SetProperty("#Radio.Guide.EpisodeName", _currentProgram.Episode);
        }
        ...
        {
          GUIPropertyManager.SetProperty("#Radio.Guide.SeriesNumber", _currentProgram.SeriesNum);
        }
        ...
        {
          GUIPropertyManager.SetProperty("#Radio.Guide.EpisodeNumber", _currentProgram.EpisodeNum);
        }
        ...
        {
          GUIPropertyManager.SetProperty("#Radio.Guide.EpisodePart", _currentProgram.EpisodePart);
        }
        ...
        {
          GUIPropertyManager.SetProperty("#Radio.Guide.Date", _currentProgram.Date);
        }
        ...
        {
          GUIPropertyManager.SetProperty("#Radio.Guide.StarRating", _currentProgram.StarRating);
        }
        ...
        {
          GUIPropertyManager.SetProperty("#Radio.Guide.Classification", _currentProgram.Classification);
        }
    ...
    private void RenderSingleChannel(RadioStation channel)
    {
      int chan = _channelOffset;
      for (int iChannel = 0; iChannel < _channelCount; iChannel++)
      {
        if (chan < _channelList.Count)
        {
          ...
          string strLogo = Util.Utils.GetCoverArt(Thumbs.Radio, tvChan.Name);
          if (File.Exists(strLogo))
          {
    ...
    private void RenderChannel(int iChannel, RadioStation channel, long iStart, long iEnd, bool selectCurrentShow)
    {
      string strLogo = Util.Utils.GetCoverArt(Thumbs.Radio, channel.Name);
      if (File.Exists(strLogo))
      {
        GUIButton3PartControl img = GetControl(iChannel + (int) Controls.IMG_CHAN1) as GUIButton3PartControl;
        if (img != null)
    ...
    private void OnRecord(bool isItemSelected)
    {
      if (_singleChannelView)
      {
