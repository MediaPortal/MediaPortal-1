...
namespace MediaPortal.GUI.Music
{
  ...
  public class GUIMusicFiles : GUIMusicBaseWindow, ISetupForm, IShowPlugin
  {
    ...
    int GetVirtualFileTrackNumber(string strFile)
    {
      string strTrack = string.Empty;
    ...
    void OnRetrieveMusicInfo(ref List<GUIListItem> items, bool aPlaylistAdd)
    {
      int nFolderCount = 0;
          ...
          if (tag == null)
          {
            ...
            if (strExtension != ".cda")  // int_20h: changed cdda to cda.
            {
              ...
              if (!bFound)
              {
                ...
                {
                  /*
                   * If the file is virtual, read infos from the cuesheet
                   */
                  ...
                  int track_rank = GetVirtualFileTrackNumber(pItem.Path);
            ...
            {
              if (m_bScan || bCDDAFailed || !Util.Win32API.IsConnectedToInternet() || aPlaylistAdd)
                continue;
