			...
			try
			{
				...
				if(bytesCount>0 && m_currAction==(int)MailAction.MAIL_ACTION_READ_ON)
				{
					...
					if( m_recMailData.EndsWith("\r\n.\r\n")==false)
					{
						...
						m_timeOutTimer.Start();
						if (recieveData!=null)
							socket.BeginReceive( m_mailBuffer, 0, m_buffSize, noFlags, recieveData , socket);
					}
					else
					{
					...
					if(strRecieved.Substring(0,3).Equals("+OK"))
					{
						switch(m_currAction)
						{
							case (int)MailAction.MAIL_SEND_LIST:
								...
								{
									Log.Write("mymail: message number {0} is size 0",Convert.ToString(m_mailNumber));
									m_mailNumber--;
									if(m_mailNumber<=0)
									{
										m_ierrorNumber=(int)SocketError.ERROR_NO_ERROR;
								...
								break;
							case (int)MailAction.MAIL_MB_CONNECTED: // send the user and begin login
							...
							case (int)MailAction.MAIL_SEND_PERFORM: // we quit now
								...
								{
									// get the mails count 
									strRecieved=strRecieved.Replace("\r\n","");
									try
									{
...
//									m_imailCount=m_imailCount;
								...
								if(m_mailAction==(int)MailAction.MAIL_SEND_RETR) // return the mail count from the inbox
								{
