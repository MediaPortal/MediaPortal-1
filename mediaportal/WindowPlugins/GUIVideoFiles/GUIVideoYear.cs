...
namespace MediaPortal.GUI.Video
{
    ...
    MapSettings       _MapSettings = new MapSettings();
		...
		public GUIVideoYear()
		{
			GetID = (int)GUIWindow.Window.WINDOW_VIDEO_YEAR;
      LoadSettings();
    }
    ~GUIVideoYear()
    {
		...
		{
			m_strDirectory = "";
			return Load(GUIGraphicsContext.Skin + @"\myvideoYear.xml");
		}
    ...
    void LoadFolderSettings(string strDirectory)
    {
      ...
      object o;
      FolderSettings.GetFolderSetting(strDirectory,"VideoYear",typeof(GUIVideoYear.MapSettings), out o);
    ...
    {
      if (strDirectory=="") strDirectory="root";
      FolderSettings.AddFolderSetting(strDirectory,"VideoYear",typeof(GUIVideoYear.MapSettings), _MapSettings);
    }
      ...
      if (strNewDirectory != m_strDirectory || _MapSettings==null) 
      {
        ...
        Utils.SetDefaultIcons(pItem);
				...
				VideoDatabase.GetMoviesByYear(m_strDirectory, ref movies);
      ...
      if ( GUIVideoFiles.ShowIMDB(iMovieId,strFile, strFilePath, "" ,false))
      {
				...
				if (!pItem.IsFolder)
				{
					for (int i = 0; i < movies.Count; ++i)
					{
						...
						if (strFile.Length > 0)
						{
							...
							{
								string strThumb;
								strThumb = Utils.GetCoverArt(ThumbsFolder,info.IMDBNumber);
                if (System.IO.File.Exists(strThumb))
                {
		...
		void OnPlayDVD()
		{
			Log.Write("GUIVideoYear playDVD");
      g_Player.PlayDVD();
		}
