...

{
	...
	/// 
	/// </summary>
	public class GUIVideoGenres : GUIWindow, IComparer
  {
    [Serializable]
    ...
    static public string VideoGenreArt=@"thumbs\videos\genre";
    
    public GUIVideoGenres()
    {
      GetID = (int)GUIWindow.Window.WINDOW_VIDEO_GENRE;
    ...
    {
      m_strDirectory = "";
      return Load(GUIGraphicsContext.Skin + @"\myVideogenre.xml");
    }
    ...
    void LoadFolderSettings(string strDirectory)
    {
      ...
      object o;
      FolderSettings.GetFolderSetting(strDirectory,"VideoGenre",typeof(GUIVideoGenres.MapSettings), out o);
    ...
    {
      if (strDirectory=="") strDirectory="root";
      FolderSettings.AddFolderSetting(strDirectory,"VideoGenre",typeof(GUIVideoGenres.MapSettings), _MapSettings);
    }
    ...
    void LoadDirectory(string strNewDirectory)
    {
      ...
      if (m_strDirectory == "")
      {
        ...
        {
          GUIListItem item = new GUIListItem((string)genres[i]);
          item.Path = ((string)genres[i]).Trim();
          item.IsFolder = true;


          string strThumb = Utils.GetCoverArt(VideoGenreArt,item.Label);
    ...
    void SetIMDBThumbs(ArrayList items)
    {
	  ...
	  void OnPlayDVD()
		{
			Log.Write("GUIVideoGenres playDVD");
      g_Player.PlayDVD();
    }
