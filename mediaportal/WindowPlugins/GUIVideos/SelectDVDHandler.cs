        ...
        for (int x = 0; x < items.Count; x++)
        {
          ...
          if (!pItem.IsFolder || pItem.IsBdDvdFolder)
          {
            ...
            if (pItem.IsBdDvdFolder)
            {
              if (!isBluRay)
              {
                pItem.Label3 = GUIVideoBaseWindow.MediaTypes.DVD + " " + percentWatched + "% #" + timesWatched;
                pItem.Label2 = string.Empty;
              }
              else
              {
                pItem.Label3 = GUIVideoBaseWindow.MediaTypes.BD + " " + percentWatched + "% #" + timesWatched;
                pItem.Label2 = string.Empty;
              }
            }
            else if (VirtualDirectory.IsImageFile(Util.Utils.GetFileExtension(pItem.Path)))
            {
              pItem.Label3 = GUIVideoBaseWindow.MediaTypes.ISO + " " + percentWatched + "% #" + timesWatched;
            }
            else
            {
          ...
          {
            Util.Utils.SetThumbnails(ref pItem);
          }
