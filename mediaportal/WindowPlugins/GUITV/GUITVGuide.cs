  ...
  public class GUITVGuide : GUIWindow
  {
    ...
    void Update()
    {
      ...
      for (int iLabel=0; iLabel <4; iLabel++)
      {
        ...
        this.Remove((int)Controls.LABEL_TIME1+iLabel);
        
        this.Remove((int)Controls.IMG_TIME1+iLabel);
        
        GUIImage img = new GUIImage(GetID,(int)Controls.IMG_TIME1+iLabel,xpos+2,ypos,iLabelWidth-4,cntlHeaderBkgImg.RenderHeight,cntlHeaderBkgImg.FileName,0x0);
        img.IsVisible=!m_bSingleChannel;
        img.AllocResources();
        GUIControl cntl2=(GUIControl)img;
        Add(ref cntl2);

        GUILabelControl label = new GUILabelControl(GetID,(int)Controls.LABEL_TIME1+iLabel,xpos,ypos,iLabelWidth,cntlHeaderBkgImg.RenderHeight,labelTime.FontName, "",labelTime.TextColor, GUIControl.Alignment.ALIGN_CENTER,false);
      ...
      for (int iChan=0; iChan <m_iChannels; ++iChan)
      {
        ...
        GUIButton3PartControl imgBut = new GUIButton3PartControl(GetID,(int)Controls.IMG_CHAN1+iChan,xpos,ypos,labelTime.XPosition-cntlChannelImg.XPosition,iItemHeight-2,
        ...
        //GUIImage img = new GUIImage(GetID,(int)Controls.IMG_CHAN1+iChan,xpos,ypos,cntlChannelImg.RenderWidth,cntlChannelImg.RenderHeight,cntlChannelImg.FileName,0x0);
        ...
        //this.Remove(iChan+(int)Controls.IMG_CHAN1_LABEL);
        //ypos=cntlChannelImg.YPosition+iChan*iItemHeight+cntlChannelImg.RenderHeight;
        //GUILabelControl label = new GUILabelControl(GetID,(int)Controls.IMG_CHAN1_LABEL+iChan,xpos,ypos,0,0,cntlChannelLabel.FontName, "",cntlChannelLabel.TextColor, cntlChannelLabel.TextAlignment,false);
        //label.AllocResources();
        //GUIControl cntl2=(GUIControl)label;
        //this.Add(ref cntl2);
      ...
      if (scrollbar!=null)
      {
        float fPercent=(float)m_dtTime.Hour*60+m_dtTime.Minute;
        fPercent/=(24.0f*60.0f);
        fPercent*=100.0f;
        if ( (int)fPercent != (int)scrollbar.Percentage )
        {
          scrollbar.Percentage=fPercent;
        }
      }
      ...
      for (int i=0; i <m_iBlocks; i++)
      {
        ...
        GUIControl.SetControlLabel(GetID,i+(int)Controls.LABEL_TIME1,strTime);
      ...
      for (int iChannel=0; iChannel <m_iChannels; iChannel++)
      {
        for (int iProgram=0; iProgram <m_iBlocks*5; ++iProgram)
        {
          ...
          Remove(iControlId);   //bar
          Remove(iControlId+1); //label
          Remove(iControlId+2); //> after
          Remove(iControlId+3); //genre
        }
      }
      if (m_bSingleChannel)
      {
    ...
    void RenderSingleChannel(TVChannel channel)
    {
      ...
      for (int ichan=0; ichan < m_iChannels;++ichan)
      {
        ...
        if (ichan < programs.Count)
          program=(TVProgram)programs[ichan];
    ...
    int ProgramCount(int iChannel)
    {
      ...
      {
        int iControlId=100+iChannel*100+iProgram*10;
        if ( GetControl(iControlId)!=null) iProgramCount++;
    ...
    void Correct()
    {
      ...
      if (m_iCursorX>0)
      {
        while (m_iCursorX>0)
        {
          ...
          if (GetControl(iControlId)==null) m_iCursorX--;
          else break;
        }
      }
      if (m_iCursorY<0)
      ...
      if (!m_bSingleChannel)
      {
        while (m_iCursorY>0)
        {
          ...
          if (GetControl(iControlId)==null) m_iCursorY--;
          else break;
        }
      }
    }
    ...
    void GetChannels()
    {
      ...
      do
      {
        ...
        for (int iChan=0; iChan < m_channels.Count;++iChan)
        {
          ...
          if ( (chan.Number>= 1000 && !chan.External) || !chan.VisibleInGuide)
          {
