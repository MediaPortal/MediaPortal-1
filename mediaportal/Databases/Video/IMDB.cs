  ...
  public class IMDB : IEnumerable
  {
    ...
    private void FindIMDB(string strURL, int iLimit)
    {
      ...
      try
      {
        ...
        if (iStartOfMovieList < 0) iStartOfMovieList = strBody.IndexOf("Popular Titles");
    ...
    private string ParseListOFDB(string strIn, string strSep)
    {
      ...
      if (strIn.Length == 0)
        return "";
      // check the length of input in endless loop
      int iOldLength = strIn.Length;
      // temp for output
      string strTemp = "";
      while (true)
      {
        ...
        // Reached the end? 
        if ((strIn == "</b>") || (strIn.Length == iOldLength))
        {
          break;
        }
        // check length
        iOldLength = strIn.Length;
      }
      ...
      try
      {
        ...
        while ((true) && (iCount < iLimit))
        {
          ...
          if (iStart == -1 || iStart == 0)
            throw new ApplicationException("Parsing failed for FRDB !");
