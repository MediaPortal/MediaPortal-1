...
namespace MediaPortal.TV.Recording
{
  public class ConflictManager
  {
    ...
    private static List<TVRecording> _recordings;
    private static TVUtil _util = null;
    private static List<TVRecording> _conflictingRecordings = null;
    ...
    {
      TVDatabase.OnRecordingsChanged += new TVDatabase.OnRecordingChangedHandler(TVDatabase_OnRecordingsChanged);
    }
    ...
    private static bool DetermineIsConflict(TVRecording rec)
    {
      ...
      {
        _cardrecordings[i] = new List<TVRecording>();
      }
      ...
      foreach (TVRecording episode in episodes)
      {
        ...
        foreach (TVRecording otherRecording in _recordings)
        {
          ...
          {
            if (otherEpisode.Canceled != 0)
            {
              continue;
            }
            if (isSameRecordings(episode, otherEpisode))
            {
              continue;
            }
    ...
    private static bool AssignRecToCard(TVRecording arec, List<TVRecording>[] cardrec)
    {
      ...
      for (_currentcardNo = 0; _currentcardNo < _cardscount; _currentcardNo++)
      {
        ...
        {
          if (IsOverlap(_inrec, arec) && !isSameRecordings(_inrec, arec))
          {
            _free = false;
          }
    ...
    private static void freeCardsRecordings(List<TVRecording>[] cardrec)
    {
    ...
    /// <returns>Nothing</returns>
    private static bool isSameRecordings(TVRecording record_1, TVRecording record_2)
    {
      if (record_2.ID == record_1.ID &&
          record_2.Start == record_1.Start &&
          record_2.End == record_1.End)
      {
        return true;
      }
      return false;
