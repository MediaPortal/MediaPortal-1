...
namespace MediaPortal.TV.Recording
{
  public class Scheduler
  {
    ...
    {
      _log.Info("Scheduler:Recordings changed");
    ...
    public void Process(CommandProcessor handler)
    {
      ...
      {
        _log.Info("Scheduler:reload recordings");
              ...
              else
              {
                ...
                if (!rec.IsAnnouncementSend)
                {
                  ...
                  if (rec.IsRecordingProgramAtTime(dtTime, prog2Min, paddingFront, paddingEnd))
                  {
                    ...
                    _log.Info("Recorder: Send announcement for recording:{0}", rec.ToString());
    ...
    bool Record(CommandProcessor handler,  DateTime currentTime, TVRecording rec, TVProgram currentProgram, int iPreRecordInterval, int iPostRecordInterval)
    {
      ...
      _log.Info("Recorder: time to record '{0}' on channel:{1} from {2}-{3} id:{4} priority:{5} quality:{6} {7}", rec.Title, rec.Channel, rec.StartTime.ToLongTimeString(), rec.EndTime.ToLongTimeString(), rec.ID, rec.Priority, rec.Quality.ToString(), rec.RecType.ToString());
      if (currentProgram != null)
      {
        _log.Info("Recorder: program:{0}-{1}", currentProgram.StartTime.ToLongTimeString(), currentProgram.EndTime.ToLongTimeString());
      }
      ...
      if (cardNo < 0)
      {
        ...
        _log.Info("Recorder:  No card found, check if a card is recording a show which has a lower priority then priority:{0}", rec.Priority);
        cardNo = FindFreeCardForRecording(handler,rec.Channel, true, rec.Priority);
        if (cardNo < 0)
        {
          //no, other recordings have higher priority...
          _log.Info("Recorder:  no recordings have a lower priority then priority:{0}", rec.Priority);
          return false;
        }
      }
      ...
      if (cardNo < 0)
      {
        ...
        for (int i = 0; i < handler.TVCards.Count; i++)
        {
          ...
          string strLogo = MediaPortal.Util.Utils.GetCoverArt(Thumbs.TVChannel, dev.CurrentTVRecording.Channel);
        ...
        if (count > 0)
        {
          ...
          if (pDlgOK.TimedOut)
          {
            ...
            _log.Info("Recorder: Canceled recording:{0} priority:{1} on card:{2}",
            ...
            if (selectedIndex >= 0)
            {
              for (int i = 0; i < handler.TVCards.Count; ++i)
              {
                ...
                if (dev.IsRecording)
                {
                  ...
                  {
                    cardNo = i;
                    _log.Info("Recorder: User canceled recording:{0} priority:{1} on card:{2}",
                      dev.CurrentTVRecording.ToString(),
                      dev.CurrentTVRecording.Priority,
                      dev.CommercialName);
                    StopRecording(handler,dev.CurrentTVRecording);
        ...
        if (cardNo < 0)
        {
          _log.Info("Recorder:  no card available for recording");
      ...
      _log.Info("Recorder:  using card:{0} prio:{1}", card.CommercialName, card.Priority);
      if (card.IsRecording)
      {
        //if its recording, we cancel it 
        _log.Info("Recorder:  card:{0} was recording. Now use it for recording new program", card.CommercialName);
        _log.Info("Recorder: Stop recording card:{0} channel:{1}", card.CommercialName, card.TVChannel);
