  ...
  public class DVBGraphTTPremium : DVBGraphBase
  {
    ...
    public override bool CreateGraph(int Quality)
    {
      try
      {
        ...
        Log.WriteFile(Log.LogType.Capture, "DVBGraphTTPremium:CreateGraph(). ");

        //no card defined? then we cannot build a graph
        if (_card == null)
        {
          Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:card is not defined");
          return false;
        }
        ...
        // Log.WriteFile(Log.LogType.Capture,"DVBGraphTTPremium:create new filter graph (IGraphBuilder)");
        _graphBuilder = (IGraphBuilder)new FilterGraph();

        _captureGraphBuilderInterface = (ICaptureGraphBuilder2)new CaptureGraphBuilder2();

        //Log.WriteFile(Log.LogType.Capture,"DVBGraphTTPremium:Link the CaptureGraphBuilder to the filter graph (SetFiltergraph)");
        int hr = _captureGraphBuilderInterface.SetFiltergraph(_graphBuilder);
        if (hr < 0)
        {
          Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:FAILED link :0x{0:X}", hr);
          return false;
        }
        ...
        //=========================================================================================================
        Log.WriteFile(Log.LogType.Capture, "DVBGraphTTPremium:CreateGraph() create TTPremium source filter");
        ...
        {
          Log.WriteFile(Log.LogType.Capture, "DVBGraphTTPremium:creategraph() _filterTTPremium not found");
          return false;
        }
        Log.WriteFile(Log.LogType.Capture, "DVBGraphTTPremium:creategraph() add filters to graph");
        hr = _graphBuilder.AddFilter(_filterTTPremium, "TTPremiumSource");
        if (hr != 0)
        {
          Log.WriteFile(Log.LogType.Capture, "DVBGraphTTPremium: FAILED to add TTPremium source filter to graph");
          return false;
        }
        ...
        {
          Log.WriteFile(Log.LogType.Capture, "DVBGraphTTPremium: cannot get ITTPremiumSource");
          return false;
        }
        ...
        // initialize tuner
        //=========================================================================================================
        Log.WriteFile(Log.LogType.Capture, "DVBGraphTTPremium: Initialize Tuner()");
        hr = _interfaceTTPremium.Init();
        if (hr != 0)
        {
          Log.WriteFile(Log.LogType.Capture, "DVBGraphTTPremium: Tuner initialize failed:0x{0:X}", hr);
          return false;
        }
        ...
        if (hr != 0)
        {
          Log.WriteFile(Log.LogType.Capture, "DVBGraphTTPremium: Network Type failed:0x{0:X}", hr);
          return false;
        }
        ...
        */
        ...
        {
          Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:Failed to get Mpeg2 Demultiplexer");
          return false;
        }
        ...
        {
          Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:FAILED to create sections pin:0x{0:X}", hr);
          return false;
        }

        Log.WriteFile(Log.LogType.Capture, "DVBGraphTTPremium: create audio/video output pin");
        ...
        if (hr != 0)
        {
          Log.WriteFile(Log.LogType.Capture, "DVBGraphTTPremium:FAILED to create video output pin on demuxer");
          return false;
        }
        hr = demuxer.CreateOutputPin(mpegAudioOut, "audio", out filterMpeg2DemuxerAudioPin);
        if (hr != 0)
        {
          Log.WriteFile(Log.LogType.Capture, "DVBGraphTTPremium: FAILED to create audio output pin on demuxer");
          return false;
        }
        ...
        if (hr != 0)
        {
          Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium: FAILED to add SectionsFilter 0x{0:X}", hr);
          return false;
        }
        ...
        if (GUIGraphicsContext.DX9Device != null && _sampleInterface != null)
        {
          ...
          if (pinData0 == null)
          {
            Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:Failed to get pin 'output' from TTPremium source");
            return false;
          }
          ...
          if (pinIn == null)
          {
            Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:Failed to get input pin from sample grabber");
            return false;
          }
          ...
          if (hr != 0)
          {
            Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:Failed to connect TTPremium->sample grabber");
            return false;
          }
          ...
          {
            Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:Failed to connect samplegrabber filter->mpeg2 demultiplexer");
            return false;
          }*/
        }
        ...
        //=========================================================================================================
        Log.WriteFile(Log.LogType.Capture, "DVBGraphTTPremium:CreateGraph() find audio/video pins");
        ...
        {
          //video pin not found
          Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:Failed to get pin (video out) from MPEG-2 Demultiplexer", _pinDemuxerVideo);
          return false;
        }
        if (_pinDemuxerAudio == null)
        {
          //audio pin not found
          Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:Failed to get pin (audio out)  from MPEG-2 Demultiplexer", _pinDemuxerAudio);
          return false;
        }
        ...
        //=========================================================================================================
        Log.WriteFile(Log.LogType.Capture, "DVBGraphTTPremium:CreateGraph() create ac3/mpg1 pins");
        if (demuxer != null)
        {
          ...
          {
            Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:FAILED to create AC3 pin:0x{0:X}", hr);
          }
          ...
          {
            Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:FAILED to create MPG1 pin:0x{0:X}", hr);
          }
          ...
          {
            Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:FAILED to create EPG pin:0x{0:X}", hr);
            return false;
          }
          hr = demuxer.CreateOutputPin(mtEPG, "MHW1", out _pinDemuxerMHWd2);
          if (hr != 0 || _pinDemuxerMHWd2 == null)
          {
            Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:FAILED to create MHW1 pin:0x{0:X}", hr);
            return false;
          }
          hr = demuxer.CreateOutputPin(mtEPG, "MHW2", out _pinDemuxerMHWd3);
          if (hr != 0 || _pinDemuxerMHWd3 == null)
          {
            Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:FAILED to create MHW2 pin:0x{0:X}", hr);
            return false;
          }
          ...
          {
            Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:FAILED to get Input pin on MSPA");
            return false;
          }
          ...
          if (pinMHW1In == null)
          {
            Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:FAILED to get MHW1 pin on MSPA");
            return false;
          }
          IPin pinMHW2In = DsFindPin.ByDirection(_filterDvbAnalyzer, PinDirection.Input, 2);
          if (pinMHW2In == null)
          {
            Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:FAILED to get MHW2 pin on MSPA");
            return false;
          }
          IPin pinEPGIn = DsFindPin.ByDirection(_filterDvbAnalyzer, PinDirection.Input, 3);
          if (pinEPGIn == null)
          {
            Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:FAILED to get EPG pin on MSPA");
            return false;
          }
          ...
          {
              Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:FAILED to connect mpeg2 demux->stream analyzer");
          }

          hr = _graphBuilder.Connect(_pinDemuxerEPG, pinEPGIn);
          if (hr != 0)
          {
            Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:FAILED to connect EPG pin:0x{0:X}", hr);
            return false;
          }
          hr = _graphBuilder.Connect(_pinDemuxerMHWd2, pinMHW1In);
          if (hr != 0)
          {
            Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:FAILED to connect MHW1 pin:0x{0:X}", hr);
            return false;
          }
          hr = _graphBuilder.Connect(_pinDemuxerMHWd3, pinMHW2In);
          if (hr != 0)
          {
            Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:FAILED to connect MHW2 pin:0x{0:X}", hr);
            return false;
          }
          ...
          if (GUIGraphicsContext.DX9Device != null)
          {
            ...
            {
              Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:FAILED to create ttx pin:0x{0:X}", hr);
              return false;
            }
            ...
            if (pinIn == null)
            {
              Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:unable to find sample grabber input:0x{0:X}", hr);
              return false;
            }
            hr = _graphBuilder.Connect(_pinTeletext, pinIn);
            if (hr != 0)
            {
              Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:FAILED to connect demux->sample grabber:0x{0:X}", hr);
              return false;
            }
          }
        }
        ...
        {
          Log.WriteFile(Log.LogType.Capture, true, "DVBGraphTTPremium:mapped IMPEG2Demultiplexer not found");
        }
    ...
    public override void DeleteGraph()
    {
      try
      {
        ...
        Log.WriteFile(Log.LogType.Capture, "DVBGraphTTPremium:DeleteGraph()");
        StopRecording();
        StopTimeShifting();
        StopViewing();
        //Log.WriteFile(Log.LogType.Capture,"DVBGraphTTPremium: free tuner interfaces");
        ...
        if (_videoWindowInterface != null)
        {
          ...
          //Log.WriteFile(Log.LogType.Capture,"DVBGraphTTPremium: hide video window");
          _videoWindowInterface.put_Visible(OABool.False);
          //_videoWindowInterface.put_Owner(IntPtr.Zero);
          _videoWindowInterface = null;
        }

        //Log.WriteFile(Log.LogType.Capture,"DVBGraphTTPremium: free other interfaces");
        _sampleInterface = null;
        ...
        //Log.WriteFile(Log.LogType.Capture,"DVBGraphTTPremium: remove filters");
        ...
        //Log.WriteFile(Log.LogType.Capture,"DVBGraphTTPremium: remove graph");
        ...
        Thread.Sleep(200);
        //Log.WriteFile(Log.LogType.Capture,"DVBGraphTTPremium: delete graph done");
      }
      catch (Exception ex)
      {
        Log.Write(ex);
