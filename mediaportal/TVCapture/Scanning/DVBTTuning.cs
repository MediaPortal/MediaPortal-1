...
namespace MediaPortal.TV.Scanning
{
  ...
  public class DVBTTuning : ITuning
  {
    ...
    private ArrayList _listFrequencies = new ArrayList();
    ...
    private bool _channelsFound = false;
    ...
    {
      if (_currentFrequencyIndex >= _listFrequencies.Count)
      {
        return true;
      }
      return false;
    ...
    private void DetectAvailableStreams()
    {
      ...
      if (_currentFrequencyIndex < 0 || _currentFrequencyIndex >= _listFrequencies.Count)
      {
        return;
      }
      int[] tmp;
