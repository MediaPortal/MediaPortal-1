  ...
  public class DVBEPG
  {
    public class EITComparer : IComparer
    {
      ...
      public int Compare(object x, object y)
      {
        ...
        //next on start time
        DateTime date1 = new DateTime(desc1.starttime_y, desc1.starttime_m, desc1.starttime_d, desc1.starttime_hh,
                                      desc1.starttime_mm, desc1.starttime_ss);
        DateTime date2 = new DateTime(desc2.starttime_y, desc2.starttime_m, desc2.starttime_d, desc2.starttime_hh,
                                      desc2.starttime_mm, desc2.starttime_ss);
        if (date1 < date2)
        {
          return -1;
        }
    ...
    private DVBSections m_sections = new DVBSections();
    private int m_cardType = 0;
    private string m_channelName = "";
    private string m_languagesToGrab = "";
    private ArrayList m_titleBuffer = new ArrayList();
    private ArrayList m_namesBuffer = new ArrayList();
    private ArrayList m_themeBuffer = new ArrayList();
    private ArrayList m_summaryBuffer = new ArrayList();
    private NetworkType m_networkType;
    private ArrayList m_streamBuffer = new ArrayList();
    private int m_addsToDatabase = 0;
    private int m_mhwChannelsCount = 0;
    private bool m_titlesParsing = false;
    private bool m_summaryParsing = false;
    private bool m_channelsParsing = false;
    private int m_channelGrabLen = 0;
    private MPLabel m_mhwChannels;
    private MPLabel m_mhwTitles;
    ...
    {
      get { return m_languagesToGrab; }
      set { m_languagesToGrab = value; }
    }
    ...
    public MPLabel LabelChannels
    {
      set { m_mhwChannels = value; }
    }

    public MPLabel LabelTitles
    {
                                ...
                                out DateTime dateProgramEnd)
    {
      try
      {
        ...
        try
        {
          datestart = new DateTime(data.starttime_y, data.starttime_m, data.starttime_d, data.starttime_hh,
                                   data.starttime_mm, data.starttime_ss);
        }
        catch
        {
          return 0;
    ...
    public int GetEPG(ArrayList epgData, int serviceID, out DateTime reGrabTime)
    {
              ...
              foreach (TVChannel chan in channels)
              {
                switch (m_networkType)
                {
                  case NetworkType.DVBC:
                    TVDatabase.GetDVBCTuneRequest(chan.ID, out provider, out freq, out symbolrate, out innerFec,
                                                  out modulation, out ONID, out TSID, out SID, out audioPid,
                                                  out videoPid, out teletextPid, out pmtPid, out audio1, out audio2,
                                                  out audio3, out ac3Pid, out audioLanguage, out audioLanguage1,
      ...
      foreach (DVBSections.EITDescr eit in eitList)
      {
        ...
        if (!tableChannels.ContainsKey(id))
        {
          continue;
        }
        string progName = (string) tableChannels[id];
        DVBSections.EITDescr eit2DB = new DVBSections.EITDescr();
        eit2DB = eit;
    ...
    public void ParseChannels(byte[] data)
    {
      ...
      lock (m_namesBuffer.SyncRoot)
      {
        for (int n = 4; n < dataLen; n += 22)
        {
          ...
          ch.ChannelName = Encoding.ASCII.GetString(data, n + 6, 16);
      ...
      lock (m_themeBuffer.SyncRoot)
      {
        try
        {
          ...
          for (int i = 0; i < count; i++)
          {
            ...
            th.ThemeText = Encoding.ASCII.GetString(data, themesNames, 15);
    ...
    public void ParseSummaries(byte[] data)
    {
      ...
      lock (m_summaryBuffer.SyncRoot)
      {
        ...
        sum.Description = "";
        n += 11 + (data[n + 10]*7);
        sum.Description = Encoding.ASCII.GetString(data, n, dataLen - n);
    ...
    public void ParseTitles(byte[] data)
    {
      ...
      lock (m_titleBuffer.SyncRoot)
      {
        ...
        prg.Title = Encoding.ASCII.GetString(data, 11, 23);
        ...
        prg.Time = new DateTime(DateTime.Now.Ticks);
    ...
    private int SubmittMHW(out DateTime reGrabTime)
    {
      ...
      lock (m_titleBuffer.SyncRoot)
      {
        ...
        {
          DVBSections.EITDescr eit = new DVBSections.EITDescr();
            ...
            case (int) EPGCard.BDACards:
              {
                ...
                foreach (TVChannel chan in channels)
                {
                  switch (m_networkType)
                  {
                    ...
                    case NetworkType.DVBT:
                      TVDatabase.GetDVBTTuneRequest(chan.ID, out provider, out freq, out ONID, out TSID, out SID,
                                                    out audioPid, out videoPid, out teletextPid, out pmtPid,
                                                    out bandWidth, out audio1, out audio2, out audio3, out ac3Pid,
                                                    out audioLanguage, out audioLanguage1, out audioLanguage2,
                                                    out audioLanguage3, out HasEITPresentFollow, out HasEITSchedule,
                                                    out pcrPid);
                      if (programID == SID && tsID == TSID)
                      {
                        channelName = chan.Name;
    ...
    //
    private string GetSummaryByPrgID(int id)
    {
      if (m_summaryBuffer == null)
      {
        return "";
    ...
    private string GetThemeText(int themeID)
    {
      if (m_themeBuffer == null)
      {
    ...
    private bool ProgramExists(int prgID)
    {
      lock (m_titleBuffer.SyncRoot)
      {
        foreach (Programm prg in m_titleBuffer)
    ...
    private bool SummaryExists(int prgID)
    {
      lock (m_summaryBuffer.SyncRoot)
      {
        foreach (Summary sum in m_summaryBuffer)
