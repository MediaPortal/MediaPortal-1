...
namespace MediaPortal.TV.Recording
{
  ...
  public class DVBGraphBDA : MediaPortal.TV.Recording.IGraph
  {
    ...
    private static extern bool GetPidMap(DirectShowLib.IPin filter, ref uint pid, ref uint mediasampletype);
    ...
    public bool CreateGraph(int Quality)
    {
      try
      {
        ...
        {
          Log.WriteFile(Log.LogType.Capture, "DVBGraphBDA: Loading card definitions for card {0} failed", _card.CaptureName);
          return false;
        }
    ...
    public bool ShouldRebuildGraph(TVChannel newChannel)
    {
      ...
      {
        Log.Write("DVBGraphBDA: ShouldRebuildGraph({0})  false, not viewing", newChannel.Name);
        return false;
      }
      ...
      Log.Write("DVBGraphBDA: ShouldRebuildGraph({0})  current ac3:{1} new channel ac3:{2}",
