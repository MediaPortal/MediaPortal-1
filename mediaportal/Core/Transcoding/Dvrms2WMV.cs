...
namespace MediaPortal.Core.Transcoding
{
  ...
  public class TranscodeToWMV : ITranscode
  {
    ...
    bool AddWmAsfWriter(string fileName, Quality quality, Standard standard)
    {
      ...
      switch (quality)
      {
        case Quality.HiDef:
          ...
          Log.Info("DVR2WMV: set WMV HiDef quality profile {0}", strprofileType);
          break;
        case Quality.VeryHigh:
          ...
          Log.Info("DVR2WMV: set WMV Very High quality profile {0}", strprofileType);
          break;
        case Quality.High:
          if (standard == Standard.Film)
          {
          ...
          Log.Info("DVR2WMV: set WMV High quality profile {0}", strprofileType);
          break;
        case Quality.Medium:
          if (standard == Standard.Film)
          {
          ...
          Log.Info("DVR2WMV: set WMV Medium quality profile {0}", strprofileType);
          break;
        case Quality.Low:
          if (standard == Standard.Film)
          {
          ...
          Log.Info("DVR2WMV: set WMV Low quality profile {0}", strprofileType);
          break;
        case Quality.Portable:
          ...
          Log.Info("DVR2WMV: set WMV Portable quality profile {0}", strprofileType);
          break;
        case Quality.Custom:
          //load custom profile
          ...
          Log.Info("DVR2WMV: custom bitrate = {0}", customBitrate);
          ...
          Log.Info("DVR2WMV: set WMV Custom quality profile {0}", strprofileType);
          break;
      }
      ...
      if (hr != 0)
      {
        Log.Info("DVR2WMV: get WMV profile - FAILED! {0}", hr);
        Cleanup();
        return false;
      }
      Log.Info("DVR2WMV: load profile - SUCCESS!");
      //configures the WM ASF Writer to the chosen profile
      hr = config.ConfigureFilterUsingProfile(profile);
      if (hr != 0)
      {
        Log.Info("DVR2WMV: configure profile - FAILED! {0}", hr);
        Cleanup();
        return false;
      }
      Log.Info("DVR2WMV: configure profile - SUCCESS!");
    ...
    void SetCutomProfile(int vidbitrate, int audbitrate, int vidheight, int vidwidth, double fps)
    {
      ...
      Log.Info("DVR2WMV: profile name {0}", profileName.ToString());
      //get the profile description
      hr = profile.GetDescription(profileDescription, ref profileDescLen);
      Log.Info("DVR2WMV: profile description {0}", profileDescription.ToString());
