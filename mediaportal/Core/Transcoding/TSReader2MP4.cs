...
namespace MediaPortal.Core.Transcoding
{
    ...
    public Transcode2MP4()
		...
		public bool Transcode(TranscodeInfo info, MediaPortal.Core.Transcoding.VideoFormat format, MediaPortal.Core.Transcoding.Quality quality, Standard standard)
		{
			...
			{
				Log.Info("Transcode2MP4: wrong file format");
				return false;
			}
			try
			{
				...
				Log.Info("Transcode2MP4: add filesource");
        ...
        Log.Info("Transcode2MP4: load file:{0}", info.file);
        ...
        Log.Info("Transcode2MP4: find tsreader compatible audio/video decoders");
        ...
        {
          Log.Error("Transcode2MP4: FAILED: unable to get output pins of tsreader");
          Cleanup();
          return false;
        }
        bool usingAAC = false;
        ...
        {
          AMMediaType mediaTypes = new AMMediaType();
          if (mediaTypes.majorType == MediaType.Audio && mediaTypes.subType == MediaSubType.LATMAAC)
          {
            usingAAC = true;
          }
        }
        ...
        {
          AMMediaType mediaTypes = new AMMediaType();
          if (mediaTypes.majorType == MediaType.Video && mediaTypes.subType == MediaSubType.H264)
          {
            usingH264 = true;
          }
        }
        //Add the type of decoder required for the output video & audio pins on TSReader.
        Log.Info("Transcode2MP4: add audio/video decoders to graph");
        if (usingH264 == false)
        {
          Log.Info("Transcode2MP4: add mpeg2 video decoder:{0}", strVideoCodec);
          VideoCodec = DirectShowUtil.AddFilterToGraph(graphBuilder, strVideoCodec);
          if (VideoCodec == null)
          {
            Log.Error("Transcode2MP4: unable to add mpeg2 video decoder");
            Cleanup();
            return false;
          }
        }
        ...
        {
          Log.Info("Transcode2MP4: add h264 video codec:{0}", strH264VideoCodec);
          VideoCodec = DirectShowUtil.AddFilterToGraph(graphBuilder, strH264VideoCodec);
          if (VideoCodec == null)
          {
            Log.Error("Transcode2MP4: FAILED:unable to add h264 video codec");
            Cleanup();
            return false;
          }
        }
        if (usingAAC == false)
        {
          Log.Info("Transcode2MP4: add mpeg2 audio codec:{0}", strAudioCodec);
          AudioCodec = DirectShowUtil.AddFilterToGraph(graphBuilder, strAudioCodec);
          if (AudioCodec == null)
          {
            Log.Error("Transcode2MP4: FAILED:unable to add mpeg2 audio codec");
            Cleanup();
            return false;
          }
        }
        ...
        {
          Log.Info("Transcode2MP4: add aac audio codec:{0}", strAACAudioCodec);
          AudioCodec = DirectShowUtil.AddFilterToGraph(graphBuilder, strAACAudioCodec);
          if (AudioCodec == null)
          {
            Log.Error("Transcode2MP4: FAILED:unable to add aac audio codec");
            Cleanup();
            return false;
          }
        }
        Log.Info("Transcode2MP4: connect tsreader->audio/video decoders");
        ...
        {
          Log.Error("Transcode2MP4: FAILED: unable to get pins of video/audio codecs");
          Cleanup();
          return false;
        }
        ...
        if (hr != 0)
        {
          Log.Error("Transcode2MP4: FAILED: unable to connect audio pins :0x{0:X}", hr);
          Cleanup();
          return false;
        }
        ...
        if (hr != 0)
        {
          Log.Error("Transcode2MP4: FAILED: unable to connect video pins :0x{0:X}", hr);
          Cleanup();
          return false;
        }
        ...
        mediaSeeking = tsreaderSource as IMediaSeeking;
				...
				//get file duration
        Log.Info("Transcode2MP4: Get duration of recording");
				long lTime = 5 * 60 * 60;
				lTime *= 10000000;
				long pStop = 0;
        ...
        Log.Info("Transcode2MP4: recording duration: {0}", MediaPortal.Util.Utils.SecondsToHMSString((int)duration));
				//run the graph to initialize the filters to be sure
        hr = mediaControl.Run();
				if (hr != 0)
				{
          Log.Error("Transcode2MP4: FAILED: unable to start graph :0x{0:X}", hr);
					Cleanup();
					return false;
				}
				int maxCount = 20;
        ...
        Log.Info("Transcode2MP4: start transcoding");
        //setup flow control
        ...
        //So we only parse decoder output whent the encoders are ready.
          ...
          Log.Error("Transcode2MP4: FAILED:unable to start graph :0x{0:X}", hr);
			...
			{
        Log.Error("Transcode2MP4: Unable create graph: {0}", ex.Message);
				Cleanup();
				return false;
			}
			return true;
		...
		void Cleanup()
		{
      Log.Info("Transcode2MP4: cleanup");
			if (_rotEntry != null)
			{
				_rotEntry.Dispose();
			}
      ...
      int hr;
      Log.Info("Transcode2MP4: add h264 video encoder to graph");
      ...
      if (h264Encoder == null)
			{
        Log.Error("Transcode2MP4: FAILED: Unable to create h264 video encoder");
				Cleanup();
				return false;
			}
      hr = graphBuilder.AddFilter(h264Encoder, "h264 video encoder");
			if (hr != 0)
			{
        Log.Error("Transcode2MP4: FAILED: h264 video encoder to filtergraph :0x{0:X}", hr);
				Cleanup();
				return false;
			}
      Log.Info("Transcode2MP4: add aac audio encoder to graph");
      ...
      if (aacEncoder == null)
			{
        Log.Error("Transcode2MP4: FAILED: Unable to create aac audio encoder");
				Cleanup();
				return false;
			}
      hr = graphBuilder.AddFilter(aacEncoder, "aac audio encoder");
			if (hr != 0)
			{
        Log.Error("Transcode2MP4: FAILED: Add aac audio encoder to filtergraph :0x{0:X}", hr);
				Cleanup();
				return false;
			}
      // dump filter ????
			//add filewriter 
      Log.Info("Transcode2MP4: add FileWriter to graph");
			...
			{
        Log.Error("Transcode2MP4: FAILED: Unable to create FileWriter");
				Cleanup();
				return false;
			}
			...
			{
        Log.Error("Transcode2MP4: FAILED: Add unable to get IFileSinkFilter for filewriter");
				Cleanup();
				return false;
			}
			...
			if (hr != 0)
			{
        Log.Error("Transcode2MP4: FAILED: Add FileWriter to filtergraph :0x{0:X}", hr);
				Cleanup();
				return false;
			}
			//set output filename
			string outputFileName = System.IO.Path.ChangeExtension(info.file, ".mp4");
      Log.Info("Transcode2MP4: set output file to :{0}", outputFileName);
			hr = fileWriterFilter.SetFileName(outputFileName, null);
			if (hr != 0)
			{
        Log.Error("Transcode2MP4: FAILED: unable to set filename for filewriter :0x{0:X}", hr);
				Cleanup();
				return false;
			}
			// add mp4 muxer
      Log.Info("Transcode2MP4: add MP4 Muxer to graph");
      ...
      mp4Muxer = Marshal.BindToMoniker(monikermp4Muxer) as IBaseFilter;
			if (mp4Muxer == null)
			{
        Log.Error("Transcode2MP4: FAILED: Unable to create MP4Mux");
				Cleanup();
				return false;
			}
			...
			if (hr != 0)
			{
        Log.Error("Transcode2MP4: FAILED: Add MP4Mux to filtergraph :0x{0:X}", hr);
				Cleanup();
				return false;
			}
      //connect output of audio codec to aac encoder
      IPin pinOut, pinIn;
      Log.Info("Transcode2MP4: connect audio codec->aac encoder");
      pinIn = DsFindPin.ByDirection(aacEncoder, PinDirection.Input, 0);
      if (pinIn == null)
      {
        Log.Error("Transcode2MP4: FAILED: cannot get input pin of aac encoder:0x{0:X}", hr);
        Cleanup();
        return false;
      }
      ...
      if (pinOut == null)
      {
        Log.Error("Transcode2MP4: FAILED: cannot get output pin of audio codec :0x{0:X}", hr);
        Cleanup();
        return false;
      }
      ...
      if (hr != 0)
      {
        Log.Error("Transcode2MP4: FAILED: unable to connect audio codec->aac encoder: 0x{0:X}", hr);
        Cleanup();
        return false;
      }
			//connect output of video codec to h264 encoder
      Log.Info("Transcode2MP4: connect video codec->h264 encoder");
			pinIn = DsFindPin.ByDirection(h264Encoder, PinDirection.Input, 0);
			if (pinIn == null)
			{
        Log.Error("Transcode2MP4: FAILED: cannot get input pin of h264 encoder:0x{0:X}", hr);
				Cleanup();
				return false;
			}
			...
			if (pinOut == null)
			{
        Log.Error("Transcode2MP4: FAILED: cannot get output pin of video codec :0x{0:X}", hr);
				Cleanup();
				return false;
			}
			hr = graphBuilder.Connect(pinOut, pinIn);
			if (hr != 0)
			{
        Log.Error("Transcode2MP4: FAILED: unable to connect video codec->h264 encoder :0x{0:X}", hr);
				Cleanup();
				return false;
			}
      //connect output of aac encoder to pin#0 of mp4mux
      Log.Info("Transcode2MP4: connect aac encoder->mp4mux");
      ...
      if (pinIn == null)
      {
        Log.Error("Transcode2MP4: FAILED: cannot get input pin#1 of mp4 muxer :0x{0:X}", hr);
        Cleanup();
        return false;
      }
      hr = graphBuilder.Connect(pinOut, pinIn);
      if (hr != 0)
      {
        Log.Error("Transcode2MP4: FAILED: unable to connect aac encoder->mp4mux: 0x{0:X}", hr);
        Cleanup();
        return false;
      }
      //connect output of h264 encoder to pin#1 of mp4mux
      Log.Info("Transcode2MP4: connect h264 encoder->mp4mux");
      ...
      if (pinIn == null)
      {
        Log.Error("Transcode2MP4: FAILED: cannot get input#0 pin of mp4mux :0x{0:X}", hr);
        Cleanup();
        return false;
      }
      hr = graphBuilder.Connect(pinOut, pinIn);
      if (hr != 0)
      {
        Log.Error("Transcode2MP4: FAILED: unable to connect h264 encoder->mp4mux :0x{0:X}", hr);
        Cleanup();
        return false;
      }
      // dump filter??
			//connect mp4 muxer out->filewriter
      Log.Info("Transcode2MP4: connect mp4mux->filewriter");
			pinOut = DsFindPin.ByDirection(mp4Muxer, PinDirection.Output, 0);
			if (pinOut == null)
			{
        Log.Error("Transcode2MP4: FAILED: cannot get output pin of avimux:0x{0:X}", hr);
				Cleanup();
				return false;
			}
			...
			if (pinIn == null)
			{
        Log.Error("Transcode2MP4: FAILED: cannot get input pin of filewriter :0x{0:X}", hr);
				Cleanup();
				return false;
			}
			hr = graphBuilder.Connect(pinOut, pinIn);
			if (hr != 0)
			{
        Log.Error("Transcode2MP4: FAILED: connect mp4 muxer->filewriter :0x{0:X}", hr);
				Cleanup();
				return false;
			}
			return true;
