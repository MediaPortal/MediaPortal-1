...
{
  public delegate void StoreStreamAction(
    string filterName, string name, int lcid, int id, DirectShowHelper.StreamType type, AMStreamSelectInfoFlags flag,
    IAMStreamSelect pStrm);
  ...
  public class DirectShowHelper
  {
                ...
                try
                {
                  ...
                  if (pStrm != null)
                  {
                    ...
                    for (int istream = 0; istream < cStreams; istream++)
                    {
                      ...
                      //SUBTITLE
                      else if (sPdwGroup == 2 && sName.LastIndexOf("off", StringComparison.Ordinal) == -1 &&
                               sName.LastIndexOf("Hide ", StringComparison.Ordinal) == -1 &&
                               sName.LastIndexOf("No ", StringComparison.Ordinal) == -1 &&
                               sName.LastIndexOf("Miscellaneous ", StringComparison.Ordinal) == -1)
                      {
                        type = StreamType.Subtitle;
                      }
                      //NO SUBTITILE TAG
                      else if ((sPdwGroup == 2 &&
                                (sName.LastIndexOf("off", StringComparison.Ordinal) != -1 ||
                                 sName.LastIndexOf("No ", StringComparison.Ordinal) != -1)) ||
                      ...
                      Log.Debug(
                        "DirectShowHelper: FoundStreams: Type={0}; Name={1}, Filter={2}, Id={3}, PDWGroup={4}, LCID={5}",
    ...
    private static readonly Regex LavSplitterAudio =
      new Regex(
        @"A\:\s*(((?<name>.+)\[(?<language>\w+)\]|\[(?<language>\w+)\]|(?<name>.+))\s*\()?\s*(?<codec>[a-z0-9\s\.'_\-]+),\s*(?<freq>\d+)\s*Hz,\s*(?<channels>[a-z0-9\s\.'_\-]+)(,\s*s(?<bit>\d+))?(,\s*(?<bitrate>\d+)\s*kb/s)?",
        RegexOptions.Compiled | RegexOptions.IgnoreCase);

    private static readonly Regex FfdshowNameAudio =
      new Regex(@"Audio\s*-\s*(?<codec>\w+),\s*(?<channels>[^,]+),\s*(?<freq>\d+)\s*Hz",
        RegexOptions.Compiled | RegexOptions.IgnoreCase);

    private static readonly Regex TsReaderAudio = new Regex(@"^(?<language>\w{2,3})$",
      RegexOptions.Compiled | RegexOptions.IgnoreCase);
        ...
        info.AudioStreams.FirstOrDefault(
          x =>
            ...
            ((string.IsNullOrEmpty(x.Name) && !string.IsNullOrEmpty(language) &&
              language.Equals(name, StringComparison.OrdinalIgnoreCase)) ||
    ...
    private static readonly Regex LavSplitterVideo =
      new Regex(
        @"V\:\s*(((?<name>.+)\[(?<language>\w+)\]|\[(?<language>\w+)\]|(?<name>.+))\s*\()?\s*(?<codec>[a-z0-9\s\.'_\-]+),\s*(?<output>[a-z0-9\s\.'_\-]+),\s*(?<width>\d+)x(?<height>\d+)",
        RegexOptions.Compiled | RegexOptions.IgnoreCase);
