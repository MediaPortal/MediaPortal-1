	...
	public class GUIThumbnailPanel : GUIControl
  {
    const int SLEEP_FRAME_COUNT=1;
		...
		[XMLSkinElement("itemWidth")]			protected int m_iItemWidth;
		 /*"itemHeight"*/						protected int m_iItemHeightLow;
		 /*"itemWidth"*/						protected int m_iItemWidthLow;
...
GUISpinControl m_upDown = null;
    ...
    protected void OnSelectionChanged()
    {
      ...
      GUIPropertyManager.SetProperty("#selectedthumb", strThumb);
		...
		{
      GUIButtonControl btn=(GUIButtonControl)m_button[iButton];
      ...
      if (fy<=0) fy=0;
			...
			view.Height = (int)(m_iRows * m_iItemHeight);
			view.MinZ = 0.0f;
			view.MaxZ = 1.0f;
		...
		void RenderText(float fPosX, float fPosY, long dwTextColor, string wszText, bool bScroll)
    {
      ...
      else
      {
		...
		void OnPageDown()
		{
			...
			while (m_iCursorY > 0 && m_iOffset + m_iCursorY * m_iColumns + m_iCursorX >= (int) m_vecItems.Count)
			{
				m_iCursorY--;
      }
      OnSelectionChanged();
      ...
      m_bRefresh = true;
		...
		public string FocusName { get { return m_strImageFolderFocus; } }
		public string NoFocusName { get { return m_strImageFolder; } }
