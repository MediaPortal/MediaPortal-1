...
{
  public class GUIMenuClassic : GUIMenuControl
  {
    ...
    [XMLSkinElement("spaceAfterSelected")]
    protected int _spaceAfterSelected = 0;
    ...
    [XMLSkinElement("mouseScrollTimeMin")]
    protected int _mouseScrollTimeMin = 250;
    [XMLSkinElement("mouseScrollTime")]
    protected int _mouseScrollTimeMax = 600;
    ...
    public GUIMenuClassic(int dwParentID)
      : base(dwParentID) { }
    ...
    public override bool FixedScroll
    {
      get { return _fixedScroll; }
      set { _fixedScroll = value; }
    }

    public override bool EnableAnimation
    {
    ...
    private double Interpolate(Easing easing, double from, double to, double start, double end, double current)
    {
    ...
    private void InsertFinalAnimation()
    {
      if (_scrollButton.Count > 0)
      {
        _scrollButton.Clear();
    ...
    private void AnimationMovement(float timePassed)
    {
      AnimationMovementButton(timePassed);
      AnimationMovementText(timePassed);
    }

    private void AnimationMovementButton(float timePassed)
    {
    ...
    private void AnimationMovementText(float timePassed)
    {
    ...
    private void AnimationFinished(float timePassed)
    {
    ...
    private void LoadHoverImage(int position)
    {
      ...
      {
        if (hover != null && hover.GetID == _buttonList[position].GetID)
        {
          _hoverImage = hover;
          _hoverImage.Begin();
          break;
