	...
	public class Twinhan : IksPropertyUtils
	{
		...
		public void SendPMT(uint videoPid, uint audioPid,byte[] PMT, int pmtLen)
		{
			...
			if(ptrPMT==IntPtr.Zero)
				return;
			Marshal.Copy(PMT,0,(IntPtr)(((int)ptrPMT)),pmtLen);
			...
			Marshal.WriteByte(thbdaBuf,14,0x3d);
			Marshal.WriteByte(thbdaBuf,15,0x67);
			Marshal.WriteInt32(thbdaBuf,16,(int)THBDA_IOCTL_CI_SEND_PMT);
			Marshal.WriteInt32(thbdaBuf,20,(int)ptrPMT);
			Marshal.WriteInt32(thbdaBuf,24,pmtLen);
			...
			if (pin!=null) 
			{
				...
				if (propertySet!=null) 
				{
					...
					if (hr!=0)
					{
						Log.Write("twinhan: failed to send pmt 0x{0:X}",hr);
					}
					else
						Log.Write("twinhan: send pmt returned ok  0x{0:X}",hr);
					Marshal.ReleaseComObject(propertySet);
				}
				Marshal.ReleaseComObject(pin);
			}
