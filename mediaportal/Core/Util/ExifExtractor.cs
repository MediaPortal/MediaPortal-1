  ...
  public class ExifMetadata : IDisposable
  {
    ...
    public Metadata GetExifMetadata(string photoName)
    {
      ...
      try
      {
        ...
        DateTime dateTime;
        var subIfdDirectory =  (ExifSubIfdDirectory)directories.OfType<ExifSubIfdDirectory>().Where((x) => x.TryGetDateTime(ExifDirectoryBase.TagDateTimeOriginal, out dateTime));
