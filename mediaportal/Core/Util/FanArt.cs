...
namespace MediaPortal.Util
{
  ...
  public class FanArt
  {
    ...
    // Get Backdrop FanArt method, random parameter is for random image grab from list
    public void GetFanArt(string filename, string path, string title, string imdbTT, bool random, bool localImage,
                          string localFile)
    {
      if (localImage == false)
      {
        ...
        try
        {
          ...
          if (Directory.Exists(configDir))
          {
            ...
            // Get Random Backdrop if parameter bool random=true
            int rnd = 0;
            if (random)
            {
              var r = new Random();
              rnd = r.Next(_fanartList.Count);
            }

            if (_fanartList.Count > 0)
            {
              var tempFile = (string)_fanartList[rnd];
              ...
              try
              {
                _fileArt = configDir + title + ext;
                var webClient = new WebClient();
                webClient.DownloadFile((string)_fanartList[rnd], _fileArt);
                _fanartURL = _fanartList[rnd].ToString();
                webClient.Dispose();
              }
              catch (Exception) {}
              ...
              else
              {
                ...
                _fileMovie = configDir + titleFolder + ext;
                File.Copy(_fileArt, _fileMovie, true);
              }
            }
          }
      ...
      else
      {
        ...
        if (Directory.Exists(configDir))
        {
          string dbFile = configDir + title + ext;
          // DBView file
          if (isURL == false)
          {
    ...
    public static void GetFanArtfilename(string title, out string fileart)
    {
      fileart = "";
      // FanArt directory
      string configDir;
    ...
    public static void DeleteFanarts(string pathAndFilename, string title)
    {
      ...
      if (!fileNameArt.ToUpper().Contains("VIDEO_TS"))
      {
        ...
        string fileMovie = configDir + fileclean + ".jpg";
      ...
      else
      {
        ...
        string titleFolder = strTrimed.Substring(start);
        //Folder name DVD backdrop for SHARES View
        string fileMovie = configDir + titleFolder + ".jpg";
        if (File.Exists(fileMovie))
        {
          File.Delete(fileMovie);
        }
