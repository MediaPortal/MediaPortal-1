...
{
	/// <summary>
	/// This Class is a copy of VirtualKeyboard.cs modified by Agree for search music
	/// </summary>
	public class VirtualSearchKeyboard: GUIWindow
	{
		const int GAP_WIDTH     = 0;
		const int GAP2_WIDTH    = 4;
		const int MODEKEY_WIDTH = 130;
		...
		string[] g_strEnglish = 
	{
			...
			public Key( Xkey key , int iwidth)
			{
				...
				switch( xKey )
				{
					...
					case Xkey.XK_ACCENTS:
						...
						break;
					case Xkey.XK_OK:
						strName = GUILocalizeStrings.Get(804);
						break;
				case Xkey.XK_SEARCH_CONTAINS:
						strName = GUILocalizeStrings.Get(801);
						break;
					case Xkey.XK_SEARCH_ENDS_WITH:
						strName = GUILocalizeStrings.Get(802);
						break;
					case Xkey.XK_SEARCH_START_WITH:
						strName = GUILocalizeStrings.Get(800);
						break;
					case Xkey.XK_SEARCH_IS:
						strName = GUILocalizeStrings.Get(803);
						break;
				}
			}
		};
		...
		// added by Agree
		int           m_searchKind; // 0=Starts with, 1=Contains, 2=Ends with
		//
		...
		public VirtualSearchKeyboard()
		{
			GetID=(int)GUIWindow.Window.WINDOW_VIRTUAL_SEARCH_KEYBOARD; // by Agree
    ...
    {
      if (m_bRunning && GUIGraphicsContext.CurrentState == GUIGraphicsContext.State.RUNNING)
      {
        GUIWindowManager.Process();
      }
    }
      ...
      GUIFontManager.Present();
		...
		void InitBoard()
		{
			...
			// First row is Done, 1-0, SearchKind
      ...
      keyRow.Add( new Key( Xkey.XK_ACCENTS, MODEKEY_WIDTH ) );
			...
			keyRow.Add(new Key(Xkey.XK_SEARCH_CONTAINS,MODEKEY_WIDTH));
				...
				// Special cases
			else switch( xk )
				 {
					 ...
					 case Xkey.XK_ALPHABET:
						 ...
						 ChangeKey((int)m_iCurrBoard,0,11, new  Key( Xkey.XK_ACCENTS, MODEKEY_WIDTH ));
						 break;
					 case Xkey.XK_SYMBOLS:
						 m_iCurrBoard = KeyboardTypes.TYPE_SYMBOLS;

						 // Adjust mode keys
						 ChangeKey((int)m_iCurrBoard,4,0, new  Key( Xkey.XK_ALPHABET, MODEKEY_WIDTH ));
						 ChangeKey((int)m_iCurrBoard,0,11, new  Key( Xkey.XK_ACCENTS, MODEKEY_WIDTH ) );
					 ...
					 case Xkey.XK_ACCENTS:
						 ...
						 // Adjust mode keys
						 ChangeKey((int)m_iCurrBoard,4,0 , new Key( Xkey.XK_ALPHABET, MODEKEY_WIDTH ));
						 ChangeKey((int)m_iCurrBoard,0,11 , new Key( Xkey.XK_SYMBOLS, MODEKEY_WIDTH ));
						 break;
      ...
      y2+=GUIGraphicsContext.OffsetY;
		...
		{
			m_Font18.DrawText( x, y, 0xff10e010, m_strData, GUIControl.Alignment.ALIGN_LEFT );
