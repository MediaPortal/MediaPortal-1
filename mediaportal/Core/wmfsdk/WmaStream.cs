...
namespace Yeti.WMFSdk
{
...

  {
...

    {
      ...
      IntPtr buffer =
        Marshal.AllocCoTaskMem(Math.Max((int)Size,
                                        Marshal.SizeOf(typeof (WM_MEDIA_TYPE)) + Marshal.SizeOf(typeof (WaveFormat))));
...

      {
        ...
        {
          throw new ArgumentException(string.Format("The format {0} of the output {1} is not a valid PCM format",
...

    {
...

      {
...

        {
          ...
          {
            m_Reader.GetNextSample(m_OuputStream, out sample, out SampleTime, out Duration, out Flags,
                                   out m_OutputNumber, out m_OuputStream);
          }
