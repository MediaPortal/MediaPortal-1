  ...
  public class HidHandler
  {
    ...
    public void OnHidEvent(object aSender, Hid.Event aHidEvent)
    {
      ...
      if (_usageActions.TryGetValue(aHidEvent.UsageId, out usageAction))
      {
        ...
        {
          // Here goes some ugly specific hack to make sure our text input is working
              ...
              (GUIWindowManager.ActiveWindowEx == (int) GUIWindow.Window.WINDOW_TV_SEARCH))
          {
            ...
            //We still let through some basic keys to enable focus control, validation and cancellation of text input
            Keys key = aHidEvent.VirtualKey;
            if (!(key == Keys.Left ||
                  key == Keys.Right ||
                  key == Keys.Up ||
                  key == Keys.Down ||
              ...
              // Make sure we don't have any modifier
              !(aHidEvent.HasModifierShift || aHidEvent.HasModifierAlt || aHidEvent.HasModifierControl || aHidEvent.HasModifierWindows))
            {
              // Do not handle keyboard events for those windows.
              // In fact they are typically text input windows and their input is handled at a higher level, see: MediaPortal.KeyPressEvent.
              return;
            }
          }
