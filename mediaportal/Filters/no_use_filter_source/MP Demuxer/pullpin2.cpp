...
// File: PullPin2.cpp
//
// Desc: DirectShow base classes - implements CPullPin22 class that pulls data
//       from IAsyncReader.
//
...
// This version modified to support Seek calls on the worker thread 
//
// Geraint Davies
...
// necessary
HRESULT
CPullPin2::Connect(IUnknown* pUnk, IMemAllocator* pAlloc, BOOL bSync)
{
...
//
// This method modified from original SDK version to allow
// calls from the worker thread (to refine seeking based on 
// the data received so far).
//
HRESULT
CPullPin2::Seek(REFERENCE_TIME tStart, REFERENCE_TIME tStop)
{
    // are we on the worker thread?
    if (m_threadid == GetCurrentThreadId())
    {
        // can't break into the pull loop like this if we are using async queue/request
        if (!m_bSync)
        {
            return VFW_E_WRONG_STATE;
        }

        // don't get the critsec (could deadlock with
        // a graph-level seek thread waiting for us)
        // and don't wait for ourself to Pause).

        // still need to flush old data before starting the new stuff
        BeginFlush();
        EndFlush();
        m_tStart = tStart;
        m_tStop = tStop;
...
HRESULT
CPullPin2::Duration(REFERENCE_TIME* ptDuration)
{
...
CPullPin2::Process(void)
{
    ...
    // this flag is used to indicate a seek callback during deliver, in the
    // sync loop below
    m_bSeek = false;
    ...
    if (!m_bSync) {
        ...
        while (tCurrent < tAlignStop) {
            ...
            // was there a seek callback during that deliver?
            if (m_bSeek)
            {
                m_bSeek = false;

                // recalculate the seek positions

                tStart = AlignDown(m_tStart / UNITS, Actual.cbAlign) * UNITS;
                tCurrent = tStart;
                ...
                // align the stop position - may be past stop, but that
                // doesn't matter
                tAlignStop = AlignUp(tStop / UNITS, Actual.cbAlign) * UNITS;
            }
        }
    }
...
// and release
void
CPullPin2::CleanupCancelled(void)
{
    while (1) {
        IMediaSample * pSample;
        DWORD_PTR dwUnused;
