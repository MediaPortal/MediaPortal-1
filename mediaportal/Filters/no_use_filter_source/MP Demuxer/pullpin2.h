...
// The DirectShow SDK version of this class does not
// support Seek calls from the worker thread itself (e.g. when
// refining the seek position for variable-bit rate accuracy).
// Also you cannot override methods to provide this since
// too much of the class is private.
//
// So this is taken from the SDK, renamed, and with a fixed
// Seek method.
//
// Geraint Davies
...
class CPullPin2 : public CAMThread
{
    // worker thread id if known
    DWORD               m_threadid;

    // flag accessed on worker thread only
    bool                m_bSeek;
...
public:
    ...
    // Flush and suspend thread, but don't set positions or restart
    // -- first half of seek operation
    bool Suspend();
