...
namespace MediaPortal.ProcessPlugins.MiniDisplayPlugin
{
  public class GUI_SettingsKeyPad : GUIWindow
  {
    ...
    {
      Log.Info("MiniDisplay.GUI_SettingsKeyPad.OnAdded(): Window {0} added to window manager", new object[] { this.GetID });
      base.OnAdded();
    }
    ...
    protected override void OnClicked(int controlId, GUIControl control, Action.ActionType actionType)
    {
      ...
      {
        Log.Info("MiniDisplay.GUI_SettingsKeyPad.OnClicked(): called - btnEnableKeyPad clicked", new object[0]);
        this.OnEnableKeyPadChanged();
      }
      if (control == this.btnEnableCustom)
      {
        Log.Info("MiniDisplay.GUI_SettingsKeyPad.OnClicked(): called - btnEnableCustom clicked", new object[0]);
        this.OnEnableCustomChanged();
      }
      if (control == this.btnKeyPadMapping)
      {
        Log.Info("MiniDisplay.GUI_SettingsKeyPad.OnClicked(): called - btnRemoteMapping clicked", new object[0]);
        try
        {
    ...
    protected override void OnPageLoad()
    {
      ...
      GUIControl.FocusControl(this.GetID, this.btnEnableKeyPad.GetID);
      GUIPropertyManager.SetProperty("#currentmodule", "MiniDisplay KeyPad Control Setup");
    }
