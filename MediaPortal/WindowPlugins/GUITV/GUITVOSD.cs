...
namespace MediaPortal.GUI.TV
{
  ...
  public class GUITVOSD : GUIWindow
  {
    ...
    private List<TVChannel> listTvChannels = new List<TVChannel>();
    ...
    public override bool OnMessage(GUIMessage message)
    {
      switch (message.Message)
      {
        case GUIMessage.MessageType.GUI_MSG_WINDOW_DEINIT: // fired when OSD is hidden
          {
            Recorder.OnTvRecordingChanged -= new Recorder.OnTvRecordingChangedHandler(OSD_OnTvRecordingChanged);
            FreeResources();
        ...
        case GUIMessage.MessageType.GUI_MSG_CLICKED:
          {
            ...
            if (iControl == btnPreviousProgram.GetID)
            {
              ...
              {
                prog =
                  GUITVHome.Navigator.GetTVChannel(GetChannelName()).GetProgramAt(
