...
namespace MediaPortal.TV.Recording
{
  public class StopTimeShiftingCommand : CardCommand
  {
    ...
    {
      bool stopped = false;
      _log.Info("Command:Stop timeshifting");
      ...
      for (int i=0; i < handler.TVCards.Count;++i)
      {
        ...
        if (!card.IsRecording)
        {
          if (card.IsTimeShifting)
          {
            ...
            {
              _log.Info("Recorder:  stop playing timeshifting file for card:{0}", card.CommercialName);

              handler.StopPlayer();
              stopped = true;
            }

            _log.Info("Recorder: stop timeshifting on card:{0} channel:{1}",
