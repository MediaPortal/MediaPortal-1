...
namespace MediaPortal.TVGuideScheduler
{
  internal class XMLTVgrab
  {
                                    ...
                                    bool runLowPrio)
    {
      ...
      int i = 0;
      Thread[] grabThreads = new Thread[Convert.ToInt32(daysToGrab.Length)];
      //create a tread for each day defined in mediaprtal.xml
      foreach (string s in daysToGrab)
      {
        //start the threads
        RunMutliGrabber helper = new RunMutliGrabber(Convert.ToInt32(s), grabber, conf, exe, op, args);
        if (runLowPrio)
        {
    ...
    public static bool RunGrabber(string grabber, string confFile, string path, string opFile, int days, int offset,
                                  string args, bool lowPrio)
    {
      //start an xmltv process to grab multiple days
      string xmltvpath = path + "\\xmltv.exe";
      string WorkingDir = xmltvpath.Substring(0, xmltvpath.Length - 9);
      string outputPath = @"""" + @opFile + @"\TvGuide.xml""";
      string xmltvArgs = grabber + " --config-file " + confFile + " --output " + outputPath + " --days " + days + " " +
                         args + " --offset " + offset;
      try
      {
        Process xmltvgrabber = new Process();
    ...
    public class RunMutliGrabber
    {
      ...
      public void RunThread()
      {
        ...
        string xmltvargs = String.Format("{0} --config-file {1} --output {2} --days 1 {3} --offset {4}", grabber,
                                         confFile, outputPath, args, offset);
        try
        {
