...
Demux::Demux(PidInfo* pids, IBaseFilter *pFilter, CFilterList *pFList) :
...
HRESULT Demux::LoadTsPin(IPin* pIPin)
{
	...
	{
		muxMapPid->MapPID(m_pids->TsArray[0] + 1, &m_pids->TsArray[1], MEDIA_TRANSPORT_PACKET);
		muxMapPid->Release();
		hr = S_OK;
	}
	return hr;
...
HRESULT Demux::CheckDemuxPids(void)
{
	...
	if ((m_SelSubtitlePid != m_pids->sub) && m_bCreateSubPinOnDemux)
		return S_FALSE;

	if (m_pids->aud | m_pids->ac3 | m_pids->aac | m_pids->dts)
		if (((m_SelAudioPid != m_pids->aud)
				&& (m_SelAudioPid != m_pids->aud2)
				&& (m_SelAudioPid != m_pids->ac3)
				&& (m_SelAudioPid != m_pids->ac3_2)
				&& (m_SelAudioPid != m_pids->aac)
				&& (m_SelAudioPid != m_pids->aac2)
				&& (m_SelAudioPid != m_pids->dts)
				&& (m_SelAudioPid != m_pids->dts2))
				|| !m_SelAudioPid)
			return S_FALSE;

	if (m_pids->vid | m_pids->h264 | m_pids->mpeg4)
		if (((m_SelVideoPid != m_pids->vid)
				&& (m_SelVideoPid != m_pids->h264)
				&& (m_SelVideoPid != m_pids->mpeg4))
				|| !m_SelVideoPid)
			return S_FALSE;
...
{
	if ((m_bMPEG2Audio2Mode && m_pids->aud2) || (m_StreamAud2 && m_pids->aud2))
		return m_pids->aud2;
	else
		return m_pids->aud;
}
...
{
	if ((m_bMPEG2Audio2Mode && m_pids->aac2) || (m_StreamAud2 && m_pids->aac2))
		return m_pids->aac2;
	else
		return m_pids->aac;
}
...
{
	if ((m_bMPEG2Audio2Mode && m_pids->dts2) || (m_StreamAud2 && m_pids->dts2))
		return m_pids->dts2;
	else
		return m_pids->dts;
}
...
{
	if ((m_bMPEG2Audio2Mode && m_pids->ac3_2) || (m_StreamAud2 && m_pids->ac3_2))
		return m_pids->ac3_2;
	else
		return m_pids->ac3;
}
