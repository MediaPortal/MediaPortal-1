...
namespace Mediaportal.TV.TvPlugin.EPG
{
  ...
  {    
    ...
    protected Program _currentProgram;
    ...
    protected TvBusinessLayer _layer = new TvBusinessLayer();
    ...
    protected void Update(bool selectCurrentShow)
    {
      lock (this)
      {
        ...
        else
        {
          ...
          Dictionary<int, List<Program>> programs = _layer.GetProgramsForAllChannels(Utils.longtodate(iStart),
      ...
      if (_channelList.Count == 0)
      {
        ...
        if (_channelList.Count == 0)
        {
          var tvGuidChannel = new GuideChannel
                                {
                                  Channel = new Channel(false, true, 0, DateTime.MinValue, false,
    ...
    private void SetProperties()
    {
      ...
      else if (_currentProgram != null)
      {
        ...
        GUIPropertyManager.SetProperty(SkinPropertyPrefix + ".Guide.CompositeTitle",
                                       TVUtil.GetDisplayTitle(_currentProgram));
    ...
    private void RenderSingleChannel(Channel channel)
    {
      ...
      if (_recalculateProgramOffset)
      {
        ...
        _programs = _layer.GetPrograms(channel, dtStart, dtEnd);
      ...
      for (int ichan = 0; ichan < _channelCount; ++ichan)
      {
        ...
        {
          program = _programs[offset + ichan];
        }
        else
        {
          // bugfix for 0 items
          ...
          {
            program = _programs[_programs.Count - 1];
    ...
    private void RenderChannel(ref Dictionary<int, List<Program>> mapPrograms, int iChannel,
                                 ...
                                 long iStart, long iEnd, bool selectCurrentShow)
    {
      ...
      if (noEPG)
      {
        ...
        var prog = new Program(channel.IdChannel, Utils.longtodate(iStart), Utils.longtodate(iProgEnd),
      ...
      foreach (Program program in programs)
      {
        ...
        bool programIsHd = program.Description.Contains(_hdtvProgramText);
    ...
    private void LoadSettings()
    {
      using (Settings xmlreader = new MPSettings())
      {
        ...
        IList<Channel> channels = _layer.GetChannelsByName(channelName);
