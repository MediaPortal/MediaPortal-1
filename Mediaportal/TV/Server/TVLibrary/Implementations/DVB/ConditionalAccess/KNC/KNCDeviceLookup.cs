...
namespace Mediaportal.TV.Server.TVLibrary.Implementations.DVB.ConditionalAccess.KNC
{
  ...
  public static class KNCDeviceLookup
  {
    ...
    public static int GetDeviceIndex(TvCardBase TvCard)
    {
      ...
      IList<TVDatabase.Gentle.Server> dbsServers = TVDatabase.Gentle.Server.ListAll();

      foreach (TVDatabase.Gentle.Server server in dbsServers)
      {
        deviceids.AddRange(from dbsCard in server.ReferringCard() where dbsCard.Name.StartsWith("KNC BDA") || dbsCard.Name.StartsWith("Mystique") select dbsCard.DevicePath);
      }
