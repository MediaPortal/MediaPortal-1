...
namespace Mediaportal.TV.Server.TVLibrary.Implementations.DVB.DisEqC
{
  ...
  public class GenericBDAS
  {
    ...
    public GenericBDAS(IBaseFilter tunerFilter)
    {
      ...
      if (pin != null)
      {
        ...
        if (_propertySet != null)
        {
          ...
          {
            Log.Debug(FormatMessage("DiSEqC capable card found!"));
            _isGenericBDAS = true;
          }
        }
      }
      else
        Log.Info(FormatMessage("tuner pin not found!"));
    }
    ...
    public void SendDiseqCommand(ScanParameters parameters, DVBSChannel channel)
    {
      switch (channel.DisEqc)
      {
        case DisEqcType.Level1AA:
          Log.Info(FormatMessage("  Level1AA - SendDiSEqCCommand(0x00)"));
          SendDiSEqCCommand(0x00);
          break;
        case DisEqcType.Level1AB:
          Log.Info(FormatMessage("  Level1AB - SendDiSEqCCommand(0x01)"));
          SendDiSEqCCommand(0x01);
          break;
        case DisEqcType.Level1BA:
          Log.Info(FormatMessage("  Level1BA - SendDiSEqCCommand(0x0100)"));
          SendDiSEqCCommand(0x0100);
          break;
        case DisEqcType.Level1BB:
          Log.Info(FormatMessage("  Level1BB - SendDiSEqCCommand(0x0101)"));
          SendDiSEqCCommand(0x0101);
          break;
        case DisEqcType.SimpleA:
          Log.Info(FormatMessage("  SimpleA - SendDiSEqCCommand(0x00)"));
          SendDiSEqCCommand(0x00);
          break;
        case DisEqcType.SimpleB:
          Log.Info(FormatMessage("  SimpleB - SendDiSEqCCommand(0x01)"));
          SendDiSEqCCommand(0x01);
          break;
        default:
          return;
    ...
    {
      Log.Info(FormatMessage("  SendDiSEqC Command {0}"), ulRange);
          ...
          if (ControlNode != null)
          {
            ...
            if (hr == 0)
            {
              if (FrequencyFilter != null)
              {
                ...
                Log.Info(FormatMessage("  put_Range:{0} success:{1}"), ulRange, hr);
                if (hr == 0)
                {
                  ...
                  if (hr == 0)
                  {
                    ...
                    if (hr == 0)
                    {
                      Log.Info(FormatMessage("  CommitChanges() Succeeded"));
                      return true;
                    }
                    // reset configuration
                    Log.Info(FormatMessage("  CommitChanges() Failed!"));
                    DecviceControl.StartChanges();
                    DecviceControl.CommitChanges();
                    return false;
                  }
                  Log.Info(FormatMessage("  CheckChanges() Failed!"));
                  return false;
                }
                Log.Info(FormatMessage("  put_Range Failed!"));
                return false;
              }
            }
          }
      ...
      Log.Info(FormatMessage("  GetControlNode Failed!"));
      return false;
    }
          ...
          if (ControlNode != null)
          {
            ...
            if (hr == 0)
            {
              if (FrequencyFilter != null)
              {
                ...
                Log.Info(FormatMessage("  get_Range:{0} success:{1}"), pulRange, hr);
                if (hr == 0)
                {
                  return true;
                }
                Log.Info(FormatMessage("  get_Range Failed!"));
                return false;
              }
            }
          }
