...
namespace Tetris
{
	...
	public class Game
	{
		...
		public State State
		{ get { return m_nState; } set { m_nState = value; } }
		...
		public int Score
		{ set { m_nScore = value; } get { return m_nScore; } }

		public int Level
		{ set { m_nLevel = value; } get { return m_nLevel + 1; } }
			...
			for(int y = 0; y < Game.Height; y++) 
			{
				...
				for(int x = 0; x < Game.Width; x++) 
				{
					if(m_nBlock[x, y] != 0)
					{
						...
						{
							// move next row?
							if((Environment.TickCount - m_nFallingTime) * Game.FallingSpeed > 1)
							{
								m_nFallingRow++;
		...
		static int nLastTimerCall = Environment.TickCount;
		...
		// game variables
		int							m_nFallingRow = Game.Height;
		int							m_nFallingTime = 0;
		int							m_nScore = 0;
		int							m_nLevel = 0;
		const int					m_nLevelMax = 9;
		int							m_nLines = 0;
		int[,]						m_nBlock = new int[Game.Width, Game.Height];
		internal Block				m_blockCurrent = null;
		internal Block				m_blockNext = null;
		public State				m_nState = State.Stopped;
		int							m_nLastBlockTick = 0;
		protected static int[]		_Timeouts = new int[] { 500, 450, 400, 350, 300, 250, 200, 150, 100, 50 };
		protected static Color[]	_Colors = new Color[] { Color.Black, Color.Red, Color.Green, Color.Blue, Color.Orange, Color.Cyan, Color.Gray, Color.Yellow };
		IHostTetris					m_pHost;
	...
	internal class Block
	{
		...
		public int X
		{ set { m_nX = value; } get { return m_nX; } }

		public int Y
		{ set { m_nY = value; } get { return m_nY; } }
		...
		public int Rotation
		{ set { m_nRotation = value; } get { return m_nRotation; } }
		...
		void ToVirtualCoordinates(ref float[] x, ref float[] y)
		{
		...
		public Rectangle Rectangle
		{
			get 
			{
				...
				for(int i = 0; i < 4; i++) 
				{
					if(r.X - x[i] + 1 > r.Width) r.Width = r.X - x[i] + 1;
					if(r.Y - y[i] + 1 > r.Height) r.Height = r.Y - y[i] + 1;
				}
