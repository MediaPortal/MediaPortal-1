	...
	public class MPRecommendations : Form
	{
		...
		public MPRecommendations(string[] args)
		{
          Thread.CurrentThread.Name = "MPRecommendations";
			...
			if(args.Length==0) 
			{
				MessageBox.Show("This post setup needs one argument to start.\nThe argument must be the directory where MediaPortal is installed.","Argument missing",MessageBoxButtons.OK,MessageBoxIcon.Error);
				Application.Exit();
...
*/					
		...
		private void btnAction_Click(object sender, EventArgs e)
		{
			...
			if (buttonAction == MP3PartInstaller.BUTTONACTION_NEXT || buttonAction == MP3PartInstaller.BUTTONACTION_INSTALLED || buttonAction == MP3PartInstaller.BUTTONACTION_DONTINSTALL)
			{
				currentPackageIndex++;
