...
namespace MediaPortal.TV.Recording
{
  ...
  public class DvbTextConverter
  {
    ...
    public static string Convert(IntPtr ptr, string lang)
    {
      ...
      for (int i = 0; i < len; i++)
      {
        text[i] = Marshal.ReadByte(ptr, i);
      }
      return Convert(text, len, lang);
    }

    public static string Convert(byte[] text, int len, string lang)
    {
      int encoding, pos;
      ...
      //}
      return Encoding.GetEncoding(encoding).GetString(text, pos, len - pos);
    }
  }
}
