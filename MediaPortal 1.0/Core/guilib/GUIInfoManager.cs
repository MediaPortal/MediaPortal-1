...
namespace MediaPortal.GUI.Library
{
  public class GUIInfoManager
  {
    ...
    */
    ...
    public const int SYSTEM_ALWAYS_TRUE = 125; // useful for <visible fade="10" start="hidden">true</visible>, to fade in a control
    public const int SYSTEM_ALWAYS_FALSE = 126;  // used for <visible fade="10">false</visible>, to fade out a control (ie not particularly useful!)
    ...
    */
    ...
    static int MULTI_INFO_END = 41000;// 1000 references is all we have for now
    static int COMBINED_VALUES_START = 100000;
    #endregion

    #region variables

    static List<CCombinedValue> m_CombinedValues = new List<CCombinedValue>();
    static Dictionary<int, bool> m_boolCache = new Dictionary<int, bool>();
    static List<string> m_stringParameters = new List<string>();
    ...
    public static int TranslateSingleString(string strCondition)
    {
      ...
      if (strCategory == "player")
      {
        ...
        else if (strTest == "player.hasaudio") ret = PLAYER_HAS_AUDIO;
        else if (strTest == "player.hasvideo") ret = PLAYER_HAS_VIDEO;
        else if (strTest == "player.playing") ret = PLAYER_PLAYING;
        else if (strTest == "player.paused") ret = PLAYER_PAUSED;
        else if (strTest == "player.rewinding") ret = PLAYER_REWINDING;
        else if (strTest == "player.forwarding") ret = PLAYER_FORWARDING;
        else if (strTest == "player.rewinding2x") ret = PLAYER_REWINDING_2x;
        else if (strTest == "player.rewinding4x") ret = PLAYER_REWINDING_4x;
        else if (strTest == "player.rewinding8x") ret = PLAYER_REWINDING_8x;
        else if (strTest == "player.rewinding16x") ret = PLAYER_REWINDING_16x;
        else if (strTest == "player.rewinding32x") ret = PLAYER_REWINDING_32x;
        else if (strTest == "player.forwarding2x") ret = PLAYER_FORWARDING_2x;
        else if (strTest == "player.forwarding4x") ret = PLAYER_FORWARDING_4x;
        else if (strTest == "player.forwarding8x") ret = PLAYER_FORWARDING_8x;
        else if (strTest == "player.forwarding16x") ret = PLAYER_FORWARDING_16x;
        else if (strTest == "player.forwarding32x") ret = PLAYER_FORWARDING_32x;
        else if (strTest == "player.canrecord") ret = PLAYER_CAN_RECORD;
        else if (strTest == "player.recording") ret = PLAYER_RECORDING;
        else if (strTest == "player.displayafterseek") ret = PLAYER_DISPLAY_AFTER_SEEK;
        else if (strTest == "player.caching") ret = PLAYER_CACHING;
        else if (strTest == "player.seekbar") ret = PLAYER_SEEKBAR;
        else if (strTest == "player.seektime") ret = PLAYER_SEEKTIME;
        else if (strTest == "player.progress") ret = PLAYER_PROGRESS;
        else if (strTest == "player.seeking") ret = PLAYER_SEEKING;
        else if (strTest == "player.showtime") ret = PLAYER_SHOWTIME;
        else if (strTest == "player.showcodec") ret = PLAYER_SHOWCODEC;
        else if (strTest == "player.showinfo") ret = PLAYER_SHOWINFO;
        else if (strTest == "player.time") ret = PLAYER_TIME;
        else if (strTest == "player.timeremaining") ret = PLAYER_TIME_REMAINING;
        else if (strTest == "player.duration") ret = PLAYER_DURATION;
        else if (strTest == "player.volume") ret = PLAYER_VOLUME;
        else if (strTest == "player.muted") ret = PLAYER_MUTED;
        else if (strTest == "player.hasduration") ret = PLAYER_HASDURATION;
      }
      else if (strCategory == "weather")
      {
        if (strTest == "weather.conditions") ret = WEATHER_CONDITIONS;
        else if (strTest == "weather.temperature") ret = WEATHER_TEMPERATURE;
        else if (strTest == "weather.location") ret = WEATHER_LOCATION;
        else if (strTest == "weather.isfetched") ret = WEATHER_IS_FETCHED;
      }
      else if (strCategory == "system")
      {
        if (strTest == "system.date") ret = SYSTEM_DATE;
        else if (strTest == "system.time") ret = SYSTEM_TIME;
        else if (strTest == "system.cputemperature") ret = SYSTEM_CPU_TEMPERATURE;
        else if (strTest == "system.gputemperature") ret = SYSTEM_GPU_TEMPERATURE;
        else if (strTest == "system.fanspeed") ret = SYSTEM_FAN_SPEED;
        else if (strTest == "system.freespace(c)") ret = SYSTEM_FREE_SPACE_C;
        else if (strTest == "system.freespace(e)") ret = SYSTEM_FREE_SPACE_E;
        else if (strTest == "system.freespace(f)") ret = SYSTEM_FREE_SPACE_F;
        else if (strTest == "system.freespace(g)") ret = SYSTEM_FREE_SPACE_G;
        else if (strTest == "system.buildversion") ret = SYSTEM_BUILD_VERSION;
        else if (strTest == "system.builddate") ret = SYSTEM_BUILD_DATE;
        else if (strTest == "system.hasnetwork") ret = SYSTEM_ETHERNET_LINK_ACTIVE;
        else if (strTest == "system.fps") ret = SYSTEM_FPS;
        else if (strTest == "system.kaiconnected") ret = SYSTEM_KAI_CONNECTED;
        else if (strTest == "system.kaienabled") ret = SYSTEM_KAI_ENABLED;
        else if (strTest == "system.hasmediadvd") ret = SYSTEM_MEDIA_DVD;
        else if (strTest == "system.dvdready") ret = SYSTEM_DVDREADY;
        else if (strTest == "system.trayopen") ret = SYSTEM_TRAYOPEN;
        else if (strTest == "system.autodetection") ret = SYSTEM_AUTODETECTION;
        else if (strTest == "system.freememory") ret = SYSTEM_FREE_MEMORY;
        else if (strTest == "system.screenmode") ret = SYSTEM_SCREEN_MODE;
        else if (strTest == "system.screenwidth") ret = SYSTEM_SCREEN_WIDTH;
        else if (strTest == "system.screenheight") ret = SYSTEM_SCREEN_HEIGHT;
        else if (strTest == "system.currentwindow") ret = SYSTEM_CURRENT_WINDOW;
        else if (strTest == "system.currentcontrol") ret = SYSTEM_CURRENT_CONTROL;
        else if (strTest == "system.xboxnickname") ret = SYSTEM_XBOX_NICKNAME;
        else if (strTest == "system.dvdlabel") ret = SYSTEM_DVD_LABEL;
        else if (strTest == "system.haslocks") ret = SYSTEM_HASLOCKS;
        else if (strTest == "system.hasloginscreen") ret = SYSTEM_HAS_LOGINSCREEN;
        else if (strTest == "system.ismaster") ret = SYSTEM_ISMASTER;
        else if (strTest == "system.internetstate") ret = SYSTEM_INTERNET_STATE;
        else if (strTest == "system.loggedon") ret = SYSTEM_LOGGEDON;
        ...
        else if (strTest.Substring(0, 16) == "system.hddsmart(")
        {
          ...
          return AddMultiInfo(new GUIInfo(bNegate ? -SYSTEM_HAS_ALARM : SYSTEM_HAS_ALARM, ConditionalStringParameter(strTest.Substring(16, strTest.Length - 17)), 0));
      ...
      {
        if (strTest == "xlinkkai.username") ret = XLINK_KAI_USERNAME;
      }
      else if (strCategory == "lcd")
      {
        if (strTest == "lcd.playicon") ret = LCD_PLAY_ICON;
        else if (strTest == "lcd.progressbar") ret = LCD_PROGRESS_BAR;
        else if (strTest == "lcd.cputemperature") ret = LCD_CPU_TEMPERATURE;
        else if (strTest == "lcd.gputemperature") ret = LCD_GPU_TEMPERATURE;
        else if (strTest == "lcd.fanspeed") ret = LCD_FAN_SPEED;
        else if (strTest == "lcd.date") ret = LCD_DATE;
        else if (strTest == "lcd.freespace(c)") ret = LCD_FREE_SPACE_C;
        else if (strTest == "lcd.freespace(e)") ret = LCD_FREE_SPACE_E;
        else if (strTest == "lcd.freespace(f)") ret = LCD_FREE_SPACE_F;
        else if (strTest == "lcd.freespace(g)") ret = LCD_FREE_SPACE_G;
      }
      else if (strCategory == "network")
      {
        if (strTest == "network.ipaddress") ret = NETWORK_IP_ADDRESS;
      }
      else if (strCategory == "musicplayer")
      {
        if (strTest == "musicplayer.title") ret = MUSICPLAYER_TITLE;
        else if (strTest == "musicplayer.album") ret = MUSICPLAYER_ALBUM;
        else if (strTest == "musicplayer.artist") ret = MUSICPLAYER_ARTIST;
        else if (strTest == "musicplayer.year") ret = MUSICPLAYER_YEAR;
        else if (strTest == "musicplayer.genre") ret = MUSICPLAYER_GENRE;
        else if (strTest == "musicplayer.time") ret = MUSICPLAYER_TIME;
        else if (strTest == "musicplayer.timeremaining") ret = MUSICPLAYER_TIME_REMAINING;
        else if (strTest == "musicplayer.timespeed") ret = MUSICPLAYER_TIME_SPEED;
        else if (strTest == "musicplayer.tracknumber") ret = MUSICPLAYER_TRACK_NUMBER;
        else if (strTest == "musicplayer.duration") ret = MUSICPLAYER_DURATION;
        else if (strTest == "musicplayer.cover") ret = MUSICPLAYER_COVER;
        else if (strTest == "musicplayer.bitrate") ret = MUSICPLAYER_BITRATE;
        else if (strTest == "musicplayer.playlistlength") ret = MUSICPLAYER_PLAYLISTLEN;
        else if (strTest == "musicplayer.playlistposition") ret = MUSICPLAYER_PLAYLISTPOS;
        else if (strTest == "musicplayer.channels") ret = MUSICPLAYER_CHANNELS;
        else if (strTest == "musicplayer.bitspersample") ret = MUSICPLAYER_BITSPERSAMPLE;
        else if (strTest == "musicplayer.samplerate") ret = MUSICPLAYER_SAMPLERATE;
        else if (strTest == "musicplayer.codec") ret = MUSICPLAYER_CODEC;
        else if (strTest == "musicplayer.discnumber") ret = MUSICPLAYER_DISC_NUMBER;
      }
      else if (strCategory == "videoplayer")
      {
        if (strTest == "videoplayer.title") ret = VIDEOPLAYER_TITLE;
        else if (strTest == "videoplayer.genre") ret = VIDEOPLAYER_GENRE;
        else if (strTest == "videoplayer.director") ret = VIDEOPLAYER_DIRECTOR;
        else if (strTest == "videoplayer.year") ret = VIDEOPLAYER_YEAR;
        else if (strTest == "videoplayer.time") ret = VIDEOPLAYER_TIME;
        else if (strTest == "videoplayer.timeremaining") ret = VIDEOPLAYER_TIME_REMAINING;
        else if (strTest == "videoplayer.timespeed") ret = VIDEOPLAYER_TIME_SPEED;
        else if (strTest == "videoplayer.duration") ret = VIDEOPLAYER_DURATION;
        else if (strTest == "videoplayer.cover") ret = VIDEOPLAYER_COVER;
        else if (strTest == "videoplayer.usingoverlays") ret = VIDEOPLAYER_USING_OVERLAYS;
        else if (strTest == "videoplayer.isfullscreen") ret = VIDEOPLAYER_ISFULLSCREEN;
        else if (strTest == "videoplayer.hasmenu") ret = VIDEOPLAYER_HASMENU;
        else if (strTest == "videoplayer.playlistlength") ret = VIDEOPLAYER_PLAYLISTLEN;
        else if (strTest == "videoplayer.playlistposition") ret = VIDEOPLAYER_PLAYLISTPOS;
      }
      else if (strCategory == "playlist")
      {
        ...
        else if (strTest == "playlist.repeat") ret = PLAYLIST_REPEAT;
        else if (strTest == "playlist.israndom") ret = PLAYLIST_ISRANDOM;
        else if (strTest == "playlist.isrepeat") ret = PLAYLIST_ISREPEAT;
        else if (strTest == "playlist.isrepeatone") ret = PLAYLIST_ISREPEATONE;
      }
      else if (strCategory == "musicpartymode")
      {
        if (strTest == "musicpartymode.enabled") ret = MUSICPM_ENABLED;
        else if (strTest == "musicpartymode.songsplayed") ret = MUSICPM_SONGSPLAYED;
        else if (strTest == "musicpartymode.matchingsongs") ret = MUSICPM_MATCHINGSONGS;
        else if (strTest == "musicpartymode.matchingsongspicked") ret = MUSICPM_MATCHINGSONGSPICKED;
        else if (strTest == "musicpartymode.matchingsongsleft") ret = MUSICPM_MATCHINGSONGSLEFT;
        else if (strTest == "musicpartymode.relaxedsongspicked") ret = MUSICPM_RELAXEDSONGSPICKED;
        else if (strTest == "musicpartymode.randomsongspicked") ret = MUSICPM_RANDOMSONGSPICKED;
      }
      else if (strCategory == "audioscrobbler")
      {
        if (strTest == "audioscrobbler.enabled") ret = AUDIOSCROBBLER_ENABLED;
        else if (strTest == "audioscrobbler.connectstate") ret = AUDIOSCROBBLER_CONN_STATE;
        else if (strTest == "audioscrobbler.submitinterval") ret = AUDIOSCROBBLER_SUBMIT_INT;
      ...
      else if (strCategory == "listitem")
      {
        ...
        else if (strTest == "listitem.icon") ret = LISTITEM_ICON;
        else if (strTest == "listitem.label") ret = LISTITEM_LABEL;
        else if (strTest == "listitem.title") ret = LISTITEM_TITLE;
        else if (strTest == "listitem.tracknumber") ret = LISTITEM_TRACKNUMBER;
        else if (strTest == "listitem.artist") ret = LISTITEM_ARTIST;
        else if (strTest == "listitem.album") ret = LISTITEM_ALBUM;
        else if (strTest == "listitem.year") ret = LISTITEM_YEAR;
        else if (strTest == "listitem.genre") ret = LISTITEM_GENRE;
        else if (strTest == "listitem.director") ret = LISTITEM_DIRECTOR;
      }
      else if (strCategory == "visualisation")
      {
        if (strTest == "visualisation.locked") ret = VISUALISATION_LOCKED;
        else if (strTest == "visualisation.preset") ret = VISUALISATION_PRESET;
        else if (strTest == "visualisation.name") ret = VISUALISATION_NAME;
        else if (strTest == "visualisation.enabled") ret = VISUALISATION_ENABLED;
      }
    ...
    static int TranslateBooleanExpression(string expression)
    {
      CCombinedValue comb = new CCombinedValue();
      comb.m_info = expression;
      comb.m_id = GUIInfoManager.COMBINED_VALUES_START + m_CombinedValues.Count;
    ...
    static bool EvaluateBooleanExpression(CCombinedValue expression, ref bool result, int dwContextWindow)
    {
      ...
      for (int i = 0; i < expression.m_postfix.Count; ++i)
      {
        ...
        { // AND the top two items on the stack
          if (save.Count < 2) return false;
          bool right = save.Peek(); save.Pop();
          bool left = save.Peek(); save.Pop();
    ...
    public static int TranslateWindowString(string strWindow)
    {
      ...
      else if (strWindow.Equals("myfiles")) wWindowID = (int)GUIWindow.Window.WINDOW_FILES;
      else if (strWindow.Equals("settings")) wWindowID = (int)GUIWindow.Window.WINDOW_SETTINGS;
      else if (strWindow.Equals("mymusic")) wWindowID = (int)GUIWindow.Window.WINDOW_MUSIC;
      else if (strWindow.Equals("mymusicfiles")) wWindowID = (int)GUIWindow.Window.WINDOW_MUSIC_FILES;
      else if (strWindow.Equals("myvideos")) wWindowID = (int)GUIWindow.Window.WINDOW_VIDEOS;
      else if (strWindow.Equals("systeminfo")) wWindowID = (int)GUIWindow.Window.WINDOW_SYSTEM_INFORMATION;
      //else if (strWindow.Equals("guicalibration")) wWindowID = (int)GUIWindow.Window.WINDOW_SCREEN_CALIBRATION;
      else if (strWindow.Equals("screencalibration")) wWindowID = (int)GUIWindow.Window.WINDOW_UI_CALIBRATION;
      else if (strWindow.Equals("mypicturessettings")) wWindowID = (int)GUIWindow.Window.WINDOW_SETTINGS_SLIDESHOW;
      ...
      else if (strWindow.Equals("mymusicsettings")) wWindowID = (int)GUIWindow.Window.WINDOW_SETTINGS_MUSIC;
      ...
      else if (strWindow.Equals("scripts")) wWindowID = (int)GUIWindow.Window.WINDOW_SCRIPTS;
      else if (strWindow.Equals("myvideofiles")) wWindowID = (int)GUIWindow.Window.WINDOW_VIDEOS;
      else if (strWindow.Equals("myvideogenres")) wWindowID = (int)GUIWindow.Window.WINDOW_VIDEO_GENRE;
      else if (strWindow.Equals("myvideoactors")) wWindowID = (int)GUIWindow.Window.WINDOW_VIDEO_ACTOR;
      else if (strWindow.Equals("myvideoyears")) wWindowID = (int)GUIWindow.Window.WINDOW_VIDEO_YEAR;
      else if (strWindow.Equals("myvideotitles")) wWindowID = (int)GUIWindow.Window.WINDOW_VIDEO_TITLE;
      else if (strWindow.Equals("myvideoplaylist")) wWindowID = (int)GUIWindow.Window.WINDOW_VIDEO_PLAYLIST;
      //else if (strWindow.Equals("profiles")) wWindowID = (int)GUIWindow.Window.WINDOW_SETTINGS_PROFILES;
      else if (strWindow.Equals("yesnodialog")) wWindowID = (int)GUIWindow.Window.WINDOW_DIALOG_YES_NO;
      else if (strWindow.Equals("progressdialog")) wWindowID = (int)GUIWindow.Window.WINDOW_DIALOG_PROGRESS;
      ...
      else if (strWindow.Equals("mymusicplaylist")) wWindowID = (int)GUIWindow.Window.WINDOW_MUSIC_PLAYLIST;
      else if (strWindow.Equals("mymusicfiles")) wWindowID = (int)GUIWindow.Window.WINDOW_MUSIC_FILES;
      ...
      //  else if (strWindow.Equals("virtualkeyboard")) wWindowID = (int)GUIWindow.Window.WINDOW_VIRTUAL_KEYBOARD;
      else if (strWindow.Equals("selectdialog")) wWindowID = (int)GUIWindow.Window.WINDOW_DIALOG_SELECT;
      else if (strWindow.Equals("musicinformation")) wWindowID = (int)GUIWindow.Window.WINDOW_MUSIC_INFO;
      else if (strWindow.Equals("okdialog")) wWindowID = (int)GUIWindow.Window.WINDOW_DIALOG_OK;
      else if (strWindow.Equals("movieinformation")) wWindowID = (int)GUIWindow.Window.WINDOW_VIDEO_INFO;
      //else if (strWindow.Equals("scriptsdebuginfo")) wWindowID = (int)GUIWindow.Window.WINDOW_SCRIPTS_INFO;
      else if (strWindow.Equals("fullscreenvideo")) wWindowID = (int)GUIWindow.Window.WINDOW_FULLSCREEN_VIDEO;
      else if (strWindow.Equals("visualisation")) wWindowID = (int)GUIWindow.Window.WINDOW_VISUALISATION;
      else if (strWindow.Equals("slideshow")) wWindowID = (int)GUIWindow.Window.WINDOW_SLIDESHOW;
      else if (strWindow.Equals("filestackingdialog")) wWindowID = (int)GUIWindow.Window.WINDOW_DIALOG_FILESTACKING;
      else if (strWindow.Equals("weather")) wWindowID = (int)GUIWindow.Window.WINDOW_WEATHER;
      //else if (strWindow.Equals("xlinkkai")) wWindowID = (int)GUIWindow.Window.WINDOW_BUDDIES;
      else if (strWindow.Equals("screensaver")) wWindowID = (int)GUIWindow.Window.WINDOW_SCREENSAVER;
      else if (strWindow.Equals("videoosd")) wWindowID = (int)GUIWindow.Window.WINDOW_OSD;
      ...
      else if (strWindow.Equals("startwindow")) wWindowID = (int)GUIWindow.Window.WINDOW_HOME;
    ...
    public static bool GetBool(int condition1, int dwContextWindow)
    {
      ...
      else if (condition >= MULTI_INFO_START && condition <= MULTI_INFO_END)
      {
        ...
        bReturn = false;// bReturn = g_settings.m_vecProfiles[0].getLockMode() != LOCK_MODE_EVERYONE && g_passwordManager.bMasterUser;
    ...
    static bool GetMultiInfoBool(GUIInfo info, int dwContextWindow)
    {
      bool bReturn = false;
    ...
    static void CacheBool(int condition, int contextWindow, bool result)
    {
    ...
    static bool IsCached(int condition, int contextWindow, ref bool result)
    {
      // windows have id's up to 13100 or thereabouts (ie 2^14 needed)
      // conditionals have id's up to 100000 or thereabouts (ie 2^18 needed)
      ...
      {
        int hash = ((contextWindow & 0x3fff) << 18) | (condition & 0x3ffff);
        if (m_boolCache.ContainsKey(hash)) return m_boolCache[hash];
