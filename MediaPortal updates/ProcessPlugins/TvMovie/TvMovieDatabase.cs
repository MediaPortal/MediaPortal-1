...
namespace ProcessPlugins.TvMovie
{
  class TvMovieDatabase
  {
    ...
    private int ImportStation(string stationName, ArrayList channelNames)
    {
      ...
      {
        Log.Write("TVMovie: Error accessing TV Movie Clickfinder database - Current import canceled, waiting for next schedule");
        Log.Write("TVMovie: Exception: {0}", ex);
        return 0;
      }
      finally
      {
    ...
    public void Import()
    {
      ...
      Log.Write("TVMovie: Imported {0} database entries for {1} stations", _programsCounter, counter);
    }
  }
