  ...
  public class NVPPowerScheduler : IPluginReceiver, ISetupForm, IWakeable
  {
    ...
    private void WakeupManager()
    {
      ...
      foreach (IWakeable wakeable in wakeables)
      {
        ...
        if (m_bExtensiveLog) Log.Write(" PowerScheduler: tmpNextStarttime {0} earliestPossibleRecording {1} - {2}", tmpNextStarttime, earliestStarttime, wakeable.PluginName());
    ...
    private void ShutdownManager()
    {
      if (m_bExtensiveLog)
      {
        ...
        Log.Write("   - Database Update        = " + TVDatabase.SupressEvents.ToString());
      }
      ...
      if (m_iActiveWindow != (int)GUIWindowManager.ActiveWindow)
      {
            ...
            (m_iActiveWindow == (int)GUIWindow.Window.WINDOW_SECOND_HOME))
        {
              ...
              (Recorder.IsRecording()) )                 //are we recording something? 
          {
            ...
            if (m_bExtensiveLog)
              Log.Write(" PowerScheduler: shutdown disabled - we are playing something or a DB update is running");
    ...
    private void ShutdownCheck()
    {
      ...
      if (m_bShutdownEnabled && (DateTime.Now.CompareTo(m_dtShutdownTime) > 0))
      {
        ...
        {
          Log.Write("PowerScheduler: Shutdown process aborted by TVDatabase");
          abortshutdown = true;
        }
    ...
    static private DateTime GetNextRecordingStarttime(DateTime earliestStarttime)
    {
      ...
      if (TVDatabase.GetRecordings(ref recordings))
      {
        foreach (TVRecording recording in recordings)
        {
          ...
          {
            Log.Write(" PowerScheduler: skipping canceled recording - {0}", recording.Title);
            continue;
          }
    ...
    public bool WndProc( ref System.Windows.Forms.Message msg )
    {
      ...
      {
        Log.Write("PowerScheduler: WM_POWERBROADCAST: {0}", msg.WParam.ToInt32());
        switch ( msg.WParam.ToInt32() )
        {
          ...
          case PBT_APMSUSPEND:
            if ( m_SDTimer.Enabled )
            {
              m_SDTimer.Stop();
              if ( m_bExtensiveLog )
                Log.Write("PowerScheduler: SDTimer.Stop()");
            }
            break;
          ...
          case PBT_APMRESUMEAUTOMATIC:
            if ( !m_SDTimer.Enabled )
            {
              m_SDTimer.Start();
              if ( m_bExtensiveLog )
                Log.Write("PowerScheduler: WndProc -> SDTimer.Start()");
            }
            break;
        }
      }
