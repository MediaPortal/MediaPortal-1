...
namespace MediaPortal.Tests.Teletext
{
  ...
  public class TeletextDecoderTest
  {
    ...
    public void DecodeStreamMode()
    {
      int[] transmission = new int[]
      {
        0x1FF,0x1FF,0x1FF,0x1FF,0x100, 0x205,0x206,0x207,0x208,0x209, 0x309,0x310,0x311,0x312,0x313,0x314,0x315,
        0x101,0x102,0x103,0x104,0x105, 0x210,0x211,0x212,0x213,0x214, 0x316,0x317,0x318,0x319,0x320,0x321,0x322,
        0x106,0x107,0x108,0x109,0x110, 0x215,0x216,0x217,0x218,0x219, 0x323,0x3FF,0x3FF,0x3FF,0x830,0x3FF,0x300,
        0x111,0x112,0x113,0x114,0x115, 0x220,0x221,0x222,0x223,0x200, 0x301,0x302,0x303,0x304,0x305,0x306,0x307,
        0x116,0x117,0x118,0x119,0x120, 0x201,0x202,0x203,0x204,0x205, 0x308,0x309,0x310,0x311,0x312,0x313,0x314,
        0x121,0x122,0x123,0x1FF,0x100, 0x206,0x207,0x208,0x209,0x210, 0x315,0x316,0x317,0x318,0x319,0x320,0x321,
        0x101,0x102,0x103,0x104,0x105, 0x211,0x212,0x213,0x214,0x215, 0x322,0x323,0x3FF,0x3ff,0x3ff,0x3ff,0x3ff,
        0x1ff,0x1ff,0x1ff,0x1ff,0x1ff, 0x216,0x217,0x218,0x219,0x220, 0x3ff,0x3ff,0x3FF,0x3ff,0x3ff,0x3ff,0x3ff,
        0x1ff,0x1ff,0x1ff,0x1ff,0x1ff, 0x221,0x222,0x223,0x2ff,0x2ff, 0x3ff,0x3ff,0x3FF,0x3ff,0x3ff,0x3ff,0x3ff
      };
      TeletextPageCache cache = new TeletextPageCache();
      TeletextDecoder decoder = new TeletextDecoder(ref cache);
    ...
    public void FilterOutIllegalRows()
    {
      ...
      {
        byte[] row;
        if (i == 0) row = CreateHeader(0x100, 0);
        else row = CreateRow(0x100, i);
        decoder.Decode(row, 1);
      }
    ...
    void VerifyRows(ref byte[] pageData, bool fastText)
    {
      for (int i = 0; i <= 31; ++i)
      {
    ...
    bool ComparePage(byte[] page1, byte[] page2)
    {
      for (int x = 0; x < page1.Length; ++x)
      {
        if (page1[x] != page2[x]) return false;
      }
      return true;
    }

    byte[] CreateHeader(int pageNr, int subNr)
    {
      byte[] rows = new byte[42];
      for (int i = 0; i < rows.Length; ++i) rows[i] = (byte)(i & 0x7f);
      Hamming.SetHeader(0, ref rows, pageNr, subNr);
      return rows;
    }

    byte[] CreateRow(int pageNr, int packetNumber)
    {
      byte[] rows = new byte[42];
      for (int i = 0; i < rows.Length; ++i) rows[i] = (byte)(i & 0x7f);
    ...
    byte[] CreatePage(int pageNr, int subNr)
    {
