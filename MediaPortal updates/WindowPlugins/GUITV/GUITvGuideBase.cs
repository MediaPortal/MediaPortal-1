...
namespace MediaPortal.GUI.TV
{
  ...
  public class GUITvGuideBase : GUIWindow
  {
    ...
    {
      if (_currentProgram == null) return;      
      Log.Write("TVGuide: _currentProgram.Title = {0}", _currentProgram.Title);
      Log.Write("TVGuide: _currentProgram.StarRating = {0}", _currentProgram.StarRating);
      ...
      if (isItemSelected)
      {
            ...
            _currentProgram.EndTime <= DateTime.Now)
        {
          ...
          try
          {
            Log.Write("TVGuide: IsAnyCardRecording: {0}", Convert.ToString(Recorder.IsAnyCardRecording()));
            ...
            if (Recorder.IsAnyCardRecording())
            {
              ...
              foreach (TVRecording rec in _recordingList)
              {
                if (rec.Title.Equals(_currentProgram.Title))
                {
                  ...
                  Log.Write("TVGuide: clicked on a currently running recording");
                  GUIDialogMenu dlg = (GUIDialogMenu)GUIWindowManager.GetWindow((int)GUIWindow.Window.WINDOW_DIALOG_MENU);
                  if (dlg == null)
                    return;
                  ...
                  if (_recordingList != null)
                  {
                    Log.Write("TVGuide: Found current program {0} in recording list", _currentTitle);
                    switch (dlg.SelectedId)
                    {
                      case 979: // Play recording from beginning                          
                        ...
                        {
                          Log.Write("TVGuide: Play recording {0} from start", _currentTitle);
                      ...
                      case 980: // Play recording from live point
                        ...
                        if (Recorder.IsViewing())
                        {
                          if (g_Player.Playing)
                          {
                            Log.Write("TVGuide: Show recording {0} at live point", _currentTitle);
                    ...
                    Log.Write("EPG: _recordingList was not available");
                }
              }
