  ...
  public class MusicDatabase
  {
    ...
    static void Open()
    {
      ...
      try
      {
        ...
        if (!File.Exists(@"database\MusicDatabaseV7.db3"))
        {
          if (!File.Exists(@"database\MusicDatabaseV6.db3"))
          {
            if (File.Exists(@"database\musicdatabase5.db3"))
            {
              File.Copy((@"database\musicdatabase5.db3"), (@"database\MusicDatabaseV7.db3"), false);
            }
            else
              Log.Write("**** Please rescan your music shares ****");
          }
          else
          {
            if (UpdateDB_V6_to_V7())
            {
              Log.Write("MusicDatabaseV7: old V6 database successfully updated");
              File.Copy((@"database\MusicDatabaseV6.db3"), (@"database\MusicDatabaseV7.db3"), false);
            }
            else
              Log.Write("MusicDatabaseV6: error while trying to update your database to V7");
          }
        }

        m_db = new SQLiteClient(@"database\MusicDatabaseV7.db3");
    ...
    {
      bool success = true;
      m_db = new SQLiteClient(@"database\MusicDatabaseV6.db3");
    ...
    public bool GetSongsByPath2(string strPath1, ref List<SongMap> songs)
    {
      ...
      {
        Log.WriteFile(Log.LogType.Log, true, "GetSongsByPath2:musicdatabase  {0} exception err:{1} stack:{2}", strSQL, ex.Message, ex.StackTrace);
        Open();
      }
    ...
    public int MusicDatabaseReorg(ArrayList shares, bool treatFolderAsAlbum, bool scanForVariousArtists)
    {
      ...
      try
      {
        Log.Write("Musicdatabasereorg: Beginning music database reorganization...");
      ...
      finally
      {
        ...
        Log.Write("Musicdatabasereorg: Music database reorganization done.  Processed {0} tracks in: {1:d2}:{2:d2}:{3:d2}{4}",
    ...
    private void UpdateAlbumArtistsCounts(int startProgress, int endProgress)
    {
      ...
      try
      {
        foreach (AlbumInfoCache album in m_albumCache)
        {
          ...
          if (artistCount > 1)
          {
            ...
            Log.Write("Musicdatabasereorg: multiple album artists album found: {0}.  Updating album artist count ({1}).", album.Album, artistCount);

            foreach (DictionaryEntry entry in artistCountTable)
            {
              Song s = (Song)entry.Value;
              Log.Write("   ArtistID:{0}  Artist Name:{1}  Track Title:{2}", s.artistId, s.Artist, s.Title);
            }
          }
        ...
        if (ScanForVariousArtists && variousArtistsFound)
        {
          ...
          {
            Log.Write("Musicdatabasereorg: updating artist id's for 'Various Artists' albums");
      ...
      {
        Log.Write("Musicdatabasereorg: {0}", ex);
      }
    }
    ...
    private void UpdateSortableArtistNames()
    {
      ...
      for (int i = 0; i < artists.Count; i++)
      {
        ...
        {
          Log.Write("UpdateSortableArtistNames: {0}", ex);
        }
      }
    }
    ...
    public int AddScrobbleUser(string userName_)
    {
      ...
      try
      {
        ...
        if (results.Rows.Count == 0)
        {
          ...
          m_db.Execute(strSQL);
          Log.Write("MusicDatabase: added scrobbleuser {0} with ID {1}", strUserName, Convert.ToString(m_db.LastInsertID()));
    ...
    public int AddScrobbleUserSettings(string userID_, string fieldName_, int fieldValue_)
    {
      ...
      try
      {
        ...
        //Log.Write("MusicDatabase: updating settings with ID {0}", currentSettingID);
        ...
        if (results.Rows.Count == 0)
        {
          ...
          m_db.Execute(strSQL);
          Log.Write("MusicDatabase: added scrobblesetting {0} for userid {1}", Convert.ToString(m_db.LastInsertID()), userID_);
          if (fieldValue_ > -1)
            return m_db.LastInsertID();
          else
            return fieldValue_;
    ...
    public bool DeleteScrobbleUser(string userName_)
    {
      ...
      try
      {
        ...
        {
          Log.Write("MusicDatabase: could not delete settings for scrobbleuser {0} with ID {1}", strUserName, strUserID);
          return false;
        }
      }
      catch (Exception ex)
