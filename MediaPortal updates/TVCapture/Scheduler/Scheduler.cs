  ...
  public class Scheduler
  {
    ...
    public void Process(CommandProcessor handler)
    {
      ...
      for (int i = 0; i < _tvChannelsList.Count; ++i)
      {
        ...
        //Log.Write(string.Format("Scheduler: checking for records on this channel: {0} between {1} and {2}", chan.Name, Convert.ToString(dtStart), Convert.ToString(dtEnd)),"");
      ...
      for (int j = 0; j < _recordingsList.Count; ++j)
      {
        ...
        if (rec.IsRecordingProgramAtTime(DateTime.Now, null, paddingFront, paddingEnd))
        {
          ...
          if (!handler.IsRecordingSchedule(rec, out card))
          {
            ...
            {
              Log.Write("Scheduler: manual recording started!", "");
            }
            else
              Log.Write("Scheduler: manual recording did not start properly!", "");
          }
        }
        else
        {
