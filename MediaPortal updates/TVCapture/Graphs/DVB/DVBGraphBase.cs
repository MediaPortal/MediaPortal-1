...
namespace MediaPortal.TV.Recording
{
  ...
  public class DVBGraphBase : MediaPortal.TV.Recording.IGraph, IHardwarePidFiltering
  {
    ...
    public bool StartRecording( Hashtable attributes, TVRecording recording, TVChannel channel, ref string strFileName, bool bContentRecording, DateTime timeProgStart )
    {
      ...
      Log.Write("DVBGraph:StartRecording()");
      ...
      try
      {
        ...
        if ( !success )
        {
          Log.Write("DVBGraph:StartRecording() FAILED to create recording");
          return false;
        }
        long lStartTime = 0;
        ...
        if ( !bContentRecording )
        {
          ...
          if ( timeProgStart.Year > 2000 )
          {
            ...
            Log.Write("DVBGraph: Start recording from {0}:{1:00}:{2:00} which is {3:00}:{4:00}:{5:00} in the past",
            ...
            Log.Write("DVBGraph: record entire timeshift buffer");
    ...
    public void StopRecording()
    {
      if ( _graphState != State.Recording )
        return;
      Log.Write("DVBGraph:stop recording...");
      if ( m_recorderId >= 0 )
      {
    ...
    public bool StartViewing( TVChannel channel )
    {
      if ( _graphState != State.Created )
        return false;
      Log.Write("DVBGraph:StartViewing() {0}", channel.Name);
      ...
      if ( _currentTuningObject.ServiceType == Mpeg4VideoServiceType )
      {
        ...
        if ( _graphBuilder.Render(_pinDemuxerVideoMPEG4) != 0 )
        {
          ...
          Log.Write("DVBGraph:Failed to render MPEG4 video out pin MPEG-2 Demultiplexer");
          return false;
        }
      }
      else
      {
        ...
        if ( _graphBuilder.Render(_pinDemuxerVideo) != 0 )
        {
          ...
          Log.Write("DVBGraph:Failed to render video out pin MPEG-2 Demultiplexer");
          return false;
        }
      ...
      if ( _isUsingAC3 )
        Log.Write("DVBGraph: channel {0} uses AC3", channel.Name);
      else
        Log.Write("DVBGraph: channel {0} uses MP2 audio", channel.Name);
      ...
      if ( !_isUsingAC3 )
      {
        ...
        if ( _graphBuilder.Render(_pinDemuxerAudio) != 0 )
        {
          ...
          Log.Write("DVBGraph:Failed to render audio out pin MPEG-2 Demultiplexer");
          return false;
        }
      ...
      else
      {
        ...
        if ( _graphBuilder.Render(_pinAC3Out) != 0 )
        {
          ...
          Log.Write("DVBGraph:Failed to render AC3 pin MPEG-2 Demultiplexer");
          return false;
        }
      }
      ...
      if ( useOverlay )
      {
        ...
        if ( _videoWindowInterface == null )
        {
          ...
          Log.Write("DVBGraph:FAILED:Unable to get IVideoWindow");
          return false;
        }
        ...
        if ( _basicVideoInterFace == null )
        {
          ...
          Log.Write("DVBGraph:FAILED:Unable to get IBasicVideo2");
          return false;
        }
        ...
        if ( hr != 0 )
          Log.Write("DVBGraph: FAILED:set Video window:0x{0:X}", hr);
        ...
        if ( hr != 0 )
          Log.Write("DVBGraph: FAILED:set Video window style:0x{0:X}", hr);

        //show overlay window
        hr = _videoWindowInterface.put_Visible(OABool.True);
        if ( hr != 0 )
          Log.Write("DVBGraph: FAILED:put_Visible:0x{0:X}", hr);
      }
      ...
      if ( hr < 0 )
      {
        ...
        Log.Write("DVBGraph: FAILED unable to start graph :0x{0:X}", hr);
        return false;
      }
    ...
    public bool StopViewing()
    {
      ...
      GUIGraphicsContext.OnVideoWindowChanged -= new VideoWindowChangedHandler(GUIGraphicsContext_OnVideoWindowChanged);
      Log.Write("DVBGraph: StopViewing()");
      if ( _videoWindowInterface != null )
    ...
    public bool StartTimeShifting( TVChannel channel, string strFileName )
    {
      ...
      if ( freeSpace < ( 1024L * 1024L * 1024L ) )// 1 GB
      {
        ...
        Log.Write("Recorder:  failed to start timeshifting since drive {0}: has less then 1GB freediskspace", strFileName[0]);
        return false;
      }
      ...
      Log.Write("DVBGraph:StartTimeShifting() {0}", channel.Name);
      ...
      {
        Log.Write("DVBGraph:Unable to create sinksource()");
        return false;
      }
    ...
    public bool StopTimeShifting()
    {
      if ( _graphState != State.TimeShifting )
        return false;
      Log.Write("DVBGraph: StopTimeShifting()");
    ...
    public void SetAudioLanguage( int audioPid )
    {
      if ( audioPid == _currentTuningObject.AudioPid )
        return;
      Log.Write("DVBGraph: change audio pid {0:X}-> pid:{1:X} {2}", _currentTuningObject.AudioPid, audioPid, _graphState);
      ...
      if ( audioPid == _currentTuningObject.AC3Pid )
      {
        ...
        if ( pin == null )
        {
          ...
          if ( _mediaControl != null )
          {
            Log.Write("DVBGraph: stop graph");
            _mediaControl.Stop();
            Log.Write("DVBGraph: disconnect MP2 pin");
            _pinDemuxerAudio.Disconnect();
            Log.Write("DVBGraph: connect AC3 pin");
            _graphBuilder.Render(_pinAC3Out);
            Log.Write("DVBGraph: start graph");
            _mediaControl.Run();
          }
        }
        else
      ...
      else
      {
        ...
        if ( pin == null )
        {
          ...
          if ( _mediaControl != null )
          {
            Log.Write("DVBGraph: stop graph");
            _mediaControl.Stop();
            Log.Write("DVBGraph: disconnect AC3 pin");
            _pinAC3Out.Disconnect();
            Log.Write("DVBGraph: connect MP2 pin");
      ...
      {
        Log.Write("DVBGraph: resending PMT to CAM");
        UpdateCAM();
      }
    ...
    protected virtual bool CreateSinkSource( string fileName, bool useAC3 )
    {
      ...
      try
      {
        ...
        if ( hr != 0 )
        {
          ...
          Log.Write("DVBGraph:FAILED cannot add StreamBufferSink:{0:X}", hr);
          return false;
        }
        ...
        if ( hr != 0 )
        {
          ...
          Log.Write("DVBGraph:FAILED cannot add mpeg2 analyzer to graph:{0:X}", hr);
          return false;
        }
        ...
        if ( pinObj0 == null )
        {
          ...
          Log.Write("DVBGraph:FAILED cannot find mpeg2 analyzer input pin");
          return false;
        }
        ...
        {
          _lastError = String.Format("failed connect mpeg2 demuxer to mpeg2 analyzer");
          Log.Write("DVBGraph:FAILED to connect demux video output->mpeg2 analyzer:{0:X}", hr);
          return false;
        }
        ...
        {
          _lastError = String.Format("failed connect mpeg2 demuxer to mpeg2 analyzer");
          Log.Write("DVBGraph:FAILED cannot find mpeg2 analyzer output pin:{0:X}", hr);
          return false;
        }
        ...
        {
          _lastError = String.Format("failed connect mpeg2 demuxer to mpeg2 analyzer");
          Log.Write("DVBGraph:FAILED cannot find SBE input pin:{0:X}", hr);
          return false;
        }
        ...
        {
          _lastError = String.Format("failed connect mpeg2 demuxer to mpeg2 analyzer");
          Log.Write("DVBGraph:FAILED to connect mpeg2 analyzer->streambuffer sink:{0:X}", hr);
          return false;
        }
        ...
        if ( !useAC3 )
        {
          ...
          if ( hr != 0 )
          {
            ...
            Log.Write("DVBGraph:FAILED cannot find SBE input pin#2");
            return false;
          }
          ...
          {
            _lastError = String.Format("failed connect stream buffer sink");
            Log.Write("DVBGraph:FAILED to connect mpeg2 demuxer audio out->streambuffer sink in#2:{0:X}", hr);
            return false;
          }
        }
        else
        {
          ...
          if ( _pinAC3Out != null )
          {
            ...
            {
              _lastError = String.Format("failed connect stream buffer sink");
              Log.Write("DVBGraph:FAILED to connect mpeg2 demuxer AC3 out->streambuffer sink in#2:{0:X}", hr);
              return false;
            }
          }
          else
          {
            ...
            Log.Write("DVBGraph:FAILED ac3 pin not found?");
          }
        }
        ...
        if ( hr != 0 )
        {
          ...
          Log.Write("DVBGraph:FAILED to set timeshift folder to:{0} {1:X}", strDir, hr);
          return false;
        }
        ...
        if ( hr != 0 )
        {
          ...
          Log.Write("DVBGraph:FAILED to set timeshifting files to 6-8 {0:X}", hr);
          return false;
        }
        ...
        if ( hr != 0 )
        {
          ...
          Log.Write("DVBGraph:FAILED to set timeshifting filesduration to {0} {1:X}", iFileDuration, hr);
          return false;
        }
        ...
        if ( hr != 0 && hr != 1 )
        {
          ...
          Log.Write("DVBGraph:FAILED to set timeshift file to:{0} {1:X}", fileName, hr);
          return false;
        }
      }
      finally
    ...
    protected bool SendPMT()
    {
      try
      {
        ...
        {
          //PMT is not on disk...
          Log.Write("pmt {0} not found", pmtName);
          _pmtRetyCount = 0;
          _lastPMTVersion = -1;
        ...
        if ( pmt.Length < 6 )
        {
          ...
          Log.Write("pmt {0} invalid", pmtName);
          _pmtRetyCount = 0;
          _lastPMTVersion = -1;
          _refreshPmtTable = true;
          return false;
        ...
        {
          //decoding failed
          Log.Write("pmt {0} failed to decode", pmtName);
          _pmtRetyCount = 0;
          _lastPMTVersion = -1;
          _refreshPmtTable = true;
          return false;
    ...
    protected void GetDisEqcSettings( ref DVBChannel ch, out int lowOsc, out int hiOsc, out int lnbKhzTone, out int diseqcUsed )
    {
      ...
      try
      {
        ...
        using ( MediaPortal.Profile.Settings xmlreader = new MediaPortal.Profile.Settings(filename) )
        {
          ...
          switch (ch.DiSEqC)
          {
            ...
            case 4:
              ...
              break;
            default:
              Log.Write("DVBGraph: unknown LNB#:{0}; using defaults: LNB:{1} kHz diseqc:{2} lnbKind:{3}", ch.DiSEqC, lnbKhz, diseqc, lnbKind);
              break;
          }// switch(disNo)
        ...
        Log.Write("DVBGraph: LNB#{0} Settings: freq={1} lnbKHz={2} lnbFreq={3} diseqc={4}", ch.DiSEqC, ch.Frequency, ch.LnbSwitchFrequency, ch.LNBFrequency, diseqcUsed);
      }
      catch ( Exception )
      {
      }
        ...
        case NetworkType.ATSC:
          {
            ...
            {
              Log.Write("DVBGraph:database invalid tuning details for channel:{0}", channel.ID);
              return;
            }
            frequency = 0;
            symbolrate = 0;
            Log.Write("DVBGraph:  tuning details: frequency:{0} kHz physicalChannel:{1} major channel:{2} minor channel:{3} modulation:{4} ONID:{5} TSID:{6} SID:{7} provider:{8} video:0x{9:X} audio:0x{10:X} pcr:0x{11:X}",
        ...
        case NetworkType.DVBC:
          {
            ...
            Log.Write("DVBGraph:  tuning details: frequency:{0} kHz symbolrate:{1} innerFec:{2} modulation:{3} ONID:{4} TSID:{5} SID:{6} provider:{7}",
        ...
        case NetworkType.DVBS:
          {
            ...
            Log.Write("DVBGraph:  tuning details: frequency:{0} kHz polarisation:{1} innerFec:{2} symbolrate:{3} ONID:{4} TSID:{5} SID:{6} provider:{7}",
        ...
        case NetworkType.DVBT:
          {
            ...
            Log.Write("DVBGraph:  tuning details: frequency:{0} kHz ONID:{1} TSID:{2} SID:{3} provider:{4}", frequency, ONID, TSID, SID, providerName);
    ...
    public void TuneChannel( TVChannel channel )
    {
      ...
      try
      {
        ...
        Log.Write("DVBGraph:TuneChannel() tune to channel:{0}", channel.ID);
    ...
    public void StoreChannels( int ID, bool radio, bool tv, ref int newChannels, ref int updatedChannels, ref int newRadioChannels, ref int updatedRadioChannels )
    {
      ...
      if ( _filterDvbAnalyzer == null )
        return;
      Log.Write("DVBGraph: StoreChannels() signal level:{0} signal quality:{1} locked:{2}", SignalStrength(), SignalQuality(), _tunerLocked);
      ...
      if ( Network() == NetworkType.ATSC )
      {
        ...
        if ( count == 0 )
        {
          Log.Write("DVBGraph: found 0 channels");
          return;
        }
      ...
      {
        Log.Write("DVBGraph: found no channels", transp.channels);
        return;
      }

      Log.Write("DVBGraph: found {0}", transp.channels.Count);
      for ( int i = 0; i < transp.channels.Count; ++i )
      {
        ...
        {
          Log.Write("DVBGraph:unknown service type: provider:{0} service:{1} scrambled:{2} frequency:{3} kHz networkid:{4} transportid:{5} serviceid:{6} tv:{7} radio:{8} audiopid:0x{9:X} videopid:0x{10:X} teletextpid:0x{11:X} program:{12} pcr pid:0x{13:X} service type:{14} major:{15} minor:{16}",
                                            info.service_provider_name,
                                            info.service_name,
                                            info.scrambled,
                                            info.freq,
        ...
        Log.Write("DVBGraph:Found provider:{0} service:{1} scrambled:{2} frequency:{3} kHz networkid:{4} transportid:{5} serviceid:{6} tv:{7} radio:{8} audiopid:0x{9:X} videopid:0x{10:X} teletextpid:0x{11:X} program:{12} pcr pid:0x{13:X} ac3 pid:0x{14:X} major:{15} minor:{16} LCN:{17} type:{18}",
                                            info.service_provider_name,
                                            info.service_name,
                                            info.scrambled,
                                            info.freq,
        ...
        if ( info.serviceID == 0 )
        {
          Log.Write("DVBGraph: channel#{0} has no service id", i);
          continue;
        }
        ...
        if ( info.serviceType == Mpeg2VideoServiceType || info.serviceType == Mpeg4VideoServiceType )//tv
        {
          ...
          Log.Write("DVBGraph: channel {0} is a tv channel", newchannel.ServiceName);
          ...
          if ( isNewChannel )
          {
            ...
            Log.Write("DVBGraph: add new channel for {0}:{1}:{2}", existingTvChannel.Name, existingTvChannel.Number, existingTvChannel.Sort);
            int id = TVDatabase.AddChannel(existingTvChannel);
            if ( id < 0 )
            {
              Log.Write("DVBGraph: failed to add new channel for {0}:{1}:{2} to database", existingTvChannel.Name, existingTvChannel.Number, existingTvChannel.Sort);
            }
            channelId = id;
            newChannels++;
          }
          else
          {
            ...
            updatedChannels++;
            Log.Write("DVBGraph: update channel {0}:{1}:{2} {3}", existingTvChannel.Name, existingTvChannel.Number, existingTvChannel.Sort, existingTvChannel.ID);
          }

          if ( Network() == NetworkType.DVBT )
          {
            Log.Write("DVBGraph: map channel {0} id:{1} to DVBT card:{2}", newchannel.ServiceName, channelId, ID);
          ...
          {
            Log.Write("DVBGraph: map channel {0} id:{1} to DVBC card:{2}", newchannel.ServiceName, channelId, ID);
          ...
          if ( Network() == NetworkType.ATSC )
          {
            Log.Write("DVBGraph: map channel {0} id:{1} to ATSC card:{2}", newchannel.ServiceName, channelId, ID);
          ...
          {
            Log.Write("DVBGraph: map channel {0} id:{1} to DVBS card:{2}", newchannel.ServiceName, channelId, ID);
        ...
        else if ( info.serviceType == Mpeg2AudioServiceType ) //radio
        {
          ...
          if ( isNewChannel )
          {
            ...
            station.Scrambled = info.scrambled;
            Log.Write("DVBGraph: add new radio channel for {0} {1}", station.Name, station.Frequency);
            int id = RadioDatabase.AddStation(ref station);
            if ( id < 0 )
            {
              Log.Write("DVBGraph: failed to add new radio channel for {0} {1} to database", station.Name, station.Frequency);
            }
            channelId = id;
            newRadioChannels++;
          }
          else
          {
            ...
            Log.Write("DVBGraph: channel {0} already exists in tv database", newchannel.ServiceName);
          }

          if ( Network() == NetworkType.DVBT )
          {
            Log.Write("DVBGraph: map radio channel {0} id:{1} to DVBT card:{2}", newchannel.ServiceName, channelId, ID);
          ...
          if ( Network() == NetworkType.DVBC )
          {
            Log.Write("DVBGraph: map radio channel {0} id:{1} to DVBC card:{2}", newchannel.ServiceName, channelId, ID);
          ...
          if ( Network() == NetworkType.DVBS )
          {
            Log.Write("DVBGraph: map radio channel {0} id:{1} to DVBS card:{2}", newchannel.ServiceName, channelId, ID);
            newchannel.ID = channelId;
        ...
        case NetworkType.ATSC:
          {
            ...
            {
              Log.Write("DVBGraph:database invalid tuning details for station:{0}", channel.ID);
              return;
            }
            Log.Write("DVBGraph:  tuning details: frequency:{0} kHz physicalChannel:{1} symbolrate:{2} innerFec:{3} modulation:{4} ONID:{5} TSID:{6} SID:{7} provider:{8}",
        ...
        case NetworkType.DVBC:
          {
            ...
            if ( frequency <= 0 )
            {
              Log.Write("DVBGraph:database invalid tuning details for channel:{0}", channel.Channel);
              return;
            }
            Log.Write("DVBGraph:  tuning details: frequency:{0} kHz symbolrate:{1} innerFec:{2} modulation:{3} ONID:{4} TSID:{5} SID:{6} provider:{7}",
              frequency, symbolrate, innerFec, modulation, ONID, TSID, SID, providerName);
    ...
    public void TuneRadioChannel( RadioStation channel )
    {
      ...
      try
      {
        ...
        _startTimer = DateTime.Now;
        Log.Write("DVBGraph:TuneRadioChannel() tune to radio station:{0}", channel.Name);
        ...
        Log.Write("DVBGraph:TuneRadioChannel() done");
    ...
    public void StartRadio( RadioStation station )
    {
      if ( _graphState != State.Radio )
      {
        ...
        {
          Log.Write("DVBGraph: FAILED cannot get IMediaControl");
        }

        TuneRadioChannel(station);
        _isGraphRunning = true;
        _graphState = State.Radio;
        Log.Write("DVBGraph:Listening to radio..");
        return;
      }
    ...
    protected void m_streamDemuxer_OnPMTIsChanged( byte[] pmtTable )
    {
      ...
      try
      {
        ...
        {
          Log.Write("DVBGraph: OnPMTIsChanged:{0}", pmtName);
        ...
        {
          Log.Write("DVBGraph: grab epg for {0}", _currentTuningObject.ServiceName);
    ...
    protected void SetHardwarePidFiltering()
    {
      ...
      if ( _inScanningMode == false )
      {
        ...
        Log.Write("DVBGraph:SetHardwarePidFiltering to:{0}", pidsText);
      }
      SendHWPids(pids);
      //      DumpMpeg2DemuxerMappings(_filterMpeg2Demultiplexer);
    }
    ...
    public bool GrabEpg( TVChannel channel )
    {
      if ( _graphState != State.Created )
        return false;
      // tune to the correct channel
      Log.Write("DVBGraph:Grab epg for :{0}", channel.Name);
    ...
    {
      _currentTimeShiftFileName = fileName;
      Log.Write("DVBGraph:add TsFileSink");
      ...
      for ( int i = 0; i < files.Length; ++i )
      {
        ...
        {
          Log.Write("DVBGraph:delete old file {0}", files[i]);
      ...
      if ( hr != 0 )
      {
        Log.Write("DVBGraph:FAILED cannot add MpgMux:{0:X}", hr);
        return false;
      }
      ...
      if ( !useAc3 )
      {
        ...
        if ( hr != 0 )
        {
          Log.Write("DVBGraph:FAILED cannot connect audio mpeg2 demux->mux input 1:{0:X}", hr);
          return false;
        }
      }
      else
      {
        ...
        if ( hr != 0 )
        {
          Log.Write("DVBGraph:FAILED cannot connect ac3 mpeg2 demux->mux input 1:{0:X}", hr);
          return false;
        }
      }
      ...
      if ( _currentTuningObject.ServiceType == Mpeg4VideoServiceType )
      {
        ...
        if ( hr != 0 )
        {
          Log.Write("DVBGraph:FAILED cannot connect video mpeg4 demux->mux input 2:{0:X}", hr);
          return false;
        }
      }
      else
      {
        ...
        if ( hr != 0 )
        {
          Log.Write("DVBGraph:FAILED cannot connect video mpeg2 demux->mux input 2:{0:X}", hr);
          return false;
        }
      }
      ...
      if ( hr != 0 )
      {
        Log.Write("DVBGraph:FAILED cannot add TsFileSink:{0:X}", hr);
        return false;
      }
      IFileSinkFilter interfaceFile = _filterTsFileSink as IFileSinkFilter;
      if ( interfaceFile == null )
      {
        Log.Write("DVBGraph:FAILED cannot get IFileSinkFilter from TsFileSink");
        return false;
      }
      ...
      if (pinIn == null)
      {
        Log.Write("DVBGraph:FAILED cannot find input pin of TsFileSink");
        return false;
      }
      hr = _graphBuilder.Connect(_pinDemuxerTS, pinIn);
      if (hr != 0)
      {
        Log.Write("DVBGraph:FAILED cannot connect demuxer->TsFileSink:{0:X}", hr);
        return false;
      }*/
      return true;
    }
