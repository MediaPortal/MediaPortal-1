  ...
  public class TStreamBufferPlayer9 : BaseTStreamBufferPlayer
  {
    ...
    protected override bool GetInterfaces(string filename)
    {
      ...
      try
      {
        ...
        Log.Write("TSStreamBufferPlayer9:initialize tsfilesource");
        try
        {
          using (RegistryKey hklm = Registry.LocalMachine)
          {
            using (RegistryKey settings = hklm.OpenSubKey(@"SOFTWARE\TSFileSource\settings\default", true))
            {
              ...
              {
                Log.Write("set tsfilesource to manual mode");
                settings.SetValue("enableAuto", valueZero, RegistryValueKind.Binary);
              }
              else
              {
                Log.Write("set tsfilesource to auto mode");
        ...
        Log.Write("TSStreamBufferPlayer9:add tsfilesource to graph");
        int hr = _graphBuilder.AddFilter((IBaseFilter)_fileSource, "TsFileSource");
        if (hr != 0)
        {
          Log.Write("TSStreamBufferPlayer9:Failed to add SBE to graph");
          return false;
        }
        ...
        {
          Log.Write("TSStreamBufferPlayer9:add mpeg-2 demultiplexer to graph");
        ...
        if (autoBuildGraph == false)
        {
          Log.Write("TSStreamBufferPlayer9:create audio/video pins");
          //create mpeg-2 demux output pins
          IMpeg2Demultiplexer demuxer = _mpegDemux as IMpeg2Demultiplexer;

          Log.Write("TSStreamBufferPlayer9:created audio output pin");
          hr = demuxer.CreateOutputPin(GetAudioMpg2Media(), "Audio", out _pinAudio);
          if (hr != 0)
          {
            Log.Write("TSStreamBufferPlayer9 FAILED to create audio output pin on demuxer");
            return false;
          }
          if (_isRadio == false)
          {
            Log.Write("TSStreamBufferPlayer9:created video output pin");
            hr = demuxer.CreateOutputPin(GetVideoMpg2Media(), "Video", out _pinVideo);
            if (hr != 0)
            {
              Log.Write("TSStreamBufferPlayer9 FAILED to create video output pin on demuxer");
              return false;
            }
          }
        }
        ...
        {
          Log.Write("TSStreamBufferPlayer9:Failed to get IFileSourceFilter");
          return false;
        }
        //Log.Write("TSStreamBufferPlayer9: open file:{0}",filename);
        if (supplyMediaType)
        {
          Log.Write("TSStreamBufferPlayer9: open file with mediatype:{0}", filename);
        ...
        if (hr != 0)
        {
          Log.Write("TSStreamBufferPlayer9:Failed to open file:{0} :0x{1:x}", filename, hr);
          return false;
        }
        ...
        if (autoBuildGraph == false)
        {
          ...
          Log.Write("TSStreamBufferPlayer9:connect tsfilesource->mpeg2 demux");
          IPin pinTsOut = DsFindPin.ByDirection((IBaseFilter)_fileSource, PinDirection.Output, 0);
          if (pinTsOut == null)
          {
            Log.Write("TSStreamBufferPlayer9:failed to find output pin of tsfilesource");
            return false;
          }
          ...
          if (hr != 0)
          {
            Log.Write("TSStreamBufferPlayer9:failed to connect tsfilesource->mpeg2 demux:{0:X}", hr);
            return false;
          }
          ...
          if (_isRadio == false)
          {
            Log.Write("TSStreamBufferPlayer9: map pid 0xe0->video pin");
            pStreamId = (IMPEG2StreamIdMap)_pinVideo;
            hr = pStreamId.MapStreamId(0xe0, MPEG2Program.ElementaryStream, 0, 0);
            if (hr != 0)
            {
              Log.Write("TSStreamBufferPlayer9: failed to map pid 0xe0->video pin");
              return false;
            }
          }
          Log.Write("TSStreamBufferPlayer9: map audio 0xc0->audio pin");
          pStreamId = (IMPEG2StreamIdMap)_pinAudio;
          hr = pStreamId.MapStreamId(0xc0, MPEG2Program.ElementaryStream, 0, 0);
          if (hr != 0)
          {
            Log.Write("TSStreamBufferPlayer9: failed  to map pid 0xc0->audio pin");
            return false;
          }
          ...
          Log.Write("TSStreamBufferPlayer9:render audio output pin");
          hr = _graphBuilder.Render(_pinAudio);
          if (hr != 0)
          {
            Log.Write("TSStreamBufferPlayer9:failed to render video output pin:{0:X}", hr);
          }

          if (_isRadio == false)
          {
            Log.Write("TSStreamBufferPlayer9:render video output pin");
            hr = _graphBuilder.Render(_pinVideo);
            if (hr != 0)
            {
              Log.Write("TSStreamBufferPlayer9:failed to render audio output pin:{0:X}", hr);
            }
          }
          #endregion
        ...
        {
          Log.Write("TSStreamBufferPlayer9:render tsfilesource outputs");
      ...
      {
        Log.Write("TSStreamBufferPlayer9:exception while creating DShow graph {0} {1}", ex.Message, ex.StackTrace);
        return false;
      }
    }
    ...
    void Cleanup()
    {
      ...
      {
        Log.Write("TSStreamBufferPlayer9: Exception while cleaning DShow graph - {0} {1}", ex.Message, ex.StackTrace);
      }
    ...
    public override void SeekAbsolute(double dTimeInSecs)
    {
      ...
      Log.Write("SeekAbsolute:seekabs:{0} duration:{1}", dTimeInSecs, Duration);
