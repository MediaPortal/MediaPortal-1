  ...
  public class DVDPlayer : IPlayer
  {
            ...
            try
            {
              ...
              if (hr != 0)
              {
                if (((uint)hr) == VFW_E_DVD_OPERATION_INHIBITED)
                  Log.Write("DVDPlayer:PlayAtTimeInTitle( {0}:{1:00}:{2:00}) not allowed at this point", hours, minutes, seconds);
                else if (((uint)hr) == VFW_E_DVD_INVALIDDOMAIN)
                  Log.Write("DVDPlayer:PlayAtTimeInTitle( {0}:{1:00}:{2:00}) invalid domain", hours, minutes, seconds);
                else
                  Log.WriteFile(Log.LogType.Log, true, "DVDPlayer:PlayAtTimeInTitle( {0}:{1:00}:{2:00}) failed:0x{3:X}", hours, minutes, seconds, hr);
              }
              //SetDefaultLanguages();
    ...
    private void SetDefaultLanguages()
    {
      ...
      if (lCID >= 0)
      {
        ...
        Log.Write("DVDPlayer:Set default language:{0} {1} {2}", _defaultAudioLanguage, lCID, errorText);
      }
      ...
      if (lCID >= 0)
      {
        ...
        Log.Write("DVDPlayer:Set default menu language:{0} {1} {2}", _defaultSubtitleLanguage, lCID, errorText);
      }
      lCID = GetLCID(_defaultSubtitleLanguage);
      if (lCID >= 0)
      {
        ...
        Log.Write("DVDPlayer:Set subtitle language:{0} {1} {2}", _defaultSubtitleLanguage, lCID, errorText);
      }
    ...
    public override int CurrentSubtitleStream
    {
      get
      {
        ...
        if (hr != 0)
        {
          Log.Write("DVDPlayer:CurrentSubtitleStream:Unable to get current subpicture with code {0}", hr);
          return 0;
        }
      ...
      set
      {
        ...
        if (hr != 0)
        {
          Log.Write("DVDPlayer:CurrentSubtitleStream:Unable to set current subpicture with code {0}", hr);
          return;
        }
    ...
    public override string SubtitleLanguage(int iStream)
    {
      ...
      if (hr == 0)
      {
        ...
        Log.Write("DVDPlayer:Failed translating subpicture ID to string");
        return Strings.Unknown;
      }
      Log.Write("DVDPlayer:Failed translating subpicture ID to string with error code {0}", hr);
      return Strings.Unknown;
    }
    ...
    public override bool EnableSubtitle
    {
      get
      {
        ...
        if (hr != 0)
        {
          Log.Write("DVDPlayer:EnableSubtitle:Unable to get subpicture state (enabled/disabled) with code {0}", hr);
          return false;
        }
      ...
      set
      {
        ...
        if (hr != 0)
        {
          Log.Write("DVDPlayer:EnableSubtitle:Unable to set subpicture state (enabled/disabled) with code {0}", hr);
          return;
        }
