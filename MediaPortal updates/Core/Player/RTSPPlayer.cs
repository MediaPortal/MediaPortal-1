  ...
  public class RTSPPlayer : IPlayer
  {
    ...
    protected  bool GetInterfaces()
    {
      ...
      try
      {
        ...
        Log.Write("RTSPPlayer: add source filter");
        ...
        if (hr != 0)
        {
          Log.Write("RTSPPlayer:unable to add RTSP source filter:{0:X}", hr);
          return false;
        }

        // add preferred video & audio codecs
        Log.Write("RTSPPlayer: add video/audio codecs");
        ...
        Log.Write("RTSPPlayer: load:{0}", m_strCurrentFile);
        IFileSourceFilter interfaceFile = (IFileSourceFilter)rtspSource;
        if (interfaceFile == null)
        {
          Log.Write("RTSPPlayer:Failed to get IFileSourceFilter");
          return false;
        }
        ...
        if (hr != 0)
        {
          Log.Write("RTSPPlayer:Failed to open file:{0} :0x{1:x}", m_strCurrentFile, hr);
          return false;
        }
        Log.Write("RTSPPlayer: render output pins of rtsp filter");
        DirectShowUtil.RenderOutputPins(graphBuilder, (IBaseFilter)rtspSource);

        if (!Vmr9.IsVMR9Connected)
        {
          //VMR9 is not supported, switch to overlay
          Log.Write("RTSPPlayer: vmr9 not connected");
          mediaCtrl = null;
          Cleanup();
          return false ;
        }
        ...
        Log.Write("RTSPPlayer: graph build successfull");
        return true;
      }
      catch (Exception ex)
      {
        Error.SetError("Unable to play movie", "Unable build graph for VMR9");
        Log.Write("RTSPPlayer:exception while creating DShow graph {0} {1}", ex.Message, ex.StackTrace);
        return false;
      }
    }
    ...
    void Cleanup()
    {
      ...
      {
        Log.Write("RTSPPlayer: Exception while cleanuping DShow graph - {0} {1}", ex.Message, ex.StackTrace);
      }
